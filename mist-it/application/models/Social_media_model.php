...
class Social_media_model extends CI_Model
{
	...
	public function update_auto_increament_value($arrdata,$arrwhere = array())
	{
		   ...
		   $this->db->where($arrwhere);
			...
			$result = $this->db->update('social_media', $arrdata);
	...
	public function get_all_social_media_record_datatable($empver_aary = array(),$where,$columns)
	{
		$this->db->select("candidates_info.CandidateName,candidates_info.ClientRefNumber,candidates_info.cmp_ref_no,candidates_info.caserecddate,(select user_name from user_profile where user_profile.id = social_media.has_case_id) as user_name,status.status_value,social_media.social_media_com_ref,social_media_result.verfstatus,social_media_result.first_qc_approve,social_media_result.first_qc_updated_on,social_media_result.first_qu_reject_reason,social_media.id,social_media.mode_of_veri,social_media.id as social_media_id,social_media.has_assigned_on,social_media.iniated_date,clients.clientname,(select created_on from social_media_activity_data where comp_table_id = social_media.id  order by id desc limit 1) as last_activity_date,social_media_result.remarks,due_date,tat_status,closuredate,social_media_insuff.insuff_raised_date");
		...
		$this->db->join("social_media_insuff",'(social_media_insuff.social_media_id = social_media.id AND  social_media_insuff.status = 1 )','left');

		
		$this->db->join("clients",'clients.id = candidates_info.clientid');

		$this->db->join("status",'status.id = social_media_result.verfstatus');
		...
		{ 
	            ...
	            if($where['status'] == "Closed")
                {
		     	...
		     	$where3 = "DATE_FORMAT(`social_media_result`.`closuredate`,'%Y-%m-%d') BETWEEN '$start_date' AND '$end_date'";
		     	...
		     	if($where['status'] == "Insufficiency")
                {
		     	...
		     	$where3 = "DATE_FORMAT(`social_media_insuff`.`insuff_raised_date`,'%Y-%m-%d') BETWEEN '$start_date' AND '$end_date'";
		...
		if(is_array($where) && $where['search']['value'] != "")
		{
			...
			$this->db->or_like('social_media.social_media_com_ref', $where['search']['value']);

			$this->db->or_like('social_media.iniated_date', $where['search']['value']);
	...
	public function get_all_social_media_record_datatable_count($empver_aary = array(),$where,$columns)
	{
		$this->db->select("candidates_info.id as cands_id,candidates_info.CandidateName,candidates_info.ClientRefNumber,candidates_info.cmp_ref_no,candidates_info.caserecddate,(select user_name from user_profile where user_profile.id = social_media.has_case_id) as user_name,status.status_value,social_media.id as social_media_id,social_media.social_media_com_ref,social_media_result.verfstatus,social_media_result.first_qc_approve,social_media_result.first_qc_updated_on,social_media_result.first_qu_reject_reason,social_media.id,social_media.has_assigned_on,social_media.iniated_date,clients.clientname,(select created_on from social_media_activity_data where comp_table_id = social_media.id  order by id desc limit 1) as last_activity_date,social_media_insuff.insuff_raised_date");
	...
	public function get_all_social_media_record($where_array)
	{
		$this->db->select("candidates_info.id as cands_id,candidates_info.CandidateName,candidates_info.ClientRefNumber,candidates_info.cmp_ref_no,candidates_info.caserecddate,(select user_name from user_profile where user_profile.id = social_media.has_case_id) as executive_name,status.status_value as verfstatus,social_media.id as social_media_id,social_media.has_case_id, social_media.social_media_com_ref,social_media.social_media_re_open_date,social_media.mode_of_veri,social_media.clientid,social_media.build_date,social_media_result.id as social_media_result_id,social_media_result.first_qc_approve,social_media_result.first_qc_updated_on,social_media_result.first_qu_reject_reason,social_media_result.mode_of_verification,social_media_result.remarks,social_media_result.closuredate,social_media.has_assigned_on,social_media.iniated_date,clients.clientname,(select created_on from social_media_activity_data where comp_table_id = social_media.id  order by id desc limit 1) as last_activity_date,due_date,tat_status,social_media_result.var_filter_status,(select entity_package_name from entity_package where entity_package.id = candidates_info.entity limit 1) as entity_name,(select entity_package_name from entity_package where entity_package.id = candidates_info.package limit 1) as package_name");
	...
	{
		$res =  $this->db->insert_batch('social_media_files', $files_arry);
	...
	{	
		...
		$result = $this->db->update('social_media_files', array('status' => STATUS_DELETED));
	...
	{	
		...
		$result = $this->db->update('social_media_files', array('status' => STATUS_ACTIVE));
	...
	public function select_file($select_array,$where_array)
	{
		...
		$this->db->from('social_media_files');
     ...
     public function update_due_date($files_arry,$arrwhere )
	{
         ...
         if(!empty($arrwhere))
	    {
			...
			$result = $this->db->update("social_media", $files_arry);
	...
	{
		$this->db->update_batch('social_media_files',$updateArray, 'id');
		return true; 
	}

	public function select_insuff($where_array)
	{
		$this->db->select('*')->from('social_media_insuff');
	...
	public function save_update_insuff($arrdata,$arrwhere = array())
	{
		if(!empty($arrwhere))
	    {
			...
			$result = $this->db->update('social_media_insuff', $arrdata);
	    ...
	    {
			$this->db->insert('social_media_insuff', $arrdata);
	...
	{
		$this->db->select('social_media_insuff.*,(select user_name from user_profile where id =  social_media_insuff.created_by limit 1) as insuff_raised_by,(select user_name from user_profile where id = social_media_insuff.insuff_cleared_by limit 1) as insuff_cleared_by');

		$this->db->from('social_media_insuff');

		$this->db->where($where_array);
		
		$this->db->where('social_media_insuff.status !=',3);

		$this->db->order_by('social_media_insuff.id','asc');
	...
	public function save_update_ver_result($arrdata,$arrwhere = array())
	{
		if(!empty($arrwhere))
	    {
			...
			$result = $this->db->update('social_media_result', $arrdata);
	    ...
	    {
			$this->db->insert('social_media_result', $arrdata);
	...
	public function save_update_result_social_media($arrdata,$arrwhere = array())
	{
		if(!empty($arrwhere))
	    {
			$this->db->where($arrwhere);

			$result = $this->db->update('social_media_ver_result', $arrdata);
	    ...
	    {
			$this->db->insert('social_media_ver_result', $arrdata);
	...
	{
		$this->db->select('social_media_ver_result.*,(select created_by from activity_log where id =  social_media_ver_result.activity_log_id) as created_by1,(select user_name from user_profile where id = created_by1) as created_by,(select activity_mode from activity_log where id =  social_media_ver_result.activity_log_id) as activity_mode ,(select activity_status from activity_log where id =  social_media_ver_result.activity_log_id) as activity_status ,(select activity_type from activity_log where id =  social_media_ver_result.activity_log_id) as activity_type,(select action from activity_log where id =  social_media_ver_result.activity_log_id) as activity_action,(select GROUP_CONCAT(social_media_files.file_name) from social_media_files where `social_media_files`.`social_media_id` = `social_media_ver_result`.`social_media_id` AND `status` = 1 AND `type` = 1)  as file_names,(select GROUP_CONCAT(social_media_files.id) from social_media_files where `social_media_files`.`social_media_id` = `social_media_ver_result`.`social_media_id` AND `status` = 1 AND `type` = 1) as file_ids');

		$this->db->from('social_media_ver_result');

		$this->db->where($where_array);

		$this->db->order_by('social_media_ver_result.id','desc');
	...
	{
		$this->db->select('social_media_ver_result.*,(select activity_mode from activity_log where id =  social_media_ver_result.activity_log_id) as activity_mode ,(select activity_status from activity_log where id =  social_media_ver_result.activity_log_id) as activity_status ,(select activity_type from activity_log where id =  social_media_ver_result.activity_log_id) as activity_type,(select action from activity_log where id =  social_media_ver_result.activity_log_id) as activity_action');
    ...
    {
        $this->db->select("view_vendor_master_log.*,(select vendor_name from vendors where vendors.id= social_media_vendor_log.vendor_id) as vendor_name,social_media_vendor_log.case_id,(select user_name from user_profile where id = view_vendor_master_log.allocated_by) as allocated_by,(select user_name from user_profile where id = view_vendor_master_log.approval_by) as approval_by");

        $this->db->from('view_vendor_master_log');
        $this->db->join('social_media_vendor_log','social_media_vendor_log.id = view_vendor_master_log.case_id');
        $this->db->where($where_array);

        if(!empty($id))
        {
        	 $this->db->where('social_media_vendor_log.case_id',$id);
        }
     ...
     public function select_vendor_result_log($where_array,$id)
    {
        ...
        $this->db->select("view_vendor_master_log.*,(select vendor_name from vendors where vendors.id= social_media_vendor_log.vendor_id) as vendor_name,(select user_name from user_profile where id = view_vendor_master_log.allocated_by) as allocated_by,(select user_name from user_profile where id = view_vendor_master_log.approval_by) as approval_by,social_media.*,candidates_info.CandidateName,candidates_info.ClientRefNumber,candidates_info.cmp_ref_no,candidates_info.CandidateName,(select clientname from clients where clients.id = social_media.clientid limit 1) as clientname,
			(select entity_package_name from entity_package where entity_package.id = candidates_info.entity limit 1) as entity_name,(select entity_package_name from entity_package where entity_package.id = candidates_info.package limit 1) as package_name,social_media.iniated_date,candidates_info.caserecddate,social_media_com_ref,(select id from social_media_result where social_media_result.social_media_id = social_media.id) as social_media_id,view_vendor_master_log.id as view_vendor_master_log_id");
        ...
        $this->db->join('social_media','social_media.id = social_media_vendor_log.case_id');
  ...
  public function uploaded_venodr_cost_file($files_arry,$arrwhere )
	...
	public function select_client_list_view_social_media($tableName, $return_as_strict_row,$select_array, $where1=array())
	{
		$this->db->select($select_array);
       
		$this->db->from($tableName);

		$this->db->join("social_media",'social_media.clientid = clients.id');

		$this->db->join("user_profile",'user_profile.id = social_media.has_case_id','left');

		$this->db->join("candidates_info",'candidates_info.id = social_media.candsid');

	
		$this->db->join("social_media_result",'social_media_result.social_media_id = social_media.id','left');


		$this->db->join("social_media_insuff",'(social_media_insuff.social_media_id = social_media.id AND  social_media_insuff.status = 1 )','left');

		$this->db->join("status",'status.id = social_media_result.verfstatus','left');
	...
	public function save_update_identity_files($arrdata,$arrwhere = array())
	{
		if(!empty($arrwhere))
	    {
			...
			$result = $this->db->update('social_media_files', $arrdata);
	    ...
	    {
			$this->db->insert('social_media_files', $arrdata);
    ...
    public function select_social_media_details($table_name,$array_data,$where_array)
    {
