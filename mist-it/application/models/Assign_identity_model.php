...
class Assign_identity_model extends CI_Model
{
	...
	public function get_all_identity_record_datatable_assign($empver_aary = array(),$where,$columns)
	{
		

		$this->db->select("candidates_info.CandidateName,candidates_info.ClientRefNumber,candidates_info.cmp_ref_no,candidates_info.caserecddate,(select user_name from user_profile where user_profile.id = identity.has_case_id) as user_name,status.status_value,identity.identity_com_ref,identity_result.first_qc_approve,identity_result.first_qc_updated_on,identity_result.first_qu_reject_reason,identity.id,identity.id as identity_id,identity.has_assigned_on,identity.iniated_date,clients.clientname,(select created_on from identity_activity_data where comp_table_id = identity.id  order by id desc limit 1) as last_activity_date,identity_result.remarks,due_date,tat_status,closuredate,id_number,(select vendor_name from vendors where vendors.id = identity.vendor_id) as vendor_name,(select user_name from user_profile where user_profile.id = identity.has_case_id) as executive_name");
	...
	public function get_all_identity_record_datatable_count_assign($empver_aary = array(),$where,$columns)
	{
		$this->db->select("candidates_info.id as cands_id,candidates_info.CandidateName,candidates_info.ClientRefNumber,candidates_info.cmp_ref_no,candidates_info.caserecddate,(select user_name from user_profile where user_profile.id = identity.has_case_id) as user_name,status.status_value,identity.id as identity_id,identity.identity_com_ref,identity_result.first_qc_approve,identity_result.first_qc_updated_on,identity_result.first_qu_reject_reason,identity.id,identity.has_assigned_on,identity.iniated_date,clients.clientname,(select created_on from identity_activity_data where comp_table_id = identity.id  order by id desc limit 1) as last_activity_date,(select vendor_name from vendors where vendors.id = identity.vendor_id) as vendor_name,(select user_name from user_profile where user_profile.id = identity.has_case_id) as executive_name");
	...
	public function vendor_logs($where_array)
    {
     ...
     public function select_client_list_assign_identity_view($tableName,$return_as_strict_row,$select_array)
	{
		$this->db->select($select_array);
