...
class Assign_social_media_model extends CI_Model
{
	...
	public function get_all_social_media_record_datatable_assign($empver_aary = array(),$where,$columns)
	{
		

		$this->db->select("candidates_info.CandidateName,candidates_info.ClientRefNumber,candidates_info.cmp_ref_no,candidates_info.caserecddate,(select user_name from user_profile where user_profile.id = social_media.has_case_id) as user_name,status.status_value,social_media.social_media_com_ref,social_media_result.first_qc_approve,social_media_result.first_qc_updated_on,social_media_result.first_qu_reject_reason,social_media.id,social_media.id as social_media_id,social_media.has_assigned_on,social_media.iniated_date,clients.clientname,(select created_on from social_media_activity_data where comp_table_id = social_media.id  order by id desc limit 1) as last_activity_date,social_media_result.remarks,due_date,tat_status,closuredate,(select user_name from user_profile where user_profile.id = social_media.has_case_id) as executive_name");
		...
		$this->db->where('(social_media_result.var_filter_status = "wip" or social_media_result.var_filter_status = "WIP")');
	...
	public function get_all_social_media_record_datatable_count_assign($empver_aary = array(),$where,$columns)
	{
		$this->db->select("candidates_info.CandidateName,candidates_info.ClientRefNumber,candidates_info.cmp_ref_no,candidates_info.caserecddate,(select user_name from user_profile where user_profile.id = social_media.has_case_id) as user_name,status.status_value,social_media.social_media_com_ref,social_media_result.first_qc_approve,social_media_result.first_qc_updated_on,social_media_result.first_qu_reject_reason,social_media.id,social_media.id as social_media_id,social_media.has_assigned_on,social_media.iniated_date,clients.clientname,(select created_on from social_media_activity_data where comp_table_id = social_media.id  order by id desc limit 1) as last_activity_date,social_media_result.remarks,due_date,tat_status,closuredate,(select user_name from user_profile where user_profile.id = social_media.has_case_id) as executive_name");
    ...
    public function select_client_list_assign_social_media_view($tableName,$return_as_strict_row,$select_array)
	{
		$this->db->select($select_array);
