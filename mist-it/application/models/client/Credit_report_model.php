...
class Credit_report_model extends CI_Model
{
	...
	public function get_credit_report_data($where_arry,$comp)
	{
		$this->db->select("credit_report.*,candidates_info.CandidateName,candidates_info.id as cands_id,candidates_info.clientid, cr1.mode_of_verification,status.status_value");

		$this->db->from('candidates_info');

		$this->db->join("credit_report",'candidates_info.id = credit_report.candsid','left');

		$this->db->join("credit_report_result as cr1",'cr1.credit_report_id = credit_report.id','left');

		$this->db->join("status",'(status.id = cr1.verfstatus)');
		...
		if($comp)
		{
			$this->db->where('credit_report.credit_report_com_ref',$comp);
		}
	...
	public function get_credit_report_data_view($where_arry)
	{
