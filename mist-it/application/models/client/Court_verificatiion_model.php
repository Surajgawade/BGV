...
class Court_verificatiion_model extends CI_Model
{
	...
	public function get_all_court_client_view($where_arry)
	{
		$this->db->select("courtver.*,candidates_info.CandidateName,candidates_info.id as cands_id,candidates_info.clientid, a1.mode_of_verification,status.status_value");
	...
	public function get_data($where_arry,$comp)
	{
		$this->db->select("courtver.*,candidates_info.CandidateName,candidates_info.id as cands_id,candidates_info.clientid, a1.mode_of_verification,status.status_value");
		...
		if($comp)
		{
			$this->db->where('courtver.court_com_ref',$comp);
		}
