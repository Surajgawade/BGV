...
class Identity_model extends CI_Model
{
	...
	public function cand_identity_details_view($where_arry)
	{
		$this->db->select("identity.*,candidates_info.CandidateName,candidates_info.id as cands_id,candidates_info.clientid, i1.mode_of_verification,status.status_value");

		$this->db->from('candidates_info');

		$this->db->join("identity",'candidates_info.id = identity.candsid','left');

		$this->db->join("identity_result as i1",'i1.identity_id = identity.id','left');

		$this->db->join("status",'(status.id = i1.verfstatus)');
	...
	public function cand_identity_details($where_arry,$comp)
	{
		$this->db->select("identity.*,candidates_info.CandidateName,candidates_info.id as cands_id,candidates_info.clientid, i1.mode_of_verification,status.status_value");
		...
		if($comp)
		{
			$this->db->where('identity.identity_com_ref',$comp);
		}
