...
class Company_database_model extends CI_Model
{
	...
	public function get_other_database_details($where_arry = array())
	{
		$this->server_2 = $this->load->database('server_2', TRUE);
		$this->server_2->select("company_database.*");

		$this->server_2->from('company_database');

	    $this->server_2->like('company_database.coname', $where_arry);

		$result = $this->server_2->get();

		record_db_error($this->server_2->last_query());

		return $result->result_array();
	} 

	public function get_other_database($where_arry = array())
	{
		$this->server_2 = $this->load->database('server_2', TRUE);
		$this->server_2->select("company_database_verifiers_details.*,(select coname from company_database where company_database.id= company_database_verifiers_details.company_database_id) as coname");

		$this->server_2->from('company_database_verifiers_details');

	    $this->server_2->where_in('company_database_verifiers_details.company_database_id', $where_arry);
