...
class Empver_model extends CI_Model
{
    ...
    public function select_file($select_array,$where_array)
    {
     ...
     public function employment_case_list($where_array = array(),$where,$columns)
    {
        $this->db->select("view_vendor_master_log.*,CandidateName,clients.clientname,empver.emp_com_ref,empver.nameofthecompany,empver.deputed_company,empver.employment_type,empver.locationaddr,empver.citylocality,empver.state,empver.pincode,(select entity_package_name from entity_package where entity_package.id = candidates_info.entity limit 1) as entity_name,(select user_name from vendor_executive_login where vendor_executive_login.id = view_vendor_master_log.has_case_id limit 1) as vendor_executive_id,candidates_info.ClientRefNumber as client_ref_no");
        $this->db->from('view_vendor_master_log');
       // $this->db->join('empver','empver.id = view_vendor_master_log.component_tbl_id');
    ...
    public function employment_case_list_closed($where_array = array(),$where,$columns)
    {
     ...
     public function select_vendor_result_log($where_array,$id)
    {
        ...
        $this->db->select("empver.*,empver.has_case_id as executive_name,view_vendor_master_log.*,(select vendor_name from vendors where vendors.id= employment_vendor_log.vendor_id) as vendor_name,(select user_name from user_profile where id = view_vendor_master_log.allocated_by) as allocated_by,(select user_name from user_profile where id = view_vendor_master_log.approval_by) as approval_by,candidates_info.CandidateName,candidates_info.ClientRefNumber,CandidatesContactNumber,candidates_info.cmp_ref_no,candidates_info.CandidateName,(select clientname from clients where clients.id = empver.clientid limit 1) as clientname,
            (select entity_package_name from entity_package where entity_package.id = candidates_info.entity limit 1) as entity_name,(select entity_package_name from entity_package where entity_package.id = candidates_info.package limit 1) as package_name,empver.iniated_date,candidates_info.caserecddate,emp_com_ref");
