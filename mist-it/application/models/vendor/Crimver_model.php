...
class Crimver_model extends CI_Model
{
  ...
  public function select_file($select_array,$where_array)
    ...
    public function pcc_case_list_closed($where_array = array(),$where,$columns)
    {
     ...
     public function select_vendor_result_log($where_array,$id)
    {
        ...
        $this->db->select("pcc.*,view_vendor_master_log.*,(select vendor_name from vendors where vendors.id= pcc_vendor_log.vendor_id) as vendor_name,(select user_name from user_profile where id = view_vendor_master_log.allocated_by) as allocated_by,(select user_name from user_profile where id = view_vendor_master_log.approval_by) as approval_by,candidates_info.CandidateName,candidates_info.ClientRefNumber,CandidatesContactNumber,candidates_info.cmp_ref_no,candidates_info.CandidateName,(select clientname from clients where clients.id = pcc.clientid limit 1) as clientname,
            (select entity_package_name from entity_package where entity_package.id = candidates_info.entity limit 1) as entity_name,(select entity_package_name from entity_package where entity_package.id = candidates_info.package limit 1) as package_name,pcc.iniated_date, candidates_info.caserecddate, pcc_com_ref");
