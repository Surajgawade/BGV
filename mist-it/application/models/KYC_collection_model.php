...
class KYC_collection_model extends CI_Model
{
	function __construct()
    {
		$this->tableName = 'kyc_collection';
	...
	public function get_all_kyc_record_datatable($empver_aary = array(),$where,$columns)
	{
		$this->db->select("ClientRefNumber,kyc_ref_no,iniated_date,company_customer_name,street_address,city,pincode,state,(select user_name from user_profile where user_profile.id = kyc_collection.has_case_id) as user_name,status.status_value,clients.clientname,(select created_on from kyc_activity_data where comp_table_id = kyc_collection.id order by id desc limit 1) as last_activity_date,kyc_collection_res.remarks,due_date,tat_status,closuredate,kyc_collection_id,kyc_collection_res.id as kyc_collection_res_id,kyc_collection.id ");

		$this->db->from('kyc_collection');

		$this->db->join("kyc_collection_res",'kyc_collection_res.kyc_collection_id =kyc_collection.id');
				
		$this->db->join("clients",'clients.id = kyc_collection.clientid');

		$this->db->join("status",'status.id = kyc_collection_res.verfstatus');

		if(is_array($where) && $where['search']['value'] != "")
		{
			$this->db->like('ClientRefNumber', $where['search']['value']);

			$this->db->or_like('kyc_ref_no', $where['search']['value']);
	...
	public function get_all_kyc_record_datatable_count($empver_aary = array(),$where,$columns)
	{
		$this->db->select("ClientRefNumber,kyc_ref_no,iniated_date,company_customer_name,street_address,city,pincode,state,(select user_name from user_profile where user_profile.id = kyc_collection.has_case_id) as user_name,status.status_value,clients.clientname,(select created_on from kyc_activity_data where comp_table_id = kyc_collection.id order by id desc limit 1) as last_activity_date,kyc_collection_res.remarks,due_date,tat_status,closuredate,kyc_collection_id,kyc_collection_res.id as kyc_collection_res_id,kyc_collection.id");
	...
	{
		$this->db->select('*')->from('kyc_collection_insuff');
	...
	public function save_update_insuff($arrdata,$arrwhere = array())
	{
		if(!empty($arrwhere)) {
			...
			$this->db->where($arrwhere);
			$result = $this->db->update('kyc_collection_insuff', $arrdata);
			record_db_error($this->db->last_query());
			return $result;

	    } else {

			$this->db->insert('kyc_collection_insuff', $arrdata);
			record_db_error($this->db->last_query());
			return $this->db->insert_id();
	...
	public function upload_file_update($tableName = 'kyc_files',$updateArray)
	{
	...
	public function KYC_ref_no()
	{
		$result = $this->db->select("SUBSTRING_INDEX(kyc_ref_no, '-',-1) as A_I")->order_by('id','desc')->limit(1)-> get($this->tableName)->row_array();
		return $result;
	}
