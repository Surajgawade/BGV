...
class Assign_global_database_model extends CI_Model
{
	...
	public function get_all_global_record_datatable_assign($empver_aary = array(),$where,$columns)
	{
		$this->db->select("candidates_info.CandidateName,candidates_info.ClientRefNumber,candidates_info.cmp_ref_no,candidates_info.caserecddate,(select user_name from user_profile where user_profile.id = glodbver.has_case_id) as executive_name,status.status_value,glodbver.global_com_ref,glodbver.id as glodbver_id,glodbver.street_address,glodbver_result.first_qc_approve,glodbver_result.first_qc_updated_on,glodbver_result.first_qu_reject_reason,glodbver.id,glodbver.has_assigned_on,glodbver.iniated_date,clients.clientname,due_date,tat_status,glodbver_result.closuredate,glodbver_result.remarks,(select created_on from glodbver_activity_data where comp_table_id = glodbver.id order by id desc limit 1) as last_activity_date,(select vendor_name from vendors where vendors.id = glodbver.vendor_id) as vendor_name");
	...
	public function get_all_global_record_datatable_count_assign($empver_aary = array(),$where,$columns)
	{
		$this->db->select("candidates_info.CandidateName,candidates_info.ClientRefNumber,candidates_info.cmp_ref_no,candidates_info.caserecddate,(select user_name from user_profile where user_profile.id = glodbver.has_case_id) as executive_name,status.status_value,glodbver.global_com_ref,glodbver.address_type,glodbver.street_address,glodbver.city,glodbver.pincode,glodbver.state,glodbver_result.first_qc_approve,glodbver_result.first_qc_updated_on,glodbver_result.first_qu_reject_reason,glodbver.id,glodbver.has_assigned_on,glodbver.iniated_date,clients.clientname,tat_status,(select created_on from glodbver_activity_data where comp_table_id = glodbver.id order by id desc limit 1) as last_activity_date");
	...
	public function upload_file_update($tableName = 'glodbver_files', $updateArray)
	{
	...
	public function select_client_list_assign_global_view($tableName,$return_as_strict_row,$select_array)
	{
		$this->db->select($select_array);

		$this->db->from($tableName);

        $this->db->join("glodbver",'glodbver.clientid = clients.id');
