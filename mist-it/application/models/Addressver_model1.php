...
*/
	...
	/*public function get_employee_address_under()
	{
	...
	public function get_all_addrs_by_client_datatable($get_employee_id,$where1,$columns)
	{
	     ...
	     if(!empty($get_employee_id))
	     {
           ...
           $where = "(`has_case_id` IN (".$get_employee_id1.") or `addrver`.`created_by` IN (".$get_employee_id1.") or `has_case_id` = ".$user_id." or `addrver`.`created_by` =".$user_id.")"; 
	     }
	     else
	     {
	     	$where = "(`has_case_id` = ".$user_id." or `addrver`.`created_by` =".$user_id.")";
	     }
		...
		{
			$this->db->like('candidates_info.ClientRefNumber', $where1['search']['value']);
		...
		if(!empty($where1['order']))
		{
		    ...
		    else
		    {
               ...
               $order_clause = "(case verfstatus when 12 THEN 0 else 1 end),(case verfstatus when 13 THEN 0 else 1 end),(case verfstatus when 11 THEN 0 else 1 end),(case verfstatus when 14 THEN 0 else 1 end)";
	...
	public function get_all_addrs_by_client_datatable_count($get_employee_id,$where1,$columns)
	{
...
GROUP BY `addrverres`.`addrverid`
    ...
    public function get_all_address_by_client($clientid,$filter_by_status,$from_date,$to_date)
	{	
	...
	public function vendor_logs_digital($where_array,$id)
    {
        $this->db->select("addrver.id as address_id,(select vendor_name from vendors where vendors.id= address_vendor_log.vendor_id) as vendor_name,address_vendor_log.case_id,(select user_name from user_profile where id = address_vendor_log.created_by) as approval_by,address_vendor_log.created_on as approval_on");
        ...
        $this->db->where('addrver.vendor_id',25);
...
*/
  ...
  public function get_date_for_update($where_array)
    ...
    {   
        ...
        $this->db->select("addrver.*,candidates_info.CandidateName,(select vendor_name from vendors where vendors.id = addrver.vendor_id) as vendor_name,candidates_info.ClientRefNumber,(select clientname from clients where clients.id = candidates_info.clientid) as clientname,(select entity_package_name from entity_package where entity_package.id = candidates_info.entity limit 1) as entity_name,addrver.id as address_id");
        ...
        $this->db->where('address_is_mail_sent',1);

        $this->db->where('address_is_sms_sent',1); 
    ...
    {
    	$where_condition = "(view_vendor_master_log.final_status = 'candidate shifted' or view_vendor_master_log.final_status = 'unable to verify')" ;
...
*/
   ...
   public function select_file($select_array,$where_array)
    ...
    {
    	$this->db->select('clients.clientname,addrver.add_com_ref,candidates_info.CandidateName,candidates_info.CandidatesContactNumber,candidates_info.ContactNo1,candidates_info.ContactNo2,addrver.vendor_id,addrver.address,addrver.city,addrver.pincode,addrver.state');
	...
	{
		if(!empty($arrwhere)) {
			$this->db->select('addrver.*,candidates_info.CandidateName,candidates_info.cands_email_id as candidate_email,candidates_info.CandidatesContactNumber as candidate_contact');
	    	$this->db->from('addrver');
	    	$this->db->join('candidates_info','candidates_info.id = addrver.candsid');
