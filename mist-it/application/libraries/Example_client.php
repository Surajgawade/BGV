...
class Example_client extends TCPDF {
	...
	public function generate_pdf($reports_data,$portal_type)
	{
	   ...
	   set_time_limit(0);
		...
		$pdf = new Example_client(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
    ...
    { 
       ...
       $background_tbl .=  '<tr>
          <td  rowspan = "4" border = "1">'.ucwords($value["coname"]).'</td>
          <td  rowspan = "4" border = "1">'.$citylocality.'</td>
          <td  rowspan = "4" border = "1">'.$employed_from.'</td>
          <td  rowspan = "4" border = "1">'.$employed_to.'</td>
          <td  rowspan = "4" border = "1">'.$years_month.'</td>
...
Any questions or clarifications with respect to the report can be addressed to MIST IT Services Private Ltd.
        ...
        $pdf->Output(ucwords('Report'.'.pdf', 'I'));
