  ...
  <br>
    <div class="nav-tabs-custom">
        <ul class="nav nav-pills nav-justified"> 
           ...
           <?php  
            echo "<li class='nav-item waves-effect waves-light res active'  role='presentation' data-tab_name='reference_list_view' ><a class = 'nav-link active' href='#reference_list_view' aria-controls='home' data-toggle='tab'>Reference List View</a></li>";
            
            echo "<li  role='presentation' data-url='".ADMIN_SITE_URL."reference_verificatiion/get_assign_reference_list_view/'  data-tab_name='assign_reference_list_view'  class='nav-item waves-effect waves-light view_assign_tab'><a class = 'nav-link' href='#assign_reference_list_view' aria-controls='home' role='tab'  data-toggle='tab'>New Cases</a></li>";
...
<div class="tab-content">
     ...
     <div class="row">
         <div class="col-sm-12">
            <div class="page-title-box">
                <h4 class="page-title">Reference - List View</h4>
                  <ol class="breadcrumb">
                  ...
                  <div class="state-information d-none d-sm-block">
                    <ol class="breadcrumb">
                       <?php
                          $access_export = ($this->permission['access_reference_list_export']) ? '#myModalExport'  : '';
                          $access_import = ($this->permission['access_reference_list_import']) ? '#myModelImport'  : ''; 
                       ?>
  ...
  <div id="assign_reference_list_view" class="tab-pane fade in">
...
<div id="myModelImport" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Import Reference Records</h4>
      </div>
...
<div id="myModalExport" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog" style="width: 700px !important;">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Export Reference Verification Records</h4>
      </div>
...
$(document).ready(function() {
  ...
  var oTable =  $('#tbl_datatable').DataTable( {
        ...
        "ajax": $.fn.dataTable.pipeline( {
            url: "<?php echo ADMIN_SITE_URL.'reference_verificatiion/reference_view_datatable'; ?>",
            pages: 1,
            async: true,
            method: 'POST',
        ...
        "columns":[{'data' :'id'},{'data' :'ClientRefNumber'},{"data":"reference_com_ref"},{"data":"iniated_date"},{"data":"clientname"},{'data' :'CandidateName'},{'data' :'vendor_name'},{'data' :'name_of_reference'},{"data":"verfstatus"},{"data":"executive_name"},{'data':'tat_status'},{'data':'due_date'},{'data':"last_activity_date"},{'data':"mode_of_veri"},{'data':"cmp_ref_no"}],
  ...
  $('#filter_by_executive,#filter_by_clientid,#filter_by_status,#filter_by_sub_status').on('change', function(){
    ...
    var new_url = "<?php echo ADMIN_SITE_URL.'reference_verificatiion/reference_view_datatable'; ?>?status="+status+"&sub_status="+sub_status+"&client_id="+client_id+"&filter_by_executive="+filter_by_executive;
   ...
   $('#filter_by_executive,#filter_by_status,#filter_by_sub_status').on('change', function(){
    ...
    var sub_status = $('#filter_by_sub_status').val();
     ...
     $.ajax({
          type:'POST',
          url:'<?php echo ADMIN_SITE_URL.'reference_verificatiion/get_clients_for_reference_list_view/' ?>',
  ...
  $('#searchrecords').on('click', function() {
    ...
    {
    var new_url = "<?php echo ADMIN_SITE_URL.'reference_verificatiion/reference_view_datatable'; ?>?status="+status+"&sub_status="+sub_status+"&client_id="+client_id+"&start_date="+start_date+"&end_date="+end_date+"&filter_by_executive="+filter_by_executive;
    oTable.ajax.url(new_url).load();
   }
  });
...
$('#btn_reset').on('click', function() {
   ...
   $("#filter_by_status option[value = All]").attr('selected','selected');
    ...
    var new_url = "<?php echo ADMIN_SITE_URL.'reference_verificatiion/reference_view_datatable'; ?>?status="+status+"&filter_by_executive="+filter_by_executive;
    oTable.ajax.url(new_url).load();
  });
  ...
  $('#frm_download_tem_and_import').validate({ 
      ...
      {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'reference_verificatiion/bulk_upload_reference'; ?>',
...
$('#download_candidate_template').on('click',function(){
        ...
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'reference_verificatiion/template_download'; ?>',
          data : '',
          type: 'post',
          dataType:'json',
          beforeSend:function(){
     ...
     $('#export_to_excel_frm').validate({ 
        ...
        {      
            ...
            $.ajax({
            url: '<?php echo ADMIN_SITE_URL.'Reference_verificatiion/export_to_excel'; ?>',
            data: dataString,
            type: 'POST',
            beforeSend:function(){
