    ...
    <div class="col-sm-12">
      <div class="page-title-box">
        <div class="text-white div-header">
           Credit Report Case Details
        </div>
          <div style="float: right;">
            <ol class="breadcrumb">
              <li> <button class="btn btn-secondary waves-effect btn-sm  edit_btn_click" data-frm_name='frm_credit_report_update' data-editUrl="<?= ($this->permission['access_credit_list_edit']) ? encrypt($selected_data['credit_report_id']) : ''?>"><i class="fa fa-edit"></i> Edit</button></li>
             
              <li><button class="btn btn-secondary waves-effect btn-sm" data-accessUrl="<?= ADMIN_SITE_URL.'credit_details/delete/'.encrypt($selected_data['credit_report_id']); ?>"><i class="fa fa-trash"></i> Delete</button></li>
            </ol>
          </div>
      </div>
    </div>
...
<div class="box-body">
  <?php echo form_open_multipart('#', array('name'=>'frm_credit_report_update','id'=>'frm_credit_report_update')); ?>
    <div class="row">
     ...
     <div class="row">
      <div class="col-sm-4 form-group">
        ...
        <input type="text" name="credit_report_com_ref" id="credit_report_com_ref"  value="<?php echo set_value('credit_report_com_ref',$selected_data['credit_report_com_ref']); ?>" class="form-control readonly">
        <?php echo form_error('credit_report_com_ref'); ?>
      </div>
      <div class="col-sm-4 form-group">
    ...
    ?>
      ...
      <div class="col-sm-4 form-group">
        <label>Mode of Verification</label>
         <input type="text" name="mode_of_veri" readonly="readonly" id="mode_of_veri" value="<?php if(isset($mode_of_verification_value->cbrver)) { echo $mode_of_verification_value->cbrver; } ?>" class="form-control cls_disabled">
     ...
     <div class="row">
      <div class="col-sm-4 form-group">
       ...
       <input type="text" name="re_iniated_date" id="re_iniated_date" value="<?php echo set_value('re_iniated_date',$selected_data['credit_report_re_open_date']); ?>" class="form-control cls_disabled myDatepicker" placeholder='DD-MM-YYYY'>
        <?php echo form_error('iniated_date'); ?>
      </div>
    </div>
      ...
      <div class="col-sm-4 form-group">
        <label>Gender</label>
       <?php
          echo form_dropdown('gender', GENDER, set_value('gender',$get_cands_details['gender']), 'class="select2 gen" readonly id="gender"');
      ...
      <div class="col-sm-4 form-group">
        ...
        <?php
         echo form_dropdown('doc_submited', array('' => 'Select', 'pan card' => 'PAN Card'), set_value('doc_submited',$selected_data['doc_submited']), 'class="select2 cls_disabled" id="doc_submited"');
          echo form_error('doc_submited'); 
        ?>
      </div>
      <div class="col-sm-4 form-group">
        <label>Id Number</label>
        <input type="text" name="id_number" id="id_number" value="<?php echo set_value('id_number',$selected_data['id_number']); ?>" class="form-control cls_disabled">
    ...
    foreach ($attachments as $key => $value) {
      //$url  = SITE_URL.UPLOAD_FOLDER.CANDIDATES.$candidate_details['clientid'].'/';
         $url  = SITE_URL.CREDIT_REPORT.$selected_data['clientid'].'/';
      if($value['type'] == 0)
      {
  ...
  <div class="col-md-12 col-sm-12 col-xs-12 form-group">
    <div style="float: right;">
      ...
      ="<?= ADMIN_SITE_URL.'activity_log/activity_log_view/2/'.$selected_data['credit_report_id'] ?>/credit_report" <?=$check_insuff_raise?> data-toggle="modal" id="activityModelClk">Activity</button>
    </div>
  </div>
</div>
...
$(document).ready(function(){
  ...
  $('#frm_credit_report_update').validate({ 
    rules: {
      identity_id : {
        required : true
      },
    ...
    {
      $.ajax({
        url : '<?php echo ADMIN_SITE_URL.'credit_report/update_form'; ?>',
 ...
 $('.remove_file').on('click',function(){
  var id =  <?= ($this->permission['access_credit_list_edit']) ? $selected_data['credit_report_id'] : '"permission denied"'?>;
  if(id != "permission denied")
  {
      ...
      if (confirmr == true) 
      {
          ...
          $.ajax({
              url: "<?php  echo ADMIN_SITE_URL.'credit_report/remove_uploaded_file/' ?>"+remove_id,
              type: 'GET',
              beforeSend:function(){
