<div class="content-wrapper">
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><button class="btn btn-sm btn-default btn_clicked" data-accessUrl="<?= ($this->permission['access_add_de']) ? ADMIN_SITE_URL.'KYC_collection/add' : '' ?>"><i class="fa fa-plus"></i> Add</button></li>
                ...
                <table id="tbl_datatable_kyc" class="table table-bordered table-hover">
                  <thead>
                    <tr class="filters">
                      ...
                      <th><input type="text" class="form-control" placeholder="KYC Ref No" disabled></th>
                      <th><input type="text" class="form-control" placeholder="KYC Int Date" disabled></th>
...
<div id="myModelImport" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Import KYC Records</h4>
      </div>
      <?php echo form_open_multipart('#', array('name'=>'frm_download_tem_and_import','id'=>'frm_download_tem_and_import')); ?>   
      <div class="modal-body">
        <div class="row">
...
$(document).ready(function() {
  $.fn.dataTable.pipeline = function ( opts ) {
    ...
    var conf = $.extend( {
        ...
        url: '<?php echo ADMIN_SITE_URL.'KYC_collection/kyc_collection_view_datatable'; ?>',
        data: null,
  ...
  var oTable =  $('#tbl_datatable_kyc').DataTable( {
        ...
        "columns":[{'data':'checkbox'},{'data' :'id'},{'data' :'ClientRefNumber'},{"data":"kyc_ref_no"},{"data":"iniated_date"},{"data":"clientname"},{'data' :'company_customer_name'},{'data' :'verfstatus'},{"data":"vendor"},{"data":"executive_name"},{'data':'tat_status'},{'data':'due_date'},{'data':"last_activity_date"}]
  } );
  ...
  $('#filter_by_clientid,#filter_by_status').on('change', function(){
    ...
    var new_url = "<?php echo ADMIN_SITE_URL.'identity/identity_view_datatable'; ?>?filter_by_clientid="+filter_by_clientid+"&filter_by_status="+filter_by_status;
  ...
  $('#btn_assign_action').on('click', function(e){
    ...
    if(users_list != '0'  && select_one != "")
    {
        $.ajax({
          type:'POST',
          url:'<?php echo ADMIN_SITE_URL.'KYC_collection/assign_to_executive/' ?>',
    ...
    if(vendor_list != "0" && select_one != "")
    {
        $.ajax({
          type:'POST',
          url:'<?php echo ADMIN_SITE_URL.'KYC_collection/assign_to_vendor/' ?>',
  ...
  $('#export_activity_frm').validate({ 
        ...
        {      
            ...
            $.ajax({
            url: '<?php echo ADMIN_SITE_URL.'pcc/export_activity_records'; ?>',
  ...
  $('#frm_download_tem_and_import').validate({ 
      ...
      {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'KYC_collection/bulk_upload_candidates'; ?>',
...
$('#download_candidate_template').click(function(e) {
    e.preventDefault();
    window.location.href = "<?= COM_ATTACHMENTS_PATH.KYC.'/kyc_bulk_template.xlsx'; ?>" ;
});
