                    ...
                    <table id="tbl_vendor_insuff" class="table table-bordered  nowrap"  style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                      ...
                      <?php
                        ...
                        foreach ($vendor_executive_list as $key => $value) {
                            ...
                            $html_view .= "<tr id = ".$value['address_id']." url = ".ADMIN_SITE_URL."address/View_vendor_log1/".$value['id']." >";
                            $html_view .= "<td>".$i."</td>";
...
$('#tbl_vendor_insuff').on('dblclick', 'tr', function () { 
    ...
    $('#append_vendor_model_insuff_details').load(url,function(){
    $('#insuffRaiseModelVendor').modal('show');
    $('#insuffRaiseModelVendor').addClass("show");
    $('#insuffRaiseModelVendor').css({background: "#0000004d"});
 });
});
 ...
 /*$('#filter_executive_insuff').on('change', function(){
    ...
    $.ajax({
            type:'GET',
            url:'<?php echo ADMIN_SITE_URL.'address/address_closure_entries_vendor_insuff/' ?>'+filter_executive,
            data : '',
            dataType:'html',
