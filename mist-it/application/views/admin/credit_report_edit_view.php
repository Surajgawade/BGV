  ...
  <div class="row">
        <div class="col-sm-12">
          <div class="page-title-box">
            <h5 class="page-title">Edit Credit Report</h5>
              <ol class="breadcrumb">
                  <li class="breadcrumb-item"><a href="<?php echo ADMIN_SITE_URL ?>dashboard"><?php echo SHORT_CO_NAME ?></a></li>
                  <li class="breadcrumb-item"><a href="<?php echo ADMIN_SITE_URL ?>credit_report">Credit Report</a></li>
                  <li class="breadcrumb-item active">Credit Report Edit</li>
              </ol>
            
              <div class="state-information d-none d-sm-block">
                <ol class="breadcrumb">
                    <li><button class="btn btn-secondary waves-effect btn_clicked btn-sm" data-accessUrl="<?= ADMIN_SITE_URL?>credit_report"><i class="fa fa-arrow-left"></i> Back</button></li>  
               </ol>
              </div>
          </div>
        </div>
       ...
       <div class="card-body">
        <div class="nav-tabs-custom">
          <ul class="nav nav-pills nav-justified" role="tablist">
            <li class="nav-item waves-effect waves-light active"><a class = 'nav-link active' href="#tab_view_comp" data-toggle="tab">Credit Report Details</a></li>
          ...
          <div class="tab-content">
            <div class="active tab-pane" id="tab_view_comp">
              <?php $this->load->view('admin/credit_report_edit'); ?> 
            </div>
      ...
      <div class="card-body">    
        <div class="nav-tabs-custom">
          <ul class="nav nav-pills nav-justified">
              ...
              <li class="nav-item waves-effect waves-light"><a class = 'nav-link' href="#clk_insuff" id='clk_insuff_log' data-emp_id="<?php echo $selected_data['credit_report_id']; ?>" data-toggle="tab">Insufficiency</a></li>
            ...
            <div class="tab-content">
              ...
              if($reinitiated == "1")
              {
                 ...
                 $access_reverification = ($this->permission['access_credit_list_reverification'] == '1') ? 'ReInitiateModel'  : '';
              ?>
              ...
              ?>
                ...
                <table id="tbl_credit_report_result"  class="table table-striped table-bordered nowrap" cellspacing="0" width="100%"></table>
              </div>
...
<div id="addAddResultModel" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static"  style="overflow-y: scroll;">
  <div class="modal-dialog modal-lg" >
    ...
    <div class="modal-content">
      ...
      <input type="hidden" name="credit_report_id" value="<?php echo set_value('identity_id',$selected_data['credit_report_id']); ?>">
      <input type="hidden" name="credit_report_result_id" value="<?php echo set_value('credit_report_result_id',$selected_data['credit_report_result_id']); ?>">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <div class="row">
            <input type="hidden" name="component_type" id="component_type" value="10">
            <input type="hidden" name="acti_candsid" value="<?php echo set_value('acti_candsid',$get_cands_details['candsid']); ?>">
            <input type="hidden" name="comp_table_id" value="<?php echo set_value('comp_table_id',$selected_data['credit_report_id']); ?>">
            <input type="hidden" name="ac_ClientRefNumber" value="<?php echo set_value('ac_ClientRefNumber',$get_cands_details['ClientRefNumber']); ?>">
            <input type="hidden" name="component_ref_no" id="component_ref_no" value="<?php echo set_value('component_ref_no',$selected_data['credit_report_com_ref']); ?>">
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input type="hidden" name="clear_update_id" id="clear_update_id" value="<?php echo set_value('clear_update_id',$selected_data['credit_report_id']); ?>">
        ...
        <div class="col-md-12 col-sm-12 col-xs-12 form-group">
          ...
          <input type="file" name="clear_attchments[]" multiple id="clear_attchments" accept=".png, .jpg, .jpeg, .pdf" class="form-control"><?php echo set_value('clear_attchments'); ?>
...
<div id="insuffRaiseModel" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" name="update_id" id="update_id" value="<?php echo set_value('update_id',$selected_data['credit_report_id']); ?>">
...
<div id="showvendorModel" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      ...
      <input type="hidden" name="credit_report_id" value="<?php echo set_value('credit_report_id',$selected_data['credit_report_id']); ?>">
...
$(document).on('change', '.setinsff_reason', function(){
  ...
  if(insff_reason != 0)
  {
    $.ajax({
      ...
      dataType:'json',
      url:'<?php echo ADMIN_SITE_URL.'credit_report/insuff_raise_remark/'; ?>',
...
$(document).on('click','.InsuffRaiseModel',function() {
  ...
  var access_insuff = <?=($this->permission['access_credit_list_insuff_raise']) ? 1 : 2 ; ?>;
...
$(document).on('click', '#clk_insuff_log', function(){
  ...
  if(emp_id != 0)
  {
      $.ajax({
          type:'GET',
          url:'<?php echo ADMIN_SITE_URL.'credit_report/insuff_tab_view/'; ?>'+emp_id,
          beforeSend :function(){
...
$(document).on('click','.clkInsuffRaiseModel',function() {
  ...
  if(insuff_data != "")
  {
    ...
    $.ajax({
        type:'POST',
        url:'<?php echo ADMIN_SITE_URL.'credit_report/insuff_edit_clear_raised_data/'; ?>',
...
$(document).on('click','.insuffClearModel',function() {
  ...
  if(insuff_data != "")
  {
    $.ajax({
        type:'POST',
        url:'<?php echo ADMIN_SITE_URL.'credit_report/insuff_raised_data/'; ?>',
        data : 'insuff_data='+insuff_data,
        dataType:'json',
        beforeSend :function(){
...
$(document).on('click','.insuffDelete',function() {
  ...
  if(insuff_data != "")
  {
    $.ajax({
        type:'POST',
        url:'<?php echo ADMIN_SITE_URL.'credit_report/insuff_delete'; ?>',
...
$(document).on('click','.ReInitiateModel',function() {
    ...
    {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'credit_report/credit_report_reinitiated_date'; ?>',
 ...
 $('#add_vendor_details_view').validate({
      ...
      { 
        ...
        $.ajax({
          url : "<?php echo ADMIN_SITE_URL.'credit_report/Save_vendor_details'; ?>",
          data :  activityData,
          type: 'post',
          async:false,
          cache: false,
  ...
  $('#add_vendor_details_view_cancel').validate({
      ...
      //  var activityData   =    new FormData(form);
        ...
        $.ajax({
          url : "<?php echo ADMIN_SITE_URL.'credit_report/Save_vendor_details_cancel'; ?>",
          data :   new FormData(form),
          type: 'post',
          contentType:false,
          cache: false,
...
$(document).ready(function(){
  ...
  $('#frm_edit_insuff_raise').validate({ 
    ...
    {      
      $.ajax({
        url : '<?php echo ADMIN_SITE_URL.'credit_report/update_insuff_details'; ?>',
        data : new FormData(form),
        type: 'post',
        contentType:false,
        cache: false,
  ...
  $('#frm_insuff_raise').validate({ 
    ...
    {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'credit_report/insuff_raised'; ?>',
          data : new FormData(form),
          type: 'post',
          contentType:false,
          cache: false,
  ...
  $('#frm_insuff_clear').validate({ 
    ...
    {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'credit_report/insuff_clear'; ?>',
          data : new FormData(form),
          type: 'post',
          contentType:false,
          cache: false,
  ...
  $('#view_insuff_log_tabs').click(function(){
    $.ajax({
      type : 'POST',
      url : '<?php echo ADMIN_SITE_URL.'credit_report/address_logs/'.$selected_data['credit_report_id']; ?>',
      data: '',
      beforeSend :function(){
  ...
  $('#view_result_log_tabs').click(function(){
    $.ajax({
      type : 'POST',
      url : '<?php echo ADMIN_SITE_URL.'credit_report/credit_report_result_list/'.$selected_data['credit_report_id']; ?>',
      data: '',
      beforeSend :function(){
        jQuery('#tbl_credit_report_result').html("Loading..");
      },
      success:function(jdata)
      {
        var message = jdata.message;
        if(jdata.status = 200)
        {
          $('#tbl_credit_report_result').html(message);
        }
        else {
          $('#tbl_credit_report_result').html(message);
        }
        
        var tbl_credit_report_result =  $('#tbl_credit_report_result').DataTable( {scrollX:true, "ordering": true,searching: false,bFilter: false,bLengthChange : false,bSortable: true,bRetrieve: true,"iDisplayLength": 25 });

        tbl_credit_report_result.columns.adjust().draw();
      }
    }); 
  });
   ...
   $('#view_tab_vendor_log').click(function(){
    $.ajax({
      type : 'POST',
      url : '<?php echo ADMIN_SITE_URL.'credit_report/vendor_logs/'.$selected_data['credit_report_id']; ?>',
      data: '',
      beforeSend :function(){
  ...
  $('#view_activity_log_tabs').click(function(){
      $.ajax({
        type : 'POST',
        url : '<?php echo ADMIN_SITE_URL.'activity_log/activity_log_databale/'.$selected_data['credit_report_id'].'/10'; ?>',
        data: '',
          beforeSend :function(){
  ...
  $('#add_verificarion_result').validate({
    ...
    submitHandler: function(form) 
    { 
      var activityData = $('#add_verificarion_result').serialize()+'&'+$('#cases_activity').serialize()+"&activity_status_val=" + $("#activity_status option:selected").text()+ "&activity_mode_val=" + $("#activity_mode option:selected").text()+ "&activity_type_val=" + $("#activity_type option:selected").text()+ "&action_val=" + $("#action option:selected").text()+'&component_type=10'+'&auto_tag=3'+'&remarks='+$('.add_res_remarks').val();
      $.ajax({
      ...
      $.ajax({
        url: '<?php echo ADMIN_SITE_URL.'credit_report/add_verificarion_result'; ?>',
        data:  activityData,
        type: 'post',
        contentType:false,
        cache: false,
   ...
   $('#add_verificarion_result_view').validate({
    ...
    { 
      ...
      $.ajax({
        url: '<?php echo ADMIN_SITE_URL.'credit_report/add_verificarion_ver_result'; ?>',
        data : new FormData(form),
        type: 'post',
        contentType:false,
        cache: false,
