    ...
    <div class="col-sm-12">
      <div class="page-title-box">
        <div class="text-white div-header">
         Global Verification Case Details
        </div><br>
          <div  style="float: right;">
            <ol class="breadcrumb">
              <li> <button class="btn btn-secondary waves-effect btn-sm  edit_btn_click" data-frm_name='frm_global_update' data-editUrl="<?= ($this->permission['access_global_list_edit']) ? encrypt($selected_data['glodbver_id']) : ''?>"><i class="fa fa-edit"></i> Edit</button></li>
             
              <li><button class="btn btn-secondary waves-effect btn-sm" data-accessUrl="<?= ADMIN_SITE_URL.'global_verificatiion/delete/'.encrypt($selected_data['glodbver_id']); ?>"><i class="fa fa-trash"></i> Delete</button></li>
            </ol>
          </div>
      </div>
    </div>
...
<div class="box-body">
  <?php echo form_open_multipart('#', array('name'=>'frm_global_update','id'=>'frm_global_update')); ?>
   <div class="row">
      <div class="col-sm-4 form-group">
        <label>Candidate Name</label>
        <input type="hidden" name="glodbver_id" id="glodbver_id" value="<?php echo set_value('glodbver_id',$selected_data['glodbver_id']); ?>" >
      ...
      <div class="col-sm-4 form-group">
        ...
        <input type="text" name="global_com_ref" id="global_com_ref" readonly value="<?php echo set_value('global_com_ref',$selected_data['global_com_ref']); ?>" class="form-control">
        <?php echo form_error('global_com_ref'); ?>
      </div>
      <div class="col-sm-4 form-group">
    ...
    <div class="row">
      <div class="col-sm-4 form-group">
        <label>Mode of Verification</label>
         <input type="text" name="mode_of_veri"  readonly="readonly" id="mode_of_veri" value="<?php if(isset($mode_of_verification_value->globdbver)) { echo $mode_of_verification_value->globdbver; } ?>" class="form-control cls_disabled">
 ...
 <?php   
    ...
    <div class="row">
      <div class="col-sm-4 form-group">
        ...
        <input type="text" name="re_iniated_date" id="re_iniated_date" value="<?php echo set_value('re_iniated_date',$selected_data['glodbver_re_open_date']); ?>" class="form-control cls_disabled myDatepicker" placeholder='DD-MM-YYYY'>
      ...
      <div class="col-sm-4 form-group">
        ...
        <?php
         echo form_dropdown('address_type', ADDRESS_TYPE, set_value('address_type',$selected_data['address_type']), 'class="custom-select cls_disabled" id="address_type"');
    ...
    foreach ($attachments as $key => $value) {
      //$url  = SITE_URL.UPLOAD_FOLDER.CANDIDATES.$candidate_details['clientid'].'/';
         $url  = SITE_URL.GLOBAL_DB.$selected_data['clientid'].'/';
      if($value['type'] == 0)
      {
  ...
  <div class="col-md-12 col-sm-12 col-xs-12 form-group">
    <div style="float: right;">
      ...
      ="<?= ADMIN_SITE_URL.'activity_log/activity_log_view/7/'.$selected_data['glodbver_id'] ?>/global_database" <?=$check_insuff_raise?> data-toggle="modal" id="activityModelClk">Activity</button>
    </div>
  </div>
</div>
<script>
...
$(document).ready(function(){
  ...
  $('#frm_global_update').validate({ 
    ...
    {
      $.ajax({
        url : '<?php echo ADMIN_SITE_URL.'global_database/update_from'; ?>',
...
$('.remove_file').on('click',function(){
  var id =  <?= ($this->permission['access_global_list_edit']) ? $selected_data['glodbver_id'] : '"permission denied"'?>;
  if(id != "permission denied")
  {
      ...
      if (confirmr == true) 
      {
          ...
          $.ajax({
              url: "<?php  echo ADMIN_SITE_URL.'global_database/remove_uploaded_file/' ?>"+remove_id,
              type: 'GET',
              beforeSend:function(){
