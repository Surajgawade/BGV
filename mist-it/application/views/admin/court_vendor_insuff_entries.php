                    ...
                    <table id="tbl_vendor_insuff" class="table table-bordered  nowrap"  style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                      ...
                      <?php
                        ...
                        foreach ($vendor_executive_list as $key => $value) {
                           ...
                           $html_view .= "<tr id = ".$value['court_id']." url = ".ADMIN_SITE_URL."court_verificatiion/check_insuff_already_raised/".$value['court_id']." >";
                            $html_view .= "<td>".$i."</td>";
...
/*$('#tbl_vendor_insuff').on('dblclick', 'tr', function () { 
    ...
    $('.court_insuff_id').val(id);
    var url = $(this).attr('url');
    $('#append_vendor_model_insuff_deatils').load(url,function(){
      $('#showvendorModelIsuff').modal('show');
      $('#showvendorModelIsuff').addClass("show");
      $('#showvendorModelIsuff').css({background: "#0000004d"});
    });
  });*/
...
/*$('#filter_executive_insuff').on('change', function(){
    ...
    $.ajax({
            type:'GET',
            url:'<?php echo ADMIN_SITE_URL.'court_verificatiion/court_closure_entries_vendor_insuff/' ?>'+filter_executive,
            data : '',
            dataType:'html',
