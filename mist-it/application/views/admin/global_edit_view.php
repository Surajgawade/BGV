  ...
  <div class="row">
        <div class="col-sm-12">
          <div class="page-title-box">
            <h5 class="page-title">Edit GLobal DB</h5>
              <ol class="breadcrumb">
                  <li class="breadcrumb-item"><a href="<?php echo ADMIN_SITE_URL ?>dashboard"><?php echo SHORT_CO_NAME ?></a></li>
                  <li class="breadcrumb-item"><a href="<?php echo ADMIN_SITE_URL ?>global_database">GLobal DB</a></li>
                  <li class="breadcrumb-item active">GLobal DB Edit</li>
              </ol>
            
              <div class="state-information d-none d-sm-block">
                <ol class="breadcrumb">
                    <li><button class="btn btn-secondary waves-effect btn_clicked btn-sm" data-accessUrl="<?= ADMIN_SITE_URL?>global_database"><i class="fa fa-arrow-left"></i> Back</button></li>  
               </ol>
              </div>
          </div>
        </div>
       ...
       <div class="card-body">
        <div class="nav-tabs-custom">
          <ul class="nav nav-pills nav-justified" role="tablist">
            <li class="nav-item waves-effect waves-light active"><a class = 'nav-link active' href="#address_tab" data-toggle="tab">Global Details</a></li>
          ...
          <div class="tab-content">
            <div class="active tab-pane" id="address_tab">
              <?php $this->load->view('admin/global_edit'); ?> 
            </div>
      ...
      <div class="card-body">    
        <div class="nav-tabs-custom">
          <ul class="nav nav-pills nav-justified">
              ...
              <li class="nav-item waves-effect waves-light"><a class = 'nav-link' href="#clk_insuff" id='clk_insuff_log' data-emp_id="<?php echo $selected_data['glodbver_id']; ?>" data-toggle="tab">Insufficiency</a></li>
             <!-- <li><a href="#address_log_tabs" id="view_insuff_log_tabs" data-toggle="tab">Global DB Log</a></li>-->
            ...
            <div class="tab-content">
              ...
              <div class="tab-pane" id="clk_insuff">
                <div style="float: right;"><button class="btn btn-info btn-sm InsuffRaiseModel" data-editUrl='<?=($this->permission['access_global_list_insuff_raise']) ? $insuff_raise_id : ''; ?>' <?php echo $check_insuff_raise;?>>Raise Insuff</button><br>
                </div>
              ...
              <?php
               if($reinitiated == "1")
               {
                 ...
                 $access_reverification = ($this->permission['access_global_list_reverification'] == '1') ? 'ReInitiateModel'  : '';
              ?>
              ...
              <div class="clearfix"></div>
                <table id="tbl_global_result" class="table table-striped table-bordered nowrap" cellspacing="0" width="100%"></table>
              </div>
...
<div id="showAddResultModel" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog modal-lg" >
    ...
    <div class="modal-content">
      ...
      <input type="hidden" name="glodbver_id" value="<?php echo set_value('glodbver_id',$selected_data['glodbver_id']); ?>">
      ...
      <input type="hidden" name="glodbver_result_id" value="<?php echo set_value('glodbver_result_id',$selected_data['glodbver_result_id']); ?>">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" name="update_id" id="update_id" value="<?php echo set_value('update_id',$selected_data['glodbver_id']); ?>">
...
<div id="addAddResultModel" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static" style="overflow-y: scroll;"> 
  <div class="modal-dialog modal-lg">
    ...
    <?php echo form_open_multipart("#", array('name'=>'global_verificarion_result','id'=>'global_verificarion_result')); ?>
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
...
<div id="showvendorModel" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      ...
      <input type="hidden" name="global_id" value="<?php echo set_value('global_id',$selected_data['glodbver_id']); ?>">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <div class="row">
            <input type="hidden" name="component_type" id="component_type" value="6">
            <input type="hidden" name="acti_candsid" value="<?php echo set_value('acti_candsid',$get_cands_details['candsid']); ?>">
            <input type="hidden" name="comp_table_id" value="<?php echo set_value('comp_table_id',$selected_data['glodbver_id']); ?>">
            <input type="hidden" name="ac_ClientRefNumber" value="<?php echo set_value('ac_ClientRefNumber',$get_cands_details['ClientRefNumber']); ?>">

            <input type="hidden" name="component_ref_no" id="component_ref_no" value="<?php echo set_value('component_ref_no',$selected_data['global_com_ref']); ?>">
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input type="hidden" name="clear_update_id" id="clear_update_id" value="<?php echo set_value('clear_update_id',$selected_data['glodbver_id']); ?>">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="col-md-6 col-sm-12 col-xs-6 form-group">
          ...
          <input type="text" name="txt_insuff_raise"  id="txt_insuff_raise" value="<?php echo set_value('txt_insuff_raise',$this->insuff_date); ?>" class="form-control myDatepicker" placeholder='DD-MM-YYYY'>
...
$(document).on('click','.clkInsuffRaiseModel',function() {
  ...
  if(insuff_data != "")
  {
    ...
    $.ajax({
        type:'POST',
        url:'<?php echo ADMIN_SITE_URL.'global_database/insuff_edit_clear_raised_data/'; ?>',
...
$(document).on('click','.InsuffRaiseModel',function() {
  ...
  var access_insuff = <?=($this->permission['access_global_list_insuff_raise']) ? 1 : 2 ; ?>;
...
$(document).on('click', '#clk_insuff_log', function(){
  ...
  if(emp_id != 0)
  {
      $.ajax({
          type:'GET',
          url:'<?php echo ADMIN_SITE_URL.'global_database/insuff_tab_view/'; ?>'+emp_id,
          beforeSend :function(){
...
$(document).on('click','.insuffClearModel',function() {
  ...
  if(insuff_data != "")
  {
    $.ajax({
        type:'POST',
        url:'<?php echo ADMIN_SITE_URL.'global_database/insuff_raised_data/'; ?>',
        data : 'insuff_data='+insuff_data,
        dataType:'json',
        beforeSend :function(){
...
$(document).on('click','.insuffDelete',function() {
  ...
  if(insuff_data != "")
  {
    $.ajax({
        type:'POST',
        url:'<?php echo ADMIN_SITE_URL.'global_database/insuff_delete'; ?>',
...
$(document).on('click','#clkAddResultModel',function() {
 ...
 $(document).on('click','.showvendorModel',function() {
   ...
   $(document).on('click','.showvendorModel_cancel',function() {
    var permissionid = <?php echo ($this->permission['access_global_aq_allow'] == '1') ? '1'  : '2' ?>;
    if(permissionid == '1'){
  ...
  $('#ReInitiateModel').css({background: "#0000004d"});
    ...
    {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'global_database/global_detabase_reinitiated_date'; ?>',
...
$('#add_vendor_details_view').validate({
    ...
    rules: {
      ...
      { 
        ...
        //var activityData = $('#add_vendor_detaills_view').serialize();
        var activityData =  new FormData($('#add_vendor_details_view')[0]);
   
        $.ajax({
          url : "<?php echo ADMIN_SITE_URL.'global_database/Save_vendor_details'; ?>",
          data :  activityData,
          type: 'post',
          async:false,
          cache: false,
  ...
  $('#add_vendor_details_view_cancel').validate({
      ...
      //  var activityData   =    new FormData(form);
        ...
        $.ajax({
          url : "<?php echo ADMIN_SITE_URL.'global_database/Save_vendor_details_cancel'; ?>",
          data :   new FormData(form),
          type: 'post',
          contentType:false,
          cache: false,
...
$(document).ready(function(){
  ...
  $('#frm_edit_insuff_raise').validate({ 
    ...
    {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'global_database/update_insuff_details'; ?>',
          data : new FormData(form),
          type: 'post',
          contentType:false,
          cache: false,
  ...
  $('#frm_insuff_raise').validate({ 
    ...
    {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'global_database/insuff_raised'; ?>',
          data : new FormData(form),
          type: 'post',
          contentType:false,
          cache: false,
  ...
  $('#frm_insuff_clear').validate({ 
    ...
    {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'global_database/insuff_clear'; ?>',
          data : new FormData(form),
          type: 'post',
          contentType:false,
          cache: false,
  ...
  $('#view_insuff_log_tabs').click(function(){
    $.ajax({
      type : 'POST',
      url : '<?php echo ADMIN_SITE_URL.'global_database/address_logs/'.$selected_data['glodbver_id']; ?>',
      data: '',
      beforeSend :function(){
  ...
  $('#view_tab_vendor_log').click(function(){
    $.ajax({
      type : 'POST',
      url : '<?php echo ADMIN_SITE_URL.'global_database/vendor_logs/'.$selected_data['glodbver_id']; ?>',
      data: '',
      beforeSend :function(){
  ...
  $('#view_result_log_tabs').click(function(){
    $.ajax({
      type : 'POST',
      url : '<?php echo ADMIN_SITE_URL.'global_database/globrver_result_list/'.$selected_data['glodbver_result_id']; ?>',
      data: '',
      beforeSend :function(){
        jQuery('#tbl_global_result').html("Loading..");
      },
      success:function(jdata)
      {
       var message = jdata.message;
          if(jdata.status = 200)
        {
          $('#tbl_global_result').html(message);
        }
        else {
          $('#tbl_global_result').html(message);
        }
        
          var tbl_global_result =  $('#tbl_global_result').DataTable( {scrollX:true, "ordering": true,searching: false,bFilter: false,bLengthChange : false,bSortable: true,bRetrieve: true,"iDisplayLength": 25 });

          tbl_global_result.columns.adjust().draw();
      }
    }); 
  });
    ...
    $('#view_activity_log_tabs').click(function(){
      $.ajax({
        type : 'POST',
        url : '<?php echo ADMIN_SITE_URL.'activity_log/activity_log_databale/'.$selected_data['glodbver_id'].'/6'; ?>',
        data: '',
          beforeSend :function(){
  ...
  $('#global_verificarion_result').validate({
      rules: {
      ...
      submitHandler: function(form) 
      { 
        var activityData = $('#global_verificarion_result').serialize()+'&'+$('#cases_activity').serialize()+"&activity_status_val=" + $("#activity_status option:selected").text()+ "&activity_mode_val=" + $("#activity_mode option:selected").text()+ "&activity_type_val=" + $("#activity_type option:selected").text()+ "&action_val=" + $("#action option:selected").text()+'&component_type=6'+'&auto_tag=3'+'&remarks='+$('.add_res_remarks').val();
        $.ajax({
        ...
        $.ajax({
          url: '<?php echo ADMIN_SITE_URL.'global_database/add_verificarion_result'; ?>',
          data:  activityData,
          type: 'post',
          contentType:false,
          cache: false,
   ...
   $('#add_verificarion_result_view').validate({
      ...
      { 
        ...
        $.ajax({
          url: '<?php echo ADMIN_SITE_URL.'global_database/add_verificarion_ver_result'; ?>',
          data : new FormData(form),
          type: 'post',
          contentType:false,
          cache: false,
