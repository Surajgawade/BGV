...
<div id="showvendorModel" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      ...
      ="<?= ADMIN_SITE_URL.'activity_log/activity_log_view_vendor_form/1/'.$addressver_details['id'] ?>/address" <?=$check_insuff_raise?> data-toggle="modal" id="activityModelClk1" type="button" >Convert To Digital</button>
...
<div id="CasesAssignvendor" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      ...
      <div class="modal-body">
        <input type="hidden" name="digital_address_id" id = "digital_address_id">
        <div class="row">
          <div class="col-md-6 col-sm-12 col-xs-12 form-group vendor_list">
            <?php echo form_dropdown('vendor_list_digital', $vendor_list, set_value('vendor_list_digital'), 'class="form-control" id="vendor_list_digital" required="required" ');?>
          ...
          <?php $vendor_list_mode = array('physical visit'=> 'Physical Visit');
             echo form_dropdown('vendor_list_mode_digital', $vendor_list_mode, set_value('vendor_list_mode_digital'), 'class="form-control" id="vendor_list_mode_digital" required="required" ');?>

          </div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-success" id="btn_assign_action_vendor_digital">Assign</button>
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
...
$(document).on('click','.CasesAssignvendor',function() {

    var id = $(this).data('id');
    $('#digital_address_id').val(id);
      
    $('#CasesAssignvendor').modal('show');
    $('#CasesAssignvendor').addClass("show");
    $('#CasesAssignvendor').css({background: "#0000004d"}); 
...
$('#view_activity_log_tabs').click(function(){
  ...
  $('#btn_assign_action_vendor_digital').on('click', function(e){
    var vendor_list = $('#vendor_list_digital').val();
    var select_one = $('#digital_address_id').val();
  
    if(vendor_list != "0" && select_one != "")
    {
        var vendor_list_mode = $('#vendor_list_mode_digital').val();
        $.ajax({
          type:'POST',
          url:'<?php echo ADMIN_SITE_URL.'address/assign_to_vendor_digital/' ?>',
          data : 'vendor_list='+vendor_list+'&cases_id='+select_one+'&vendor_list_mode='+vendor_list_mode,
          dataType:'json',
          beforeSend :function(){
            jQuery('#btn_assign_action_vendor_digital').text('Processing...');
          },
          complete:function(){
            jQuery('#btn_assign_action_vendor_digital').text('');
