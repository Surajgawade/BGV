      ...
      <div class="page-title-box">
        ...
        <br>
          <div style="float: right;">
            <ol class="breadcrumb">
              <li><button class="btn btn-secondary waves-effect btn-sm  edit_btn_click" data-frm_name='frm_social_media_update' data-editUrl="<?= ($this->permission['access_social_media_list_edit']) ? encrypt($selected_data['social_media_id']) : ''?>"><i class="fa fa-edit"></i> Edit</button></li>
             
              <li><button class="btn btn-secondary waves-effect btn-sm" data-accessUrl="<?= ADMIN_SITE_URL.'social_media/delete/'.encrypt($selected_data['social_media_id']); ?>"><i class="fa fa-trash"></i> Delete</button></li>
            </ol>
          </div>
      </div>
    </div>
...
<div class="box-body">
  <?php echo form_open_multipart('#', array('name'=>'frm_social_media_update','id'=>'frm_social_media_update')); ?>
    <div class="row">
      ...
      <div class="col-sm-4 form-group">
        ...
        <input type="text" name="social_media_com_ref" id="social_media_com_ref"  value="<?php echo set_value('social_media_com_ref',$selected_data['social_media_com_ref']); ?>" class="form-control readonly">
        <?php echo form_error('social_media_com_ref'); ?>
      </div>
      <div class="col-sm-4 form-group">
    ...
    ?>
      ...
      <div class="col-sm-4 form-group">
        <label>Mode of Verification</label>
         <input type="text" name="mode_of_veri" readonly="readonly" id="mode_of_veri" value="<?php if(isset($mode_of_verification_value->social_media)) { echo $mode_of_verification_value->social_media; } ?>" class="form-control cls_disabled">
     ...
     <div class="row">
       ...
       <input type="text" name="re_iniated_date" id="re_iniated_date" value="<?php echo set_value('re_iniated_date',$selected_data['social_media_re_open_date']); ?>" class="form-control cls_disabled myDatepicker" placeholder='DD-MM-YYYY'>
      </div>
    ...
    foreach ($attachments as $key => $value) {
      //$url  = SITE_URL.UPLOAD_FOLDER.CANDIDATES.$candidate_details['clientid'].'/';
         $url  = SITE_URL.SOCIAL_MEDIA.$selected_data['clientid'].'/';
      if($value['type'] == 0)
      {
    ...
    ?>
      ...
      <div class="col-sm-8 form-group">
        <input type="submit" name="btn_social_media_edit" id='btn_social_media_edit' value="Update" class="btn btn-success cls_disabled">
  ...
  <div class="col-sm-12 form-group">
    <div style="float: right;">
      ...
      ="<?= ADMIN_SITE_URL.'activity_log/activity_log_view/14/'.$selected_data['social_media_id'] ?>/social_media" <?=$check_insuff_raise?> data-toggle="modal" id="activityModelClk">Activity</button>
    </div>
  </div>
</div>
...
$(document).ready(function(){
  ...
  $('#frm_social_media_update').validate({ 
    rules: {
      social_media_id : {
        required : true
      },
    ...
    {
      $.ajax({
        url : '<?php echo ADMIN_SITE_URL.'social_media/update_form'; ?>',
        ...
        dataType:'json',
        beforeSend:function(){
          $('#btn_social_media_edit').attr('disabled','disabled');
 ...
 $('.remove_file').on('click',function(){
  var id =  <?= ($this->permission['access_social_media_list_edit']) ? $selected_data['social_media_id'] : '"permission denied"'?>;
  if(id != "permission denied")
  {
      ...
      if (confirmr == true) 
      {
          ...
          $.ajax({
              url: "<?php  echo ADMIN_SITE_URL.'social_media/remove_uploaded_file/' ?>"+remove_id,
              type: 'GET',
              beforeSend:function(){
