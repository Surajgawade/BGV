    ...
    <div class="col-sm-12">
      <div class="page-title-box">
        <div class="text-white div-header">
         PCC Verification Case Details
        </div>
        <br>
          <div  style="float: right;">
            <ol class="breadcrumb">
              <li><button class="btn btn-secondary waves-effect btn-sm  edit_btn_click" data-frm_name='frm_pcc_update' data-editUrl="<?= ($this->permission['access_pcc_list_edit']) ? encrypt($selected_data['pcc_id']) : ''?>"><i class="fa fa-edit"></i> Edit</button></li>
            
              <li><button class="btn btn-secondary waves-effect btn-sm" data-accessUrl="<?= ADMIN_SITE_URL.'pcc/delete/'.encrypt($selected_data['pcc_id']); ?>"><i class="fa fa-trash"></i> Delete</button> </li>
            </ol>
          </div>
      </div>
    </div>
...
<div class="box-body">
  <?php echo form_open_multipart('#', array('name'=>'frm_pcc_update','id'=>'frm_pcc_update')); ?>
     <div class="row">
      <div class="col-sm-4 form-group">
        <label>Candidate Name</label>
        <input type="hidden" name="pcc_id" id="pcc_id" value="<?php echo set_value('pcc_id',$selected_data['pcc_id']); ?>">
      ...
      <div class="col-sm-4 form-group">
        ...
        <input type="text" name="pcc_com_ref" id="pcc_com_ref" value="<?php echo set_value('pcc_com_ref',$selected_data['pcc_com_ref']); ?>" class="form-control readonly">
    ...
    ?>
     ...
     <div class="row"> 
      <div class="col-sm-4 form-group">
        <label>Mode of Verification</label>
         <input type="text" name="mode_of_veri" readonly="readonlys"  id="mode_of_veri" value="<?php if(isset($mode_of_verification_value->crimver)) { echo $mode_of_verification_value->crimver; } ?>" class="form-control cls_disabled">
     ...
     <div class="col-sm-4 form-group">
      ...
      <div class="row">
       <div class="col-sm-4 form-group">
        <label>Re-Initiated date</label>
        <input type="text" name="re_iniated_date" id="re_iniated_date" value="<?php echo set_value('re_iniated_date',$selected_data['pcc_re_open_date']); ?>" class="form-control cls_disabled myDatepicker" placeholder='DD-MM-YYYY'>
      </div>
     ...
     <div class="row">
      ...
      <?php 
      $total = json_decode($selected_data['references']);

      $references_name = json_decode($selected_data['references']);
      $references_no = json_decode($selected_data['references_no']);
      if(count($total) > 0)
      {
        for ($i=0; $i < count($total); $i++) { 
            ...
            <div class="row">
              <div class="col-sm-4 form-group">
                ...
                <input type="text" name="references[]" id="references" value="<?php echo set_value('references', $references_name[$i]); ?>" class="form-control cls_disabled">
                <?php echo form_error('references'); ?>
              </div>
              <div class="col-sm-4 form-group">
                <label>Contact Number </label>
                <input type="text" name="references_no[]" maxlength="14" id="references_no" value="<?php echo set_value('references_no',$references_no[$i]); ?>" class="form-control cls_disabled">
      ...
      <div class="copyCode">
        ...
        <div class="col-sm-4 form-group">
          <label>Reference </label>
          <input type="text" name="references[]" id="references" value="<?php echo set_value('references'); ?>" class="form-control cls_disabled">
          <?php echo form_error('references'); ?>
        </div>
        <div class="col-sm-4 form-group">
          <label>Contact Number </label>
          <input type="text" name="references_no[]" maxlength="14" id="references_no" value="<?php echo set_value('references_no'); ?>" class="form-control cls_disabled">
          <?php echo form_error('references_no'); ?>
        </div>
       </div>
      </div>
    ...
    foreach ($attachments as $key => $value) {
      //$url  = SITE_URL.UPLOAD_FOLDER.CANDIDATES.$candidate_details['clientid'].'/';
         $url  = SITE_URL.PCC.$selected_data['clientid'].'/';
      if($value['type'] == 0)
      {
    ...
    ?>
      ...
      <div class="col-md-8 col-sm-12 col-xs-8 form-group">
        <input type="submit" name="btn_pcc_update" id='btn_pcc_update' value="Update" class="btn btn-success cls_disabled">
      </div>
  ...
  <div class="col-md-12 col-sm-12 col-xs-12 form-group">
    <div style="float: right;">
      ...
      ="<?= ADMIN_SITE_URL.'activity_log/activity_log_view/4/'.$selected_data['pcc_id'] ?>/pcc" <?=$check_insuff_raise?> data-toggle="modal" id="activityModelClk">Activity</button>
    </div>
  </div>
</div>
<script>
...
$('input').attr('autocomplete','off');
  ...
  $('#frm_pcc_update').validate({ 
    ...
    {
      $.ajax({
        url : '<?php echo ADMIN_SITE_URL.'pcc/update_form'; ?>',
        ...
        dataType:'json',
        beforeSend:function(){
          $('#btn_pcc_update').attr('disabled','disabled');
          jQuery('.body_loading').show();
        },
        complete:function(){
          //$('#btn_pcc_update').attr('disabled',false);
...
$('.remove_file').on('click',function(){
  var id =  <?= ($this->permission['access_pcc_list_edit']) ? $selected_data['pcc_id'] : '"permission denied"'?>;
  ...
  { 
      ...
      if (confirmr == true) 
      {
          ...
          $.ajax({
              url: "<?php  echo ADMIN_SITE_URL.'pcc/remove_uploaded_file/' ?>"+remove_id,
              type: 'GET',
              beforeSend:function(){
