  ...
  <div class="row">
        <div class="col-sm-12">
          <div class="page-title-box">
            <h5 class="page-title">Edit Social Media Verification</h5>
              <ol class="breadcrumb">
                  <li class="breadcrumb-item"><a href="<?php echo ADMIN_SITE_URL ?>dashboard"><?php echo SHORT_CO_NAME ?></a></li>
                  <li class="breadcrumb-item"><a href="<?php echo ADMIN_SITE_URL ?>social_media">Social Media Verification</a></li>
                  <li class="breadcrumb-item active">Social Media Verification Edit</li>
              </ol>
            
              <div class="state-information d-none d-sm-block">
                <ol class="breadcrumb">
                    <li><button class="btn btn-secondary waves-effect btn_clicked btn-sm" data-accessUrl="<?= ADMIN_SITE_URL?>social_media"><i class="fa fa-arrow-left"></i> Back</button></li>  
               </ol>
              </div>
          </div>
        </div>
       ...
       <div class="card-body">
        <div class="nav-tabs-custom">
          <ul class="nav nav-pills nav-justified" role="tablist">
            <li class="nav-item waves-effect waves-light active"><a class = 'nav-link active' href="#tab_view_comp" data-toggle="tab">Social Media Details</a></li>
          ...
          <div class="tab-content">
            <div class="active tab-pane" id="tab_view_comp">
              <?php $this->load->view('admin/social_media_edit'); ?> 
            </div>
      ...
      <div class="card-body">    
        <div class="nav-tabs-custom">
          <ul class="nav nav-pills nav-justified">
              ...
              <li class="nav-item waves-effect waves-light"><a class = 'nav-link' href="#clk_insuff" id='clk_insuff_log' data-emp_id="<?php echo $selected_data['social_media_id']; ?>" data-toggle="tab">Insufficiency</a></li>
            ...
            <div class="tab-content">
              ...
              <div class="tab-pane" id="clk_insuff">
                <div style="float: right;"><button class="btn btn-info btn-sm InsuffRaiseModel" data-editUrl='<?=($this->permission['access_social_media_list_insuff_raise']) ? $insuff_raise_id : ''; ?>' <?php echo $check_insuff_raise;?>>Raise Insuff</button><br>
                </div>
            ...
            <?php
              ...
              if($reinitiated == "1")
              {
                ...
                $access_reverification = ($this->permission['access_social_media_list_reverification'] == '1') ? 'ReInitiateModel'  : '';
              ?>
              ...
              <div class="clearfix"></div>
                <table id="tbl_social_media_result" class="table table-striped table-bordered nowrap" cellspacing="0" width="100%"></table>
              </div>
...
<div id="addAddResultModel" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static" style="overflow-y: scroll;">
  <div class="modal-dialog modal-lg" >
    ...
    <div class="modal-content">
      ...
      <input type="hidden" name="social_media_id" value="<?php echo set_value('social_media_id',$selected_data['social_media_id']); ?>">
      <input type="hidden" name="social_media_result_id" value="<?php echo set_value('social_media_result_id',$selected_data['social_media_result_id']); ?>">
      <input type="hidden" name="social_media_com_ref" value="<?php echo set_value('social_media_com_ref',$selected_data['social_media_com_ref']); ?>">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <div class="row">
            ...
            <input type="hidden" name="comp_table_id" value="<?php echo set_value('comp_table_id',$selected_data['social_media_id']); ?>">
            <input type="hidden" name="ac_ClientRefNumber" value="<?php echo set_value('ac_ClientRefNumber',$get_cands_details['ClientRefNumber']); ?>">
            <input type="hidden" name="component_ref_no" id="component_ref_no" value="<?php echo set_value('component_ref_no',$selected_data['social_media_com_ref']); ?>">
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input type="hidden" name="clear_update_id" id="clear_update_id" value="<?php echo set_value('clear_update_id',$selected_data['social_media_id']); ?>">
...
<div id="insuffRaiseModel" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" name="update_id" id="update_id" value="<?php echo set_value('update_id',$selected_data['social_media_id']); ?>">
  ...
  return theWin;
   ...
   $('#upload_copied_image_identity_result').on('click',function() {
      ...
      if(get_image_base_value) {
          ...
          $('.image-content-result-identity-result').empty();
   ...
   $('#upload_copied_image_identity_ver_result').on('click',function() {
      ...
      if(get_image_base_value) {
          ...
          $('.image-content-result-identity-ver-result').empty();
          show_alert('Image copied success', 'success');
      }
  });   
...
$(document).on('click','.InsuffRaiseModel',function() {
  ...
  var access_insuff = <?=($this->permission['access_social_media_list_insuff_raise']) ? 1 : 2 ; ?>;
...
$(document).on('click', '#clk_insuff_log', function(){
  ...
  if(emp_id != 0)
  {
      $.ajax({
          type:'GET',
          url:'<?php echo ADMIN_SITE_URL.'social_media/insuff_tab_view/'; ?>'+emp_id,
          beforeSend :function(){
...
$(document).on('click','.clkInsuffRaiseModel',function() {
  ...
  if(insuff_data != "")
  {
    ...
    $.ajax({
        type:'POST',
        url:'<?php echo ADMIN_SITE_URL.'social_media/insuff_edit_clear_raised_data/'; ?>',
...
$(document).on('click','.insuffClearModel',function() {
  ...
  if(insuff_data != "")
  {
    $.ajax({
        type:'POST',
        url:'<?php echo ADMIN_SITE_URL.'social_media/insuff_raised_data/'; ?>',
        data : 'insuff_data='+insuff_data,
        dataType:'json',
        beforeSend :function(){
...
$(document).on('click','.insuffDelete',function() {
  ...
  if(insuff_data != "")
  {
    $.ajax({
        type:'POST',
        url:'<?php echo ADMIN_SITE_URL.'social_media/insuff_delete'; ?>',
...
$(document).ready(function(){
  ...
  $('#frm_edit_insuff_raise').validate({ 
    ...
    {      
      $.ajax({
        url : '<?php echo ADMIN_SITE_URL.'social_media/update_insuff_details'; ?>',
...
$(document).on('click','.ReInitiateModel',function() {
  ...
  $('#frm_insuff_raise').validate({ 
    ...
    {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'social_media/insuff_raised'; ?>',
          data : new FormData(form),
          type: 'post',
          contentType:false,
          cache: false,
  ...
  $('#frm_insuff_clear').validate({ 
    ...
    {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'social_media/insuff_clear'; ?>',
          data : new FormData(form),
          type: 'post',
          contentType:false,
          cache: false,
  ...
  $('#view_insuff_log_tabs').click(function(){
    $.ajax({
      type : 'POST',
      url : '<?php echo ADMIN_SITE_URL.'pcc/address_logs/'.$selected_data['social_media_id']; ?>',
      data: '',
      beforeSend :function(){
  ...
  $('#view_result_log_tabs').click(function(){
    $.ajax({
      type : 'POST',
      url : '<?php echo ADMIN_SITE_URL.'social_media/social_media_result_list/'.$selected_data['social_media_id']; ?>',
      data: '',
      beforeSend :function(){
        jQuery('#tbl_social_media_result').html("Loading..");
      },
      success:function(jdata)
      {
        var message = jdata.message;
          if(jdata.status = 200)
        {
          $('#tbl_social_media_result').html(message);
        }
        else {
          $('#tbl_social_media_result').html(message);
        }
        
        var tbl_social_media_result =  $('#tbl_social_media_result').DataTable( {scrollX:true, "ordering": true,searching: false,bFilter: false,bLengthChange : false,bSortable: true,bRetrieve: true,"iDisplayLength": 25 });

        tbl_social_media_result.columns.adjust().draw();
  ...
  $('#view_activity_log_tabs').click(function(){
      $.ajax({
        type : 'POST',
        url : '<?php echo ADMIN_SITE_URL.'activity_log/activity_log_databale/'.$selected_data['social_media_id'].'/11'; ?>',
        data: '',
          beforeSend :function(){
 ...
 $(document).on('click','.showvendorModel_cost',function() {
  ...
  $('#add_verificarion_result').validate({
    ...
    submitHandler: function(form) 
    { 
      var activityData = $('#add_verificarion_result').serialize()+'&'+$('#cases_activity').serialize()+"&activity_status_val=" + $("#activity_status option:selected").text()+ "&activity_mode_val=" + $("#activity_mode option:selected").text()+ "&activity_type_val=" + $("#activity_type option:selected").text()+ "&action_val=" + $("#action option:selected").text()+'&component_type=11'+'&auto_tag=3'+'&remarks='+$('.add_res_remarks').val();
      $.ajax({
      ...
      $.ajax({
        url: '<?php echo ADMIN_SITE_URL.'social_media/add_verificarion_result'; ?>',
        data:  activityData,
        type: 'post',
        contentType:false,
        cache: false,
  ...
  $('#add_verificarion_result_view').validate({
    ...
    { 
      ...
      $.ajax({
        url: '<?php echo ADMIN_SITE_URL.'social_media/add_verificarion_ver_result'; ?>',
        data : new FormData(form),
        type: 'post',
        contentType:false,
        cache: false,
