...
<div class="container-fluid">
   ...
   <div class="nav-tabs-custom">
        <ul class="nav nav-pills nav-justified"> 
           ...
           <?php  
            echo "<li class='nav-item waves-effect waves-light res active'  role='presentation' data-tab_name='court_list_view' ><a class = 'nav-link active' href='#court_list_view' aria-controls='home' data-toggle='tab'>Court List View</a></li>";
            
            echo "<li  role='presentation' data-url='".ADMIN_SITE_URL."court_verificatiion/get_assign_court_list_view/'  data-tab_name='assign_court_list_view'  class='nav-item waves-effect waves-light view_assign_tab'><a class = 'nav-link' href='#assign_court_list_view' aria-controls='home' role='tab'  data-toggle='tab'>New Cases</a></li>";
...
<div class="tab-content">
  <div id="court_list_view" class="tab-pane active">       
    <div class="row">
         <div class="col-sm-12">
            <div class="page-title-box">
                <h4 class="page-title">Court - List View</h4>
                  <ol class="breadcrumb">
                  ...
                  <div class="state-information d-none d-sm-block">
                    <ol class="breadcrumb">
                       <?php
                          $access_import_candidate = ($this->permission['access_court_list_export']) ? '#myModelImportCandidate'  : '';
                          $access_export = ($this->permission['access_court_list_export']) ? '#myModalExport'  : '';
                          $access_import = ($this->permission['access_court_list_import']) ? '#myModelImport'  : '';
                          $access_update_case_received_date = ($this->user_info['import_permission'] == "1") ? '#myModalUpdateCaseReceiveDate'  : '';  
                       ?>
                         <li><button class="btn btn-secondary waves-effect" data-toggle="modal" data-target="<?php echo $access_import_candidate;?>"><i class="fa fa-download"></i> Import Candidate</button></li>
    ...
    <div id="assign_court_list_view" class="tab-pane fade in">
...
<div id="myModelImport" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Import Court Records</h4>
      </div>
...
<div id="myModelImportCandidate" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <?php echo form_open_multipart('#', array('name'=>'frm_download_tem_and_import_candidate','id'=>'frm_download_tem_and_import_candidate')); ?>   
      <div class="modal-body">
        <div class="row">
          <div class="col-md-8 col-sm-12 col-xs-8 form-group">
        ...
        <div class="row">
          ...
          <div class="col-md-8 col-sm-12 col-xs-8 form-group">
            <label>Upload File</label>
            <input type="file" name="candidate_bulk_sheet" id="candidate_bulk_sheet" class="form-control">
          </div>
        </div>
      </div>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-info btn-md" id="import_candidate_template_candidate">Import</button>
        <button type="button" class="btn btn-default btn-md" data-dismiss="modal" >Close</button>
      </div>
      <?php echo form_close(); ?>
    </div>
...
<div id="myModalExport" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
  <div class="modal-dialog" style="width: 700px !important;">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Export Court Verification Records</h4>
      </div>
...
<div id="myModalUpdateCaseReceiveDate" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Import Court Case Received Date</h4>
      </div>
...
$(document).ready(function() {
  ...
  var oTable =  $('#tbl_datatable').DataTable( {
        ...
        "ajax": $.fn.dataTable.pipeline( {
            url: "<?php echo ADMIN_SITE_URL.'court_verificatiion/court_view_datatable'; ?>",
            pages: 1,
            async: true,
            method: 'POST',
  ...
  $('#filter_by_executive,#filter_by_clientid,#filter_by_status,#filter_by_sub_status').on('change', function(){
    ...
    var new_url = "<?php echo ADMIN_SITE_URL.'court_verificatiion/court_view_datatable'; ?>?status="+status+"&sub_status="+sub_status+"&client_id="+client_id+"&filter_by_executive="+filter_by_executive;
  ...
  $('#filter_by_executive,#filter_by_status,#filter_by_sub_status').on('change', function(){
    ...
    var sub_status = $('#filter_by_sub_status').val();
     ...
     $.ajax({
          type:'POST',
          url:'<?php echo ADMIN_SITE_URL.'court_verificatiion/get_clients_for_court_list_view/' ?>',
    ...
    {
    var new_url = "<?php echo ADMIN_SITE_URL.'court_verificatiion/court_view_datatable'; ?>?status="+status+"&sub_status="+sub_status+"&client_id="+client_id+"&start_date="+start_date+"&end_date="+end_date+"&filter_by_executive="+filter_by_executive;
    oTable.ajax.url(new_url).load();
   }
  });
  ...
  $('#btn_reset').on('click', function() {
   ...
   $("#filter_by_status option[value = All]").attr('selected','selected');
    ...
    var new_url = "<?php echo ADMIN_SITE_URL.'court_verificatiion/court_view_datatable'; ?>?status="+status+"&filter_by_executive="+filter_by_executive;
    oTable.ajax.url(new_url).load();
  });
  ...
  $('#btn_assign_action').on('click', function(e){
    ...
    if(users_list != '0'  && select_one != "")
    {
        $.ajax({
          type:'POST',
          url:'<?php echo ADMIN_SITE_URL.'court_verificatiion/assign_to_executive/' ?>',
  ...
  $('#frm_download_tem_and_import').validate({ 
      ...
      {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'court_verificatiion/bulk_upload_court'; ?>',
  ...
  $('#frm_download_tem_and_import_candidate').validate({ 
      rules: {
        clientid : {
          required : true,
          greaterThan: 0
        },
        candidate_bulk_sheet :
        {
           required : true,
           extension: "xls|xlsx"
        },
      ...
      {      
            $.ajax({
              url : '<?php echo ADMIN_SITE_URL.'court_verificatiion/bulk_upload_candidates_court'; ?>',
              data : new FormData(form),
              type: 'post',
              contentType:false,
              cache: false,
              ...
              dataType:'json',
              beforeSend:function(){
                $('#import_candidate_template_candidate').attr('disabled','disabled');
              },
              complete:function(){
                $('#import_candidate_template_candidate').removeAttr('disabled');
              },
              success: function(jdata){
                var message =  jdata.message || '';
                if(jdata.status == '<?php echo SUCCESS_CODE; ?>'){
                  show_alert(message,'success');
                  $('#myModelImportCandidate').modal('hide');
                  $('#frm_download_tem_and_import_candidate')[0].reset();
                }
                if(jdata.status == <?php echo ERROR_CODE; ?>){
                  show_alert(message,'error'); 
                }
...
$('#frm_download_tem_and_import_update_case_received_date').validate({ 
      ...
      {      
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'court_verificatiion/bulk_update_case_received_date'; ?>',
          data : new FormData(form),
          type: 'post',
          contentType:false,
          cache: false,
...
$('#download_candidate_template').on('click',function(){
        ...
        $.ajax({
          url : '<?php echo ADMIN_SITE_URL.'court_verificatiion/template_download'; ?>',
          data : '',
          type: 'post',
          dataType:'json',
          beforeSend:function(){
     ...
     $('#export_to_excel_frm').validate({ 
        ...
        {      
            ...
            $.ajax({
            url: '<?php echo ADMIN_SITE_URL.'court_verificatiion/export_to_excel'; ?>',
            data: dataString,
            type: 'POST',
            beforeSend:function(){
