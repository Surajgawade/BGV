                  ...
                  <thead>
                    <tr class="filters">
                     <th><input type="text" class="form-control" placeholder="Sr No" ></th>
                      ...
                      <th><input type="text" class="form-control" placeholder="ID Number" ></th>
...
<div id="addAddcreditreportModel" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
<div class="modal-dialog" style="overflow-y: scroll; max-height:90%;width: 80%;  margin-top: 40px; margin-bottom:40px;">

    <?php echo form_open_multipart("#", array('name'=>'credit_report_add','id'=>'credit_report_add')); ?>
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Add Credit Report</h4>
      </div>
      <span class="errorTxt"></span>
    
      <div id="append_add_credit_report_model"></div>
      <div class="modal-footer">
        <button type="button" id="addcreditreportBack" name="addcreditreportBack" class="btn btn-default btn-sm pull-left">Back</button>
        <button type="submit" id="sbcreditreport" name="sbcreditreport" class="btn btn-success btn-sm">Submit</button>
...
$.fn.dataTable.pipeline = function ( opts ) {
    ...
    var conf = $.extend( {
        ...
        url: '<?php echo CANDIDATE_SITE_URL.'credit_report/credit_report_view'; ?>/',
        data: {'status': status},  
...
$(document).ready(function() {
  ...
  $('#tbl_datatable1 tbody').on('dblclick', 'tr', function () { 
    ...
    $('#append_add_credit_report_model').load(url,function(){
      $('#addAddcreditreportModel').modal('show');
    });
  });
}); 

 $('#credit_report_add').validate({ 
        rules: {
       doc_submited : {
        required : function(){
         
          if ($("input[name ='credit_report_check']:checked").val() == '1') 
          {
            return false;
          }
          else
    ...
    messages: {
      doc_submited : {
        required : function(){
          ...
          {
            return "Select Document Submitted";
          }
        }
      },
      street_address : {
    ...
    {
      $.ajax({
        url : '<?php echo CANDIDATE_SITE_URL.'credit_report/save_credit_report'; ?>',
        ...
        dataType:'json',
        beforeSend:function(){
          $('#sbcreditreport').attr('disabled','disabled');
        ...
        success: function(jdata){
          ...
          if(jdata.status == '<?php echo SUCCESS_CODE; ?>'){
               ...
               $.ajax({
                ...
                data : $('#credit_report_add').serialize(),
                type: 'post',
                async:false,
                cache: false,
                processData:false,
