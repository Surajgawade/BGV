...
<div id="addAddpccModel" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static">
<div class="modal-dialog" style="overflow-y: scroll; max-height:90%;width: 80%;  margin-top: 40px; margin-bottom:40px;">

    <?php echo form_open_multipart("#", array('name'=>'pcc_add','id'=>'pcc_add')); ?>
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Add PCC</h4>
      </div>
      <span class="errorTxt"></span>
    
      <div id="append_add_pcc_model"></div>
      <div class="modal-footer">
        <button type="button" id="addpccBack" name="addpccBack" class="btn btn-default btn-sm pull-left">Back</button>
        <button type="submit" id="sbpcc" name="sbpcc" class="btn btn-success btn-sm">Submit</button>
...
$.fn.dataTable.pipeline = function ( opts ) {
    ...
    var conf = $.extend( {
        ...
        url: '<?php echo CANDIDATE_SITE_URL.'pcc/pcc_view'; ?>/',
        data: {'status': status},  
...
$(document).ready(function() {
  ...
  $('#tbl_datatable1 tbody').on('dblclick', 'tr', function () { 
    ...
    $('#append_add_pcc_model').load(url,function(){
 ...
 $('#pcc_add').validate({ 
    rules: {
      street_address : {
        required : function(){
        if ($("input[name ='pcc_check']:checked").val() == '1') 
          {
            return false;
          }
          else
    ...
    {
      $.ajax({
        url : '<?php echo CANDIDATE_SITE_URL.'pcc/save_pcc'; ?>',
