...
class Court extends MY_Candidate_Cotroller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('candidate_info/Court_model'));
    }

    public function list_view()
    {
        $data['header_title'] = "Court Pending List";

        $this->load->view('candidate_info/header', $data);

        $this->load->view('candidate_info/court_list');
    ...
    {
        $data['header_title'] = "Court Submit List";

        $this->load->view('candidate_info/header', $data);

        $this->load->view('candidate_info/court_submit_list');

        $this->load->view('candidate_info/footer');
    }

    public function court_view()
    {
        ...
        $add_address = $this->Court_model->get_all_court_by_candidate_datatable($params, $columns);

        $totalRecords = count($this->Court_model->get_all_court_by_candidate_datatable_count($params, $columns));
        ...
        foreach ($add_address as $court_candidate) {
            ...
            $data_arry[$x]['encry_id'] = CANDIDATE_SITE_URL . "court/add/" . $court_candidate['candidates_info_id'] . "/" . $court_candidate['courtver_id'];
    ...
    public function court_submit_view()
    {
        ...
        $add_address = $this->Court_model->get_all_court_submit_by_candidate_datatable($params, $columns);

        $totalRecords = count($this->Court_model->get_all_court_submit_by_candidate_datatable_count($params, $columns));
    ...
    public function add($candsid, $id)
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $data['details'] = $this->court_model->get_all_court_record(array('courtver.id' => $id));

            $data['mode_of_verification'] = $this->court_model->get_mode_of_verification(array('entity' => $data['get_cands_details']['entity_id'], 'package' => $data['get_cands_details']['package_id'], 'tbl_clients_id' => $data['get_cands_details']['clientid']));

            echo $this->load->view('candidate_info/courtver_add', $data, true);
        } else {
    ...
    public function save_court()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $result = $this->Court_model->save(array_map('strtolower', $field_array), array('courtver.id' => $frm_details['court_id']));

                $config_array = array('file_upload_path' => $file_upload_path, 'file_permission' => 'jpeg|jpg|png|pdf|docx|xls|xlsx', 'file_size' => BULK_UPLOAD_MAX_SIZE_MB * 1000, 'file_id' => $frm_details['court_id'], 'component_name' => 'courtver_id');
                if (empty($error_msgs)) {
                ...
                if ($result) {
                    ...
                    $check = $this->Court_model->select_insuff(array('courtver_insuff.courtver_id' => $frm_details['court_id'], 'status' => 1));

                    if (!empty($check)) {
                        $result = $this->Court_model->save_update_insuff(array('insuff_clear_date' => date('Y-m-d'), 'insuff_cleared_timestamp' => date(DB_DATE_FORMAT), 'insuff_remarks' => 'Auto - Candidate has updated the info', 'status' => 2, 'courtver_id' => $frm_details['court_id'], 'auto_stamp' => 2, 'insuff_cleared_by' => "Candidate"), array('courtver_id' => $frm_details['court_id']));

                    }

                    $result_update_record = $this->Court_model->save_update(array('verfstatus' => '1', 'var_filter_status' => 'WIP', 'var_report_status' => 'WIP'), array('courtver_result.courtver_id' => $frm_details['court_id']));
