...
class Candidates_closed extends MY_Client_Cotroller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('client/candidates_model_closed'));
    }
    public function index()
    {
        ...
        $this->load->view('client/candidates_list_closed');
    ...
    public function data_table_cands_view()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $cands_results = $this->candidates_model_closed->get_all_cand_with_search($this->user_package, $this->user_role, $params, $columns);

            $totalRecords = count($this->candidates_model_closed->get_all_cand_with_search_count($this->user_package, $this->user_role, $params, $columns));
            $x = 0;
            ...
            foreach ($cands_results as $key => $cands_result) {
                ...
                $data_arry[$x]['view'] = "<a href='#' title='View' class='view_details' data-toggle='modal' data-target='#myModal' data-raw_id='" . $cands_result['id'] . "'><i class='fa fa-eye'></i></a>";
                ...
                $result_address = $this->candidates_model_closed->get_addres_ver_status(array('candidates_info.id' => $cands_result['id']));
                ...
                $result_education = $this->candidates_model_closed->get_education_ver_status(array('candidates_info.id' => $cands_result['id']));
                ...
                $result_employment = $this->candidates_model_closed->get_employment_ver_status(array('candidates_info.id' => $cands_result['id']));
                ...
                $result_court = $this->candidates_model_closed->get_court_ver_status(array('candidates_info.id' => $cands_result['id']));
                ...
                $result_pcc = $this->candidates_model_closed->get_pcc_ver_status(array('candidates_info.id' => $cands_result['id']));
                ...
                $result_reference = $this->candidates_model_closed->get_reference_ver_status(array('reference.candsid' => $cands_result['id']));
                ...
                $result_global = $this->candidates_model_closed->get_global_db_ver_status(array('candidates_info.id' => $cands_result['id']));
    ...
    public function data_table_cands_view_for_client()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $cands_results = $this->candidates_model_closedll_cand_with_search($this->user_role, $params, $columns);

            $totalRecords = count($this->candidates_model_closedll_cand_with_search_count($this->user_role, $params, $columns));
            $x = 0;
            ...
            foreach ($cands_results as $key => $cands_result) {
                ...
                if ($cands_result['addrver'] == 1) {
                    $result = $this->candidates_model_closedddres_ver_status(array('cands.id' => $cands_result['id']));
                    if (!empty($result)) {
                ...
                if ($cands_result['eduver'] == 1) {
                    $result = $this->candidates_model_closedducation_ver_status(array('cands.id' => $cands_result['id']));
                    if (!empty($result)) {
                ...
                if ($cands_result['empver'] == 1) {
                    $result = $this->candidates_model_closedmployment_ver_status(array('cands.id' => $cands_result['id']));
                    if (!empty($result)) {
                ...
                if ($cands_result['courtver'] == 1) {
                    $result = $this->candidates_model_closedourt_ver_status(array('cands.id' => $cands_result['id']));
                    if (!empty($result)) {
                ...
                if ($cands_result['crimver'] == 1) {
                    $result = $this->candidates_model_closedcc_ver_status(array('cands.id' => $cands_result['id']));
                    if (!empty($result)) {
                ...
                if ($cands_result['globdbver'] == 1) {
                    $result = $this->candidates_model_closedlobal_db_ver_status(array('cands.id' => $cands_result['id']));
    ...
    public function view_details($cand_id)
    {
        ...
        if (!empty($cand_id)) {
            $candidate_details = $this->candidates_model_closed->select(true, array(), array('id' => $cand_id));
            ...
            if (!empty($candidate_details)) {
                ...
                echo $this->load->view('client/candidate_details_view_closed', $data, true);
            } else {
                show_404();
            }
        } else {
    ...
    public function new_candidates()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $result = $this->candidates_model_closed($fields);
    ...
    public function report($id = null)
    {
        ...
        if (!empty($id)) {
            ...
            $cands_result = $this->candidates_model_closedandidates_info_info_report(array('candidates_info.id' => $id));

            if (!empty($cands_result)) {
                $report['cand_info'] = $cands_result;

                $report['education_info'] = $this->candidates_model_closedtion_info_report(array('education.candsid' => $id, 'education_files.type' => 1));

                $report['employment_info'] = $this->candidates_model_closedyment_info_report(array('empver.candsid' => $id));

                $report['address_info'] = $this->candidates_model_closedss_info_report(array('addrver.candsid' => $id));

                $report['pcc_info'] = $this->candidates_model_closednfo_report(array('p1.candsid' => $id));

                $report['court_info'] = $this->candidates_model_closedinfo_report(array('courtver.candsid' => $id));

                $report['references_info'] = $this->candidates_model_closedences_report(array('reference.candsid' => $id));
...
//                $ctime = filemtime($path . $file) .":".  $file;
    ...
    public function genetare_excel_reprots()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if (!empty($frm_details)) {
                ...
                $report_requested_save_id = $this->candidates_model_closedt_requested_save($requested_data);
                ...
                if ($report_requested_save_id) {
                    ...
                    $this->candidates_model_closedt_requested_save(array('query' => $cmd), array('id' => $report_requested_save_id));
    ...
    $prod  =  $this->candidates_model_closedxcel($clientid,array('from_date' => $from_date,'to_date' => $to_date));
    ...
    public function downloading($id, $folder_name)
    {
        ...
        $status = $this->candidates_model_closedt_requested_select(array('id' => $id, 'folder_name' => $folder_name, 'requested_id' => $this->session->userdata('id')));

        if (!empty($status)) {
            $this->db->set('downloaded_status', 'downloaded_status+1', false);

            $this->candidates_model_closedt_requested_save(array('downloaded_on_date' => date(DB_DATE_FORMAT)), array('id' => $id));
