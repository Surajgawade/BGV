...
class Candidates_insufficiency extends MY_Client_Cotroller
{
    ...
    public function index()
    {
        ...
        $this->load->view('client/candidates_list_insufficiency');
    ...
    public function data_table_cands_view_for_client()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $cands_results = $this->candidates_model_insufficiency->get_all_cand_with_search($this->user_role, $params, $columns);

            $totalRecords = count($this->candidates_model_insufficiency->get_all_cand_with_search_count($this->user_role, $params, $columns));
            $x = 0;
            ...
            foreach ($cands_results as $key => $cands_result) {
                ...
                if ($cands_result['addrver'] == 1) {
                    $result = $this->candidates_model_insufficiency->get_addres_ver_status(array('cands.id' => $cands_result['id']));
                    if (!empty($result)) {
                ...
                if ($cands_result['eduver'] == 1) {
                    $result = $this->candidates_model_insufficiency->get_education_ver_status(array('cands.id' => $cands_result['id']));
                    if (!empty($result)) {
                ...
                if ($cands_result['empver'] == 1) {
                    $result = $this->candidates_model_insufficiency->get_employment_ver_status(array('cands.id' => $cands_result['id']));
                    if (!empty($result)) {
                ...
                if ($cands_result['courtver'] == 1) {
                    $result = $this->candidates_model_insufficiency->get_court_ver_status(array('cands.id' => $cands_result['id']));
                    if (!empty($result)) {
                ...
                if ($cands_result['crimver'] == 1) {
                    $result = $this->candidates_model_insufficiency->get_pcc_ver_status(array('cands.id' => $cands_result['id']));
                    if (!empty($result)) {
                ...
                if ($cands_result['globdbver'] == 1) {
                    $result = $this->candidates_model_insufficiency->get_global_db_ver_status(array('cands.id' => $cands_result['id']));
    ...
    public function view_details($cand_id)
    {
        ...
        if (!empty($cand_id)) {
            $candidate_details = $this->candidates_model_insufficiency->select(true, array(), array('id' => $cand_id));
            ...
            if (!empty($candidate_details)) {
                ...
                echo $this->load->view('client/candidate_details_view_insufficiency', $data, true);
            } else {
                show_404();
            }
        } else {
    ...
    public function new_candidates()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $result = $this->candidates_model_insufficiency->save($fields);
    ...
    public function report($id = null)
    {
        ...
        if (!empty($id)) {
            ...
            $cands_result = $this->candidates_model_insufficiency->get_candidates_info_info_report(array('candidates_info.id' => $id));

            if (!empty($cands_result)) {
                $report['cand_info'] = $cands_result;

                $report['education_info'] = $this->candidates_model_insufficiency->education_info_report(array('education.candsid' => $id, 'education_files.type' => 1));

                $report['employment_info'] = $this->candidates_model_insufficiency->employment_info_report(array('empver.candsid' => $id));

                $report['address_info'] = $this->candidates_model_insufficiency->address_info_report(array('addrver.candsid' => $id));

                $report['pcc_info'] = $this->candidates_model_insufficiency->pcc_info_report(array('p1.candsid' => $id));

                $report['court_info'] = $this->candidates_model_insufficiency->courtinfo_report(array('courtver.candsid' => $id));

                $report['references_info'] = $this->candidates_model_insufficiency->references_report(array('reference.candsid' => $id));
...
//                $ctime = filemtime($path . $file) .":".  $file;
    ...
    public function genetare_excel_reprots()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if (!empty($frm_details)) {
                ...
                $report_requested_save_id = $this->candidates_model_insufficiency->report_requested_save($requested_data);
                ...
                if ($report_requested_save_id) {
                    ...
                    $this->candidates_model_insufficiency->report_requested_save(array('query' => $cmd), array('id' => $report_requested_save_id));
    ...
    public function downloading($id, $folder_name)
    {
        ...
        $status = $this->candidates_model_insufficiency->report_requested_select(array('id' => $id, 'folder_name' => $folder_name, 'requested_id' => $this->session->userdata('id')));

        if (!empty($status)) {
            $this->db->set('downloaded_status', 'downloaded_status+1', false);

            $this->candidates_model_insufficiency->report_requested_save(array('downloaded_on_date' => date(DB_DATE_FORMAT)), array('id' => $id));
