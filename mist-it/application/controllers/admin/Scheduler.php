...
class Scheduler extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['clients_id_prepost'] = $this->get_clients_prepost();
        $data['clients_id_report'] = $this->get_clients(array('status' => STATUS_ACTIVE));
    ...
    protected function get_clients_prepost()
    {
        $this->load->model('candidates_model');

        $clients = $this->candidates_model->select_pre_post_client('clients', false, array('id', 'clientname'));
    ...
    public function export_to_excel()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $requested_data = array('type' => 'Candidates Export', 'requested_id' => $this->user_info['id'], 'file_name' => '', 'created_on' => date(DB_DATE_FORMAT), 'mail_send_status' => 0, 'folder_generated_status' => 0, 'downloaded_status' => 0, 'downloaded_on_date' => '', 'folder_name' => '');

            $report_requested_save_id = $this->report_generated_user->report_requested_save($requested_data);

            if ($report_requested_save_id) {

                $client_id = ($this->input->post('clientid') != "All") ? $this->input->post('clientid') : 0;

                $entity_id = ($this->input->post('entity') != 0) ? $this->input->post('entity') : 0;

                $package_id = ($this->input->post('package') != 0) ? $this->input->post('package') : 0;

                $client_name = $this->input->post('client_name');

                $fil_by_status = ($this->input->post('fil_by_status') != "All") ? $this->input->post('fil_by_status') : 0;

                $fil_by_sub_status = ($this->input->post('fil_by_sub_status') != 0) ? $this->input->post('fil_by_sub_status') : 0;

                $this->report_generated_user->report_requested_schedular(array('file_name' => '', 'run_status' => ''), array('id' => '6'));

                $url = "cli_request_only export_to_excel $report_requested_save_id $client_id $entity_id $package_id $client_name $fil_by_status $fil_by_sub_status";
                ...
                $this->update_request_schedular_list(array('last_run_by' => $this->user_info['id']), array('id' => '6'));
    ...
    public function export_to_excel_prepost()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $requested_data = array('type' => 'Pre Post', 'requested_id' => $this->user_info['id'], 'file_name' => '', 'created_on' => date(DB_DATE_FORMAT), 'mail_send_status' => 0, 'folder_generated_status' => 0, 'downloaded_status' => 0, 'downloaded_on_date' => '', 'folder_name' => '');
            ...
            if ($report_requested_save_id) {
                ...
                $this->report_generated_user->report_requested_schedular(array('file_name' => '', 'run_status' => ''), array('id' => '1'));

                $url = "cli_request_only export_to_excel_prepost $report_requested_save_id $client_id $entity_id $package_id $client_name";
                ...
                $this->update_request_schedular_list(array('last_run_by' => $this->user_info['id']), array('id' => '1'));
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($frm_details['activity_from'] <= $frm_details['activity_to']) {
                ...
                if ($report_requested_save_id) {
                    ...
                    $activity_from = $frm_details['activity_from'];
                    $activity_to = $frm_details['activity_to'];

                    $this->report_generated_user->report_requested_schedular(array('file_name' => '', 'run_status' => ''), array('id' => '1'));

                    $url = "cli_request_only generate_export_activity_data $report_requested_save_id  $activity_from $activity_to";
    ...
    public function export_wip_insuff_report()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $requested_data = array('type' => 'WIP_Insuff_report', 'requested_id' => $this->user_info['id'], 'file_name' => '', 'created_on' => date(DB_DATE_FORMAT), 'mail_send_status' => 0, 'folder_generated_status' => 0, 'downloaded_status' => 0, 'downloaded_on_date' => '', 'folder_name' => '');

            $report_requested_save_id = $this->report_generated_user->report_requested_save($requested_data);

            if ($report_requested_save_id) {

                $url = "cli_request_only generate_export_wip_insuff_data $report_requested_save_id";
    ...
    public function file_download_zip($file_name)
    {
 
        if ($file_name) {
            $file = SITE_BASE_PATH . CANDIDATES . $file_name;
    ...
    public function prepost_recent_file()
    {

        $select_prepost_file = $this->report_generated_user->get_prepost_file();
        ...
        $srno = 1;
        foreach ($select_prepost_file as $key => $value) {
    ...
    public function wip_insuff_file()
    {

        $select_activity_file = $this->report_generated_user->get_wip_insuff_file();
        $file_path = UPLOAD_FOLDER . 'bulk_reports/';
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($frm_details['activity_from_axis'] <= $frm_details['activity_to_axis']) {
                ...
                if ($report_requested_save_id) {
                    ...
                    $this->update_request_schedular_list(array('last_run_by' => $this->user_info['id']), array('id' => '2'));
    ...
    public function export_axis_report_new()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($frm_details['activity_from_axis'] <= $frm_details['activity_to_axis']) {
                ...
                $requested_data = array('type' => 'Axis New Format Tracker', 'requested_id' => $this->user_info['id'], 'file_name' => '', 'created_on' => date(DB_DATE_FORMAT), 'mail_send_status' => 0, 'folder_generated_status' => 0, 'downloaded_status' => 0, 'downloaded_on_date' => '', 'folder_name' => '');

                $report_requested_save_id = $this->report_generated_user->report_requested_save($requested_data);

                if ($report_requested_save_id) {
                    $this->report_generated_user->report_requested_schedular(array('file_name' => '', 'run_status' => ''), array('id' => '7'));

                    $url = "cli_request_only generate_excel_axis_report_new $report_requested_save_id";

                    $cmd = 'php /var/www/html/'.SITE_FOLDER.'index.php ' . $url;

                    //   $this->candidates_model->report_requested_save(array('query'=>$cmd),array('id'=>$report_requested_save_id));

                    $this->update_request_schedular_list(array('last_run_by' => $this->user_info['id']), array('id' => '7'));
    ...
    public function export_axis_ikya_report_new()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($frm_details['activity_from_axis'] <= $frm_details['activity_to_axis']) {
                ...
                $requested_data = array('type' => 'Axis Securities Limited New(Ikya)', 'requested_id' => $this->user_info['id'], 'file_name' => '', 'created_on' => date(DB_DATE_FORMAT), 'mail_send_status' => 0, 'folder_generated_status' => 0, 'downloaded_status' => 0, 'downloaded_on_date' => '', 'folder_name' => '');

                $report_requested_save_id = $this->report_generated_user->report_requested_save($requested_data);

                if ($report_requested_save_id) {

                    $this->report_generated_user->report_requested_schedular(array('file_name' => '', 'run_status' => ''), array('id' => '8'));

                    $url = "cli_request_only generate_excel_axis_ikya_report_new $report_requested_save_id";

                    $cmd = 'php /var/www/html/'.SITE_FOLDER.'index.php ' . $url;

                    //   $this->candidates_model->report_requested_save(array('query'=>$cmd),array('id'=>$report_requested_save_id));

                    $this->update_request_schedular_list(array('last_run_by' => $this->user_info['id']), array('id' => '8'));
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($frm_details['activity_from_axis'] <= $frm_details['activity_to_axis']) {
                ...
                if ($report_requested_save_id) {
                    ...
                    $this->update_request_schedular_list(array('last_run_by' => $this->user_info['id']), array('id' => '3'));
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($frm_details['activity_from_axis'] <= $frm_details['activity_to_axis']) {
                ...
                if ($report_requested_save_id) {
                    ...
                    $this->update_request_schedular_list(array('last_run_by' => $this->user_info['id']), array('id' => '4'));
    ...
    public function create_report()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $requested_data = array('type' => 'Report', 'requested_id' => $this->user_info['id'], 'file_name' => '', 'created_on' => date(DB_DATE_FORMAT), 'mail_send_status' => 0, 'folder_generated_status' => 0, 'downloaded_status' => 0, 'downloaded_on_date' => '', 'folder_name' => '');
            ...
            if ($report_requested_save_id) {
                ...
                $this->report_generated_user->report_requested_schedular(array('file_name' => '', 'run_status' => ''), array('id' => '2'));

                $url = "cli_request_only create_closed_report $report_requested_save_id $client_id $entity_id $package_id $client_name";
    ...
    public function create_id_report()
    {
         $json_array = array();
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $mist_ids  =  trim($frm_details['candidate_id']);
           
            $mist_id = preg_replace('/\s+/', '', $mist_ids );
            ...
            if ($report_requested_save_id) {
                ...
                $url = "cli_request_only create_closed_report_id $report_requested_save_id $mist_id";
    ...
    public function create_closure()
    {
         $json_array = array();
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($report_requested_save_id) {
                ...
                $this->report_generated_user->report_requested_schedular(array('file_name' => '', 'run_status' => ''), array('id' => '3'));

                $url = "cli_request_only create_closure_report $report_requested_save_id $client_id $entity_id $package_id $client_name";
    ...
    public function create_component_report()
    {
        $json_array = array();
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $requested_data = array('type' => 'Component Export', 'requested_id' => $this->user_info['id'], 'file_name' => '', 'created_on' => date(DB_DATE_FORMAT), 'mail_send_status' => 0, 'folder_generated_status' => 0, 'downloaded_status' => 0, 'downloaded_on_date' => '', 'folder_name' => '');

            $report_requested_save_id = $this->report_generated_user->report_requested_save($requested_data);
            
            if ($report_requested_save_id) {
               
                $component_name = ($this->input->post('components_id') != "") ? $this->input->post('components_id') : 0;

                $client_id = ($this->input->post('clientid') != "All") ? $this->input->post('clientid') : 0;

                $status_value = $this->input->post('status_value_component') ;

                $from_date = ($this->input->post('from_date') != "") ? convert_display_to_db_date($this->input->post('from_date')) : 0;

                $to_date = ($this->input->post('end_date') != "") ? convert_display_to_db_date($this->input->post('end_date')) : 0;

                $client_name = $this->input->post('client_name');
 
                

                $this->report_generated_user->report_requested_schedular(array('file_name' => '', 'run_status' => ''), array('id' => '4'));

                $url = "cli_request_only export_to_excel_component $report_requested_save_id $component_name $client_id $status_value $from_date $to_date $client_name";
    ...
    public function export_tracker()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $requested_data = array('type' => 'Tracker Export', 'requested_id' => $this->user_info['id'], 'file_name' => '', 'created_on' => date(DB_DATE_FORMAT), 'mail_send_status' => 0, 'folder_generated_status' => 0, 'downloaded_status' => 0, 'downloaded_on_date' => '', 'folder_name' => '');
            ...
            if ($report_requested_save_id) {
                ...
                $client_name = str_replace(' ','_',$this->input->post('client_name'));

                $fil_by_status = ($this->input->post('fil_by_status') != "All") ? $this->input->post('fil_by_status') : 0;

                $fil_by_sub_status = ($this->input->post('fil_by_sub_status') != 0) ? $this->input->post('fil_by_sub_status') : 0;

                $this->report_generated_user->report_requested_schedular(array('file_name' => '', 'run_status' => ''), array('id' => '5'));

                $url = "cli_request_only export_to_excel_tracker $report_requested_save_id $client_id $entity_id $package_id $client_name $fil_by_status $fil_by_sub_status";
                ...
                $this->update_request_schedular_list(array('last_run_by' => $this->user_info['id']), array('id' => '5'));
