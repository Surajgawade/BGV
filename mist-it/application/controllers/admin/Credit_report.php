...
class Credit_report extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->perm_array = array('page_id' => 15);
        if ($this->input->is_ajax_request()) {
    ...
    public function index()
    {
        ...
        $this->load->view('admin/credit_report_list');

        $this->load->view('admin/footer');
    }

    public function get_clients_for_credit_report_list_view()
    {
        $params = $_REQUEST;

        $clients = $this->credit_report_model->select_client_list_view_credit_report('clients', false, array('clients.id', 'clients.clientname'), $params);
    ...
    public function credit_report_view_datatable()
    {
        ...
        $credit_report_candidates = $this->credit_report_model->get_all_credit_report_record_datatable(false, $params, $columns);

        $totalRecords = count($this->credit_report_model->get_all_credit_report_record_datatable_count(false, $params, $columns));
    ...
    public function add($candsid)
    {
        if ($this->input->is_ajax_request()) {
            ...
            //$data['assigned_user_id'] = $this->credit_report_model->get_assign_users_id('user_profile', false, array("`user_profile`.`status`,`user_profile`.`id`,`user_profile`.`email`,`user_profile`.`department`,`user_profile`.`user_name`,concat(`user_profile`.`firstname`,' ',`user_profile`.`lastname`) as fullname,`user_profile`.`profile_pic`,`user_profile`.`firstname`,`user_profile`.`lastname`,`roles`.`role_name`,`roles`.`groups_id`,`roles_permissions`.*"), array('user_profile.status' => STATUS_ACTIVE));

            $data['assigned_user_id']  = $this->get_reporting_manager_for_execituve($data['get_cands_details']['clientid']);

            echo $this->load->view('admin/credit_report_add', $data, true);
        }
    }

    /*   protected function credit_report_com_ref()
    {
    $lists = $this->credit_report_model->credit_report_com_ref();

    $number =  (!empty($lists)) ? $lists['A_I']+1 : 1000;

    return COMPONENT_REF_NO['CREDIT_REPORT'].$number;
    }

    protected function bulk_credit_report_com_ref()
    {
    $lists = $this->credit_report_model->credit_report_com_ref();
    ...
    public function save_form()
    {
        if ($this->input->is_ajax_request()) {
            ...
            // $this->form_validation->set_rules('credit_report_com_ref', 'Component Ref','required');
            ...
            if ($this->form_validation->run() == false) {
                ...
                if (empty($error_msgs)) {
                    ...
                    if ($_FILES['attchments_cs']['name'][0] != '') {
                        ...
                        if (!empty($retunr_cd)) {
                            $this->common_model->common_insert_batch('credit_report_files', $retunr_cd['success']);
                        }
                    }
                }
                ...
                if ($result) {
                    ...
                    $user_activity_data = $this->common_model->user_actity_data(array('component' => "Credit Report",
                        'ref_no' => $credit_report_com_ref, 'candidate_name' => $frm_details['CandidateName'], 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id'], 'activity_type' => 'Manual', 'action' => 'Add'));

                    auto_update_overall_status($frm_details['candsid']);
                    auto_update_tat_status($frm_details['candsid']);
                    
                    $vendor_id = $this->credit_report_model->get_vendor_id($frm_details['clientid']);  

                        if(!empty($vendor_id))
                        {
                            $update = $this->credit_report_model->upload_vendor_assign('credit_report', array('vendor_id' => $vendor_id[0]['id'], 'vendor_list_mode' => 'written', 'vendor_assgined_on' => date(DB_DATE_FORMAT)), array('id' => $result, 'vendor_id =' => 0));

                            $update1 = $this->credit_report_model->update_status('credit_report_result', array('verfstatus' => 1), array('credit_report_id' => $result));
                            ...
                            $save_vendor_log = $this->credit_report_model->save_vendor_log($fiels);
                    ...
                    if ($result) {
                        ...
                        $json_array['redirect'] = ADMIN_SITE_URL . 'credit_report';
    ...
    public function get_assign_credit_report_list_view()
    {
        log_message('error', 'Credit Report List Assign in Assign View');
        try {
            $data['header_title'] = "Credit Report Verification";

            $data['filter_view'] = $this->filter_view_assign_credit_list();
            $data['users_list'] = $this->users_list();
            $data['vendor_list'] = $this->vendor_list('cbrver');

        } catch (Exception $e) {
            log_message('error', 'Credit_report::get_assign_credit_list_view');
            log_message('error', $e->getMessage());
        }

        echo $this->load->view('admin/assign_credit_report', $data, true);
    }

    protected function filter_view_assign_credit_list($true = false)
    {
        ...
        $data['clients'] = $this->get_clients_for_list_view_credit();
        $data['user_list_name'] = $this->users_list_filter();

        return $this->load->view('admin/filter_view_assign', $data, true);
    }
    public function get_clients_for_list_view_credit()
    {
        $this->load->model('assign_credit_report_model');
    ...
    public function view_from_inside($add_comp_id = false)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $cget_details = $this->credit_report_model->get_all_credit_report_record(array('credit_report.credit_report_com_ref' => $add_comp_id));
            if (!empty($cget_details)) {
              ...
              //  $data['assigned_user_id'] = $this->credit_report_model->get_assign_users_id('user_profile', false, array("`user_profile`.`status`,`user_profile`.`id`,`user_profile`.`email`,`user_profile`.`department`,`user_profile`.`user_name`,concat(`user_profile`.`firstname`,' ',`user_profile`.`lastname`) as fullname,`user_profile`.`profile_pic`,`user_profile`.`firstname`,`user_profile`.`lastname`,`roles`.`role_name`,`roles`.`groups_id`,`roles_permissions`.*"), array('user_profile.status' => STATUS_ACTIVE));
                ...
                $data['attachments'] = $this->credit_report_model->select_file(array('id', 'file_name', 'real_filename', 'type'), array('credit_report_id' => $cget_details[0]['credit_report_id'], 'status' => 1));

                $check_insuff_raise = $this->credit_report_model->select_insuff(array('credit_report_id' => $cget_details[0]['credit_report_id'], 'status' => STATUS_ACTIVE, 'insuff_clear_date is null' => null));

                $data['reinitiated'] = ($cget_details[0]['var_filter_status'] == 'Closed' || $cget_details[0]['var_filter_status'] == 'closed') ? '1' : '2';

                
                $data['insuff_reason_list'] = $this->insuff_reason_list(10);
                ...
                echo $this->load->view('admin/credit_report_edit_view_inside', $data, true);
            } else {
    ...
    {
        if ($this->input->is_ajax_request()) {
            $this->form_validation->set_rules('credit_report_id', 'ID', 'required');
            ...
            if ($this->form_validation->run() == false) {
                ...
                $field_array = array('clientid' => $frm_details['clientid'],
                    'candsid' => $frm_details['candsid'],
                    'credit_report_com_ref' => $frm_details['credit_report_com_ref'],
                    'iniated_date' => convert_display_to_db_date($frm_details['iniated_date']),
                    'doc_submited' => $frm_details['doc_submited'],
                    'id_number' => $frm_details['id_number'],
                    'street_address' => $frm_details['street_address'],
                ...
                $result = $this->credit_report_model->save(array_map('strtolower', $field_array), array('id' => $frm_details['credit_report_id']));
                ...
                if ($result) {
                    $user_activity_data = $this->common_model->user_actity_data(array('component' => "Credit Report",
                        'ref_no' => $frm_details['credit_report_com_ref'], 'candidate_name' => $frm_details['CandidateName'], 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id'], 'activity_type' => 'Manual', 'action' => 'Edit'));
    ...
    public function view_details($credit_report_id = '')
    {
        if (!empty($credit_report_id)) {
            $details = $this->credit_report_model->get_all_credit_report_record(array('credit_report.id' => decrypt($credit_report_id)));

            if ($credit_report_id && !empty($details)) {
                $data['header_title'] = 'Credit Report Verification';
              ...
              //  $data['assigned_user_id'] = $this->credit_report_model->get_assign_users_id('user_profile', false, array("`user_profile`.`status`,`user_profile`.`id`,`user_profile`.`email`,`user_profile`.`department`,`user_profile`.`user_name`,concat(`user_profile`.`firstname`,' ',`user_profile`.`lastname`) as fullname,`user_profile`.`profile_pic`,`user_profile`.`firstname`,`user_profile`.`lastname`,`roles`.`role_name`,`roles`.`groups_id`,`roles_permissions`.*"), array('user_profile.status' => STATUS_ACTIVE));
                ...
                $check_insuff_raise = $this->credit_report_model->select_insuff(array('credit_report_id' => decrypt($credit_report_id), 'status' => STATUS_ACTIVE, 'insuff_clear_date is null' => null));
                ...
                $data['attachments'] = $this->credit_report_model->select_file(array('id', 'file_name', 'real_filename', 'type'), array('credit_report_id' => decrypt($credit_report_id), 'status' => 1));
                ...
                $this->load->view('admin/credit_report_edit_view');
    ...
    {
        $details = $this->credit_report_model->get_all_credit_report_record(array('credit_report.id' => $where_id));
        ...
        if ($where_id && !empty($details)) {
            ...
            $data['attachments'] = $this->credit_report_model->select_file(array('id', 'file_name', 'status'), array('credit_report_id' => $where_id, 'type' => 1));

            $data['vendor_attachments'] = $this->credit_report_model->select_file_vendor(array('view_vendor_master_log_file.id', 'view_vendor_master_log_file.file_name', 'view_vendor_master_log_file.real_filename', 'view_vendor_master_log_file.status'), array('view_vendor_master_log_file.component_tbl_id' => 10), $where_id);

            echo $this->load->view('admin/credit_report_add_result_model_view', $data, true);
        } else {
            ...
            if ($this->form_validation->run() == false) {
                ...
                if (($frm_details['action_val'] != "Select") || ($frm_details['activity_last_id'] != "")) {
                    ...
                    $field_array = array('verfstatus' => $verfstatus['id'],
                        ...
                        'credit_report_id' => $frm_details['credit_report_id'],
                    ...
                    {
                        $this->credit_report_model->delete_uploaded_file($frm_details['remove_file']);
                    }
                    if (isset($frm_details['add_file'])) // delete uploaded file
                    {
                        $this->credit_report_model->add_uploaded_file($frm_details['add_file']);
                    }

                    if (isset($frm_details['vendor_remove_file'])) // delete uploaded file
                    {
                        $this->credit_report_model->vendor_delete_uploaded_file($frm_details['vendor_remove_file']);
                    }
                    if (isset($frm_details['vendor_add_file'])) // delete uploaded file
                    {
                        $this->credit_report_model->vendor_add_uploaded_file($frm_details['vendor_add_file']);
                    }

                    $field_array = array_map('strtolower', $field_array);

                    $result = $this->credit_report_model->save_update_ver_result($field_array, array('id' => $frm_details['credit_report_result_id']));

                    $result_credit_report = $this->credit_report_model->save_update_result_credit(array_map('strtolower', $field_array));

                    if ($verfstatus['id'] == 9 || $verfstatus['id'] == 27 || $verfstatus['id'] == 28) {

                        $get_vendor_log_deatail = $this->credit_report_model->check_vendor_status_closed_or_not(array('view_vendor_master_log.component' => "cbrver", 'view_vendor_master_log.component_tbl_id' => 10, 'credit_report.id' => $frm_details['credit_report_id']));

                        if (!empty($get_vendor_log_deatail)) {

                            $update_vendor_log_deatail = $this->credit_report_model->reject_cost_vendor(array('final_status' => 'cancelled', 'status' => 5, 'remarks' => 'Stop Check', 'rejected_by' => $this->user_info['id'], 'rejected_on' => date(DB_DATE_FORMAT), 'vendor_remark' => 'Stop Check', 'modified_on' => date(DB_DATE_FORMAT)), array('view_vendor_master_log.id' => $get_vendor_log_deatail[0]['id']));

                            $field_array_credit_report = array(
                                'remarks' => "Stop Check",
                                'modified_by' => $this->user_info['id'],
                                'modified_on' => date(DB_DATE_FORMAT),
                                'status' => 2,
                            ...
                            $credit_report_vendor_result = $this->credit_report_model->update_credit_report_vendor_log('credit_report_vendor_log', $field_array_credit_report, array('id' => $get_vendor_log_deatail[0]['credit_report_vendor_log_id']));
                    ...
                    $config_array = array(
                        ...
                        'file_id'           => $frm_details['credit_report_id'], 
                        'component_name'    => 'credit_report_id'
                    );
    ...
    public function add_verificarion_ver_result()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $result_credit_report = $this->credit_report_model->save_update_result_credit(array_map('strtolower', $field_array), array('id' => $frm_details['result_update_id']));
    ...
    public function credit_report_result_list($emp_id = '')
    {

        if ($emp_id && $this->input->is_ajax_request()) {
            $credit_report_result = $this->credit_report_model->select_result_log(array('credit_report_ver_result.credit_report_id' => $emp_id, 'activity_log_id !=' => null));
            ...
            if (!empty($credit_report_result[0]['id'])) {
                $l = 1;
                foreach ($credit_report_result as $key => $value) {
                    ...
                    if ($value['file_names']) {
                        ...
                        for ($i = 0; $i < count($files); $i++) {
                            $url = "'" . SITE_URL . CREDIT_REPORT . $value['clientid'] . '/';
                            $actual_file = $files[$i] . "'";
                            $myWin = "'" . "myWin" . "'";
                    ...
                    if ($l == 1) {
                        $html_view .= '<td><button data-id="showAddResultModel" data-url ="' . ADMIN_SITE_URL . 'credit_report/credit_report_result_list_idwise/' . $value['id'] . '/' . str_replace(" ", "", $value['activity_action']) . '" data-toggle="modal" data-target="#showAddResultModel1"  class="btn-info  showAddResultModel"> View </button></td>';
                    } else {
                        $html_view .= '<td>  </td>';
                    }
                    $html_view .= '</tr>';
    ...
    public function credit_report_result_list_idwise($where_id, $url)
    {

        $details = $this->credit_report_model->select_result_log1(array('credit_report_ver_result.id' => $where_id));
        ...
        if ($where_id && !empty($details)) {
            ...
            $data['url'] = $url;
            $data['attachments'] = $this->credit_report_model->select_file(array('id', 'file_name', 'status'), array('credit_report_id' => $details[0]['credit_report_id'], 'type' => 1));
          
            $data['vendor_attachments'] = $this->credit_report_model->select_file_vendor(array('view_vendor_master_log_file.id', 'view_vendor_master_log_file.file_name', 'view_vendor_master_log_file.real_filename', 'view_vendor_master_log_file.status'), array('view_vendor_master_log_file.component_tbl_id' => 10), $details[0]['credit_report_id']);


            echo $this->load->view('admin/credit_report_add_result_model_view_log', $data, true);
        } else {
            echo "<h4>Record Not Found</h4>";
        }
    ...
    public function credit_report_reinitiated_date()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $credit_report_id = $this->input->post('update_id');
            $reinitiated_date = $this->input->post('reinitiated_date');
            $reinitiated_remark = $this->input->post('reinitiated_remark');
            $clientid = $this->input->post('clientid');

            $check = $this->credit_report_model->select_reinitiated_date(array('id' => $credit_report_id));

            if ($check[0]['credit_report_re_open_date'] == "0000-00-00" || $check[0]['credit_report_re_open_date'] == "") {
                $reinitiated_dates = $reinitiated_date;
            } else {
                $reinitiated_dates = $check[0]['credit_report_re_open_date'] . "||" . $reinitiated_date;
            }

            $result = $this->credit_report_model->save_update_initiated_date(array('credit_report_re_open_date' => $reinitiated_dates, 'credit_report_reinitiated_remark' => $reinitiated_remark), array('id' => $credit_report_id));

            $result_credit_report = $this->credit_report_model->save_update_initiated_date_credit_report(array('verfstatus' => 26, 'var_filter_status' => "WIP", 'var_report_status' => "WIP", 'first_qc_approve' => "", 'first_qc_updated_on' => "", 'first_qu_reject_reason' => "", 'first_qu_updated_by' => ""), array('credit_report_id' => $credit_report_id));

            $file_upload_path = SITE_BASE_PATH . CREDIT_REPORT . $clientid;
            if (!folder_exist($file_upload_path)) {
                mkdir($file_upload_path, 0777);
            } else if (!is_writable($file_upload_path)) {
                array_push($error_msgs, 'Problem while uploading');
            ...
            $config_array = array('file_upload_path' => $file_upload_path, 'file_permission' => 'jpeg|jpg|png|pdf|docx|xls|xlsx', 'file_size' => BULK_UPLOAD_MAX_SIZE_MB * 1000, 'file_id' => $credit_report_id, 'component_name' => 'credit_report_id');
            ...
            $result_credit_report_activity_data = $this->credit_report_model->initiated_date_credit_report_activity_data(array('candsid' => $this->input->post('candidates_info_id'), 'comp_table_id' => $credit_report_id, 'action' => "Re-Initiated", '  activity_status' => "Re-Initiated", 'remarks' => 'Client requested to re-verify the case [' . $reinitiated_remark . ']', 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id']));

            if ($result && $result_credit_report) {
    ...
    public function approve_first_qc()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $result = $this->credit_report_model->save_first_qc_result(array("first_qc_approve" => "First QC Approve", "first_qc_updated_on" => $accepted_on, "first_qu_updated_by" => $this->user_info['id']), array("credit_report_id" => $comp_id, "id" => $frist_qc_id, "candsid" => $cands_id));
    ...
    public function insuff_raised()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if (empty($check)) {
                $result = $this->credit_report_model->save_update_insuff(array('insuff_raised_date' => convert_display_to_db_date($insff_date), 'insuff_raise_remark' => $this->input->post('insuff_raise_remark'), 'status' => STATUS_ACTIVE, 'insff_reason' => $insff_reason, 'created_by' => $this->user_info['id'], 'credit_report_id' => $credit_report_id, 'auto_stamp' => 1));

                if ($result) {
                    $user_activity_data = $this->common_model->user_actity_data(array('component' => "Credit Report", 'ref_no' => $ref_no, 'candidate_name' => $CandidateName, 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id'], 'activity_type' => 'Manual', 'action' => 'Insuff Raised'));
                }
    ...
    public function insuff_tab_view($emp_id = '')
    {
        if ($this->input->is_ajax_request() && $emp_id) {
            $data['insuff_details'] = $this->credit_report_model->select_insuff_join(array('credit_report_id' => $emp_id));

            echo $this->load->view('admin/cbrver_insuff_view', $data, true);
        } else {
    ...
    public function insuff_clear()
    {
        ...
        if ($this->input->is_ajax_request() && $this->permission['access_credit_list_insuff_clear'] == 1) {
            if (convert_display_to_db_date($frm_details['insuff_clear_date']) >= convert_display_to_db_date($frm_details['check_insuff_raise'])) {
                ...
                $get_vendor_log_deatail = $this->credit_report_model->check_vendor_status_insufficiency(array('view_vendor_master_log.component' => "cbrver", 'view_vendor_master_log.component_tbl_id' => 10,
                    'view_vendor_master_log.final_status' => 'insufficiency', 'credit_report.id' => $frm_details['clear_update_id']));

                if (!empty($get_vendor_log_deatail)) {

                    $update_vendor_log_deatail = $this->credit_report_model->reject_cost_vendor(array('final_status' => 'wip', 'vendor_remark' => '', 'modified_on' => date(DB_DATE_FORMAT)), array('view_vendor_master_log.id' => $get_vendor_log_deatail[0]['id']));

                }

                if ($result) {
                    $user_activity_data = $this->common_model->user_actity_data(array('component' => "Credit Report", 'ref_no' => $frm_details['component_ref_no'], 'candidate_name' => $frm_details['CandidateName'], 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id'], 'activity_type' => 'Manual', 'action' => 'Insuff Cleared'));
                }
                ...
                if (!empty($_FILES['clear_attchments']['name'][0])) {
                    ...
                    for ($i = 0; $i < $files_count; $i++) {
                        ...
                        if ($this->upload->do_upload('attchment')) {
                            array_push($file_array, array(
                                ...
                                'credit_report_id' => $frm_details['clear_update_id'],
                                'type' => 2)
                            );
                        } else {
                    ...
                    if (!empty($file_array)) {
                        $this->credit_report_model->uploaded_files($file_array);
                    }
                }
    ...
    public function insuff_delete()
    {
        ...
        $insuff_data = $this->input->post('insuff_data');
        if ($this->input->is_ajax_request() && $this->permission['access_credit_list_insuff_delete'] == 1) {
            $fields = array('status' => 3, 'modified_on' => date(DB_DATE_FORMAT), 'modified_by' => $this->user_info['id']);

            $result = $this->credit_report_model->save_update_insuff($fields, array('id' => $insuff_data));
    ...
    public function update_insuff_details()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $result = $this->credit_report_model->save_update_insuff($fields, array('id' => $frm_details['id']));
    ...
    public function componet_html_form($cmp_id)
    {
        if ($this->input->is_ajax_request() && $cmp_id) {
            $details = $this->credit_report->get_all_credit_report_record(array('identity.id' => decrypt($cmp_id)));

            if (!empty($details)) {
                $data['details'] = $details[0];
                echo $this->load->view('admin/credit_report_add_result_model_view_first_qc', $data, true);
            } else {
                echo "<h4>Record Not Found</h4>";
            }
    ...
    public function bulk_upload_credit_report()
    {

        if ($this->input->is_ajax_request()) {
            $file_upload_path = SITE_BASE_PATH . CREDIT_REPORT;
                ...
                if (!empty($excel_data)) {
                    ...
                    foreach ($excel_data as $value) {
                        ...
                        if (!empty($check_record_exits) && $value[0] != "") {
                            ...
                            $closed_date = getWorkingDays(get_date_from_timestamp($value[1]), $get_holiday, $tat_day[0]['tat_cbrver']);

                            $mode_of_verification = $this->credit_report_model->get_mode_of_verification(array('entity' => $check_record_exits['entity'], 'package' => $check_record_exits['package'], 'tbl_clients_id' => $check_record_exits['clientid']));

                            if (!empty($mode_of_verification)) {
                                $mode_of_verification_value = json_decode($mode_of_verification[0]['mode_of_verification']);

                                $mode_of_veri = $mode_of_verification_value->cbrver;
                            } else {
                            ...
                            $insert_id = $this->credit_report_model->save($record);

                            $credit_report_com_ref = $this->credit_report_com_ref($insert_id);

                            $result_get_count = $this->common_model->get_count(array('controllers' => 'assign_credit_report'));

                            $total_get_count = $result_get_count + 1;

                            $result_update_count = $this->common_model->update_count(array('count' => $total_get_count), array('controllers' => 'assign_credit_report'));

                            auto_update_overall_status($check_record_exits['id']);
                            auto_update_tat_status($check_record_exits['id']);

                            $data['success'] = $credit_report_com_ref . " This Component Code Records Created Successfully";
    ...
    {
        $this->load->model(array('credit_report_vendor_log_model'));

        $data['header_title'] = "Vendor Approve List";

        $assigned_option = array(0 => 'select');

        ($this->permission['access_credit_aq_allow'] == 1) ? $assigned_option[1] = 'Assign' : '';
        ($this->permission['access_credit_aq_allow'] == 1) ? $assigned_option[2] = 'Reject' : '';
        ...
        $this->load->view('admin/credit_report_vendor_aq');
    ...
    public function view_approval_queue()
    {
        ...
        $lists = $this->credit_report_vendor_log_model->get_new_list(array('credit_report_vendor_log.status' => 0), $params);

        $totalRecords = count($this->credit_report_vendor_log_model->get_new_list(array('credit_report_vendor_log.status' => 0), $params));

        if ($this->permission['access_credit_aq_view'] == 1) {
            $x = 0;
            ...
            foreach ($lists as $list) {
                ...
                $data_arry[$x]['credit_report_com_ref'] = $list['credit_report_com_ref'];
                ...
                $data_arry[$x]['encry_id'] = ADMIN_SITE_URL . "credit_report/view_details/" . encrypt($list['case_id']);
    ...
    {
        $json_array = array();
        if ($this->input->is_ajax_request() && ($this->permission['access_credit_list_re_assign'] == '1')) {
            $frm_details = $this->input->post();

            if ($frm_details['users_list'] != 0 && $frm_details['cases_id'] != "") {
                $return = $this->common_model->update_in('credit_report', array('has_case_id' => $frm_details['users_list'], 'has_assigned_on' => date(DB_DATE_FORMAT)), array('where_id' => $frm_details['cases_id']));
                if ($return) {
                    $json_array['status'] = SUCCESS_CODE;
    ...
    public function assign_to_vendor()
    {
        ...
        if ($this->input->is_ajax_request() && $this->permission['access_credit_assign_credit_assign'] == true) {
            $frm_details = $this->input->post();
            ...
            if ($frm_details['vendor_list'] != 0 && $frm_details['cases_id'] != "" && !empty($list)) {
                ...
                foreach ($list as $key => $value) {
                    ...
                    $update = $this->credit_report_model->upload_vendor_assign('credit_report', array('vendor_id' => $frm_details['vendor_list'], 'vendor_list_mode' => $frm_details['vendor_list_mode'], 'vendor_assgined_on' => date(DB_DATE_FORMAT)), array('id' => $value, 'vendor_id =' => 0));

                    $update1 = $this->credit_report_model->update_status('credit_report_result', array('verfstatus' => 1), array('credit_report_id' => $value));
                ...
                if (!empty($files)) {
                    $inserted = $this->common_model->common_insert_batch('credit_report_vendor_log', $files);
                }

                if (!empty($inserted)) {

                    $result_get_count_plus = $this->common_model->get_count(array('controllers' => 'credit_report/approval_queue'));

                    $total_get_count_plus = $result_get_count_plus + count($list);

                    $result_update_count_plus = $this->common_model->update_count(array('count' => $total_get_count_plus), array('controllers' => 'credit_report/approval_queue'));

                    $result_get_count_minus = $this->common_model->get_count(array('controllers' => 'assign_credit_report'));

                    $total_get_count_minus = $result_get_count_minus - count($list);

                    $result_update_count_minus = $this->common_model->update_count(array('count' => $total_get_count_minus), array('controllers' => 'assign_credit_report'));
    ...
    public function credit_report_final_assigning()
    {
        $this->load->library('email');

        if ($this->input->is_ajax_request()) {

            if ($this->permission['access_credit_aq_allow'] == 1) {
                $frm_details = $this->input->post();
                ...
                if ($frm_details['action'] == 2 && $frm_details['cases_id'] != "") {
                    ...
                    foreach ($list as $key => $value) {
                        ...
                        $update = $this->credit_report_model->upload_vendor_assign('credit_report_vendor_log', array('status' => 2, 'remarks' => $frm_details['reject_reason'], 'modified_by' => $this->user_info['id'], 'modified_on' => date(DB_DATE_FORMAT)), array('id' => $value));
...
/*
                     ...
                     /*   $f = fsockopen(SMTPHOSTEMAIL, 587);
                        ...
                        if ($f !== false) {*/
                            ...
                            foreach ($list as $key => $value) {
                               ...
                               /* $update = $this->credit_report_model->upload_vendor_assign('credit_report_vendor_log', array('status' => 1, 'approval_by' => $this->user_info['id'], "modified_on" => date(DB_DATE_FORMAT)), array('id' => $value));
                                ...
                                $credit_report_details[] = $this->credit_report_model->get_credit_report_details_for_approval($value);
                            }
                            ...
                            if(isset($credit_report_details) && !empty($credit_report_details))
                            {
                                ...
                                foreach($credit_report_detail as $k => $v) {
                                        ...
                                        if(!empty($result) &&  $result == "Success")
                                        {
                                            if(!empty($credit_report_vendor_log_id))
                                            {
                                                foreach($credit_report_vendor_log_id as $credit_report_vendor_log_key => $credit_report_vendor_log_value)
                                                {
                                                    ...
                                                    $update = $this->credit_report_model->upload_vendor_assign('credit_report_vendor_log', array('status' => 1, 'approval_by' => $this->user_info['id'], "modified_on" => date(DB_DATE_FORMAT)), array('id' => $credit_report_vendor_log_value));
    ...
    public function vendor_logs($id)
    {
        if ($this->input->is_ajax_request() && $id) {
            ...
            $vendor_result = $this->credit_report_model->vendor_logs(array('component_tbl_id' => "10", "component" => "cbrver"), $id);
            ...
            foreach ($vendor_result as $key => $value) {
                ...
                if ($value['status'] != "5") {
                    ...
                    $access_cost = ($this->permission['access_credit_aq_allow'] == 1) ? '#showvendorModel_cost'  : '';
                    $access_cancle = ($this->permission['access_credit_aq_allow'] == 1) ? '#showvendorModel_cancel'  : '';
                      
                    $html_view .= '<td><button data-id=' . $value['id'] . ' data-url ="' . ADMIN_SITE_URL . 'credit_report/View_vendor_log/' . $value['id'] . '" data-toggle="modal" data-target="#showvendorModel"  class="btn-info  showvendorModel"> View </button>';

                    if ($value['final_status'] != "closed") {
                            
                        $html_view .= ' <button data-id="showvendorModel_cost" data-url ="' . ADMIN_SITE_URL . 'credit_report/View_vendor_log_cost/' . $value['id'] . '" data-toggle="modal" data-target="'.$access_cost.'"  class="btn-info  showvendorModel_cost">Charge</button></tb>';

                    }
                    $html_view .= ' <button data-id="showvendorModel_cancel" data-url ="' . ADMIN_SITE_URL . 'credit_report/View_vendor_log_cancel/' . $value['id'] . '" data-toggle="modal" data-target="'.$access_cancle.'"   class="btn-info  showvendorModel_cancel">Cancel</button></tb> ';
                }else {
                    $html_view .= '<td></td>';
                }
                $html_view .= '</tr>';
    ...
    public function View_vendor_log($where_id)
    {
        ...
        $details = $this->credit_report_model->select_vendor_result_log(array('component_tbl_id' => "10", "component" => "cbrver"), $where_id);

        if ($where_id && !empty($details)) {
            $details['attachments'] = $this->credit_report_model->select_file_from_vendor(array('id', 'file_name', 'real_filename'), array('view_venor_master_log_id' => $where_id, 'status' => 1, 'component_tbl_id' => 10));

            $details['attachments_file'] = $this->credit_report_model->select_file(array('id', 'file_name', 'real_filename'), array('credit_report_id' => $details[0]['credit_report_id'], 'status' => 1));
            ...
            echo $this->load->view('admin/credit_report_ads_vendor_view', $details, true);
        } else {
            echo "<h4>Record Not Found</h4>";
        }
    ...
    public function View_vendor_log1($where_id)
    {
        ...
        if ($where_id && !empty($details)) {
            ...
            try {
                ...
                $details['attachments'] = $this->credit_report_model->select_file_from_vendor(array('id', 'file_name', 'real_filename'), array('view_venor_master_log_id' => $where_id, 'status' => 1, 'component_tbl_id' => 10));
                $details['attachments_file'] = $this->credit_report_model->select_file(array('id', 'file_name', 'real_filename'), array('credit_report_id' => $details[0]['credit_report_id'], 'status' => 1,'type' => 0));
                ...
                echo $this->load->view('admin/credit_report_ads_vendor_view_approve_tab', $details, true);

            } catch (Exception $e) {
                log_message('error', 'Credit_report::View_vendor_log1');
                log_message('error', $e->getMessage());
            }
    ...
    public function View_vendor_log_cancel($where_id)
    {
        ...
        $details = $this->credit_report_model->select_vendor_result_log_cost(array('component_tbl_id' => "10", "component" => "cbrver"), $where_id);

        if ($where_id && !empty($details)) {

            $details['details'] = $details[0];

            echo $this->load->view('admin/credit_report_ads_vendor_view_cancel', $details, true);
        } else {
            echo "<h4>Record Not Found</h4>";
        }
    ...
    public function Save_vendor_details()
    {
        if ($this->input->is_ajax_request()) {
            ...
            log_message('error', 'Credit Report Save Vendor  Details');
            try { 
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    $file_upload_path = SITE_BASE_PATH . CREDIT_REPORT . $folder_name;
                    if (!folder_exist($file_upload_path)) {
                        mkdir($file_upload_path, 0777);
                    } else if (!is_writable($file_upload_path)) {
                        array_push($error_msgs, 'Problem while uploading');
                    ...
                    $result = $this->credit_report_model->save_vendor_details("view_vendor_master_log", array_map('strtolower', $field_array), array('trasaction_id' => $transaction_id));

                   

                    $config_array = array('file_upload_path' => $file_upload_path, 'file_permission' => 'jpeg|jpg|png|pdf|tiff', 'file_size' => BULK_UPLOAD_MAX_SIZE_MB * 2000, 'view_venor_master_log_id' => $frm_details['view_vendor_master_log_id'], 'component_tbl_id' => 10, 'status' => 1);
                ...
                log_message('error', 'Credit_report::Save_vendor_details');
                log_message('error', $e->getMessage());
            }
        }
    }
    ...
    public function Save_vendor_details_cancel()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            log_message('error', 'Credit Report Vendor Details Cancel');
            try {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $result = $this->credit_report_model->save_vendor_details_cancel($field_array, array('id' => $frm_details['update_id']));
                ...
                $credit_report_vendor_result = $this->credit_report_model->update_credit_report_vendor_log('credit_report_vendor_log', $field_array_court, array('id' => $frm_details['case_id']));

                if ($result && $credit_report_vendor_result) {
    ...
    public function credit_report_closure_entries()
    {
        if ($this->input->is_ajax_request()) {

            log_message('error', 'Credit Reports Closure Entries');
            try {

                $details['user_list_closed'] = $this->users_list_filter();

                $details['vendor_executive_list'] = $this->credit_report_model->credit_report_case_list(array('view_vendor_master_log.status =' => 1, 'view_vendor_master_log.final_status =' => 'closed', 'component' => 'cbrver'));

            } catch (Exception $e) {
                log_message('error', 'Credit_report::credit_report_closure_entries');
                log_message('error', $e->getMessage());
            }
        ...
        echo $this->load->view('admin/credit_report_vendor_closure_entries', $details, true);
    }

    public function credit_report_closure_entries_vendor_insuff()
    {
        if ($this->input->is_ajax_request()) {

            log_message('error', 'Credit Report vendor insuff');
            try {

                $details['user_list_insuff'] = $this->users_list_filter();

                $details['vendor_executive_list'] = $this->credit_report_model->credit_report_case_list_insuff(array('view_vendor_master_log.status =' => 1, 'view_vendor_master_log.final_status =' => 'insufficiency', 'component' => 'cbrver'));

            } catch (Exception $e) {
                log_message('error', 'Credit_report::credit_report_closure_entries_vendor_insuff');
                log_message('error', $e->getMessage());
            }
        ...
        echo $this->load->view('admin/credit_report_vendor_insuff_entries', $details, true);
    }

    public function check_insuff_already_raised($where_id)
    {

        $credit_report_details = $this->credit_report_model->get_all_credit_report_record(array('credit_report.id' => $where_id));

        $check_insuff_raise = $this->credit_report_model->select_insuff(array('credit_report_id' => $where_id, 'status' => STATUS_ACTIVE, 'insuff_clear_date is null' => null));
        ...
        if (empty($check_insuff_raise)) {
            $data['get_cands_details'] = $this->candidate_entity_pack_details($credit_report_details[0]['cands_id']);
            $data['credit_report_details'] = $credit_report_details[0];

            echo $this->load->view('admin/credit_report_insuff_view', $data, true);
    ...
    public function credit_report_closure()
    {

        if ($this->input->is_ajax_request()) {

            if ($this->permission['access_credit_list_activity'] == 1) {
                $frm_details = $this->input->post();
                $action = $frm_details['action'];
                ...
                if ($frm_details['action'] == 2 && $frm_details['closure_id'] != "") {
                    ...
                    foreach ($list as $key => $value) {
                        ...
                        $update_closure = $this->credit_report_model->update_closure_approval('view_vendor_master_log_closure_status', array('view_vendor_master_log_id' => $value, 'approve_reject_status' => 2, 'approve_reject_by' => $this->user_info['id'], 'reject_reasons' => $frm_details['reject_reason'], "approve_rejected_on" => date(DB_DATE_FORMAT)));

                        $update_closure1 = $this->credit_report_model->update_closure_approval_details('view_vendor_master_log', array('final_status' => 'wip'), array('id' => $value));
                    ...
                    foreach ($list as $key => $value) {
                        ...
                        $update_closure = $this->credit_report_model->update_closure_approval('view_vendor_master_log_closure_status', array('view_vendor_master_log_id' => $value, 'approve_reject_status' => 1, 'approve_reject_by' => $this->user_info['id'], "approve_rejected_on" => date(DB_DATE_FORMAT)));

                        $update_closure1 = $this->credit_report_model->update_closure_approval_details('view_vendor_master_log', array('final_status' => 'approve'), array('id' => $value));

                        //$get_client_id = $this->credit_report_model->get_client_id(array('view_vendor_master_log.final_status' => 'approve', 'view_vendor_master_log.id' => $value));

                       // $insert_task_manager = $this->credit_report_model->update_closure_approval('client_new_cases', array('client_id' => $get_client_id[0]['client_id'], ' total_cases' => 1, 'status' => 'wip', 'created_by' => $this->user_info['id'], 'created_on' => date(DB_DATE_FORMAT), 'type' => 'closures', 'remarks' => 'closures', 'case_type' => 1, 'view_vendor_master_log_id' => $value));
    ...
    public function template_download()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $spreadsheet->getProperties()->setCreator(CRMNAME)
                ...
                ->setTitle(CRMNAME)
                ->setSubject('Credit Report Import Template')
                ->setDescription('Credit Report Import Template File for bulk upload');
            ...
            header("Content-Disposition: attachment;filename=Credit Report Bulk Uplaod Template.xlsx");
            ...
            $json_array['file_name'] = "Credit Report Bulk Uplaod Template";
    ...
    public function export_to_excel()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header("Content-Disposition: attachment;filename=Credit Report Records of $client_name.xlsx");
            header('Cache-Control: max-age=0');
            // If you're serving to IE 9, then the following may be needed
            header('Cache-Control: max-age=1');
            ...
            $json_array['file_name'] = "Credit Report Records of $client_name";
    ...
    public function remove_uploaded_file($id)
    {
        ...
        if ($this->input->is_ajax_request()) {
            $result = $this->credit_report_model->save_update_credit_report_files(array('status' => 2), array('id' => $id));
