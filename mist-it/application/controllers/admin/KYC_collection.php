...
class KYC_collection extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->perm_array = array('page_id' => 18);
        $this->assign_options = array('0' => 'Select', '1' => 'Assign to Executive');
        $this->load->model(array('KYC_collection_model'));
    }

    public function index()
    {
        $data['header_title'] = "KYC Collection Lists";

        $data['filter_view'] = $this->filter_view();

        $data['vendor_list'] = $this->vendor_list('KYC_collection');

        $this->load->view('admin/header', $data);

        $this->load->view('admin/kyc_collection_list');

        $this->load->view('admin/footer');
    }

    public function kyc_collection_view_datatable()
    {
        $params = $records = $data_arry = $columns = array();

        $params = $_REQUEST;

        $columns = array('kyc_collection.id', 'ClientRefNumber', 'KYCRefNumber', 'company_customer_name', 'iniated_date', 'last_modified', 'encry_id', 'verfstatus', 'street_address', 'closuredate', 'remarks');

        $records = $this->KYC_collection_model->get_all_kyc_record_datatable(false, $params, $columns);

        $totalRecords = count($this->KYC_collection_model->get_all_kyc_record_datatable_count(false, $params, $columns));

        $x = 0;
        foreach ($records as $record) {
            $data_arry[$x]['id'] = $x + 1;
            $data_arry[$x]['checkbox'] = $record['kyc_collection_id'];
            $data_arry[$x]['ClientRefNumber'] = $record['ClientRefNumber'];
            $data_arry[$x]['kyc_ref_no'] = $record['kyc_ref_no'];
            ...
            $data_arry[$x]['encry_id'] = ADMIN_SITE_URL . "KYC_collection/view_details/" . encrypt($record['kyc_collection_id']);
            $x++;
        }
    ...
    public function add()
    {
        ...
        $data['KYCRefNumber'] = $this->KYCRefNumber();

        $data['assigned_user_id'] = $this->users_list();

        $this->load->view('admin/header', $data);

        $this->load->view('admin/kyc_collection_add', $data);

        $this->load->view('admin/footer');
    }

    protected function KYCRefNumber()
    {
        $lists = $this->KYC_collection_model->KYC_ref_no();

        $number = (!empty($lists)) ? $lists['A_I'] + 1 : 1000;

        return COMPONENT_REF_NO['KYC'] . $number;
    }
    ...
    public function save_form()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->form_validation->set_rules('KYCRefNumber', 'KYC Ref Number', 'required');
            ...
            if ($this->form_validation->run() == false) {
                ...
                $field_array = array('clientid' => $frm_details['clientid'],
                    ...
                    'kyc_ref_no' => $frm_details['KYCRefNumber'],
                ...
                $result = $this->KYC_collection_model->save(array_map('strtolower', $field_array));

                $config_array = array('file_upload_path' => $file_upload_path, 'file_permission' => 'jpeg|jpg|png|pdf|docx|xls|xlsx', 'file_size' => BULK_UPLOAD_MAX_SIZE_MB * 1000, 'file_id' => $result, 'component_name' => 'kyc_id');
                if (empty($error_msgs)) {
                    if ($_FILES['attchments']['name'][0] != '') {
                        ...
                        if (!empty($retunr_de)) {
                            $this->common_model->common_insert_batch('kyc_files', $retunr_de['success']);
                        }
                    }
                }
                ...
                if ($result) {
                    ...
                    $json_array['redirect'] = ADMIN_SITE_URL . 'KYC_collection';
                } else {
                    $json_array['status'] = ERROR_CODE;
    ...
    {
        if (!empty($kyc_id)) {
            $details = $this->KYC_collection_model->select(true, array('*'), array('id' => decrypt($kyc_id)));

            if ($kyc_id && !empty($details)) {
                $data['header_title'] = 'KYC Verification';
                ...
                $check_insuff_raise = $this->KYC_collection_model->select_insuff(array('kyc_collection_id' => decrypt($kyc_id), 'status' => STATUS_ACTIVE, 'insuff_clear_date is null' => null));
                ...
                $this->load->view('admin/kyc_edit_view');
    ...
    {
        $this->load->model(array('KYC_vendor_log_model'));

        $data['header_title'] = "Vendor Approve List";

        $data['lists'] = $this->KYC_vendor_log_model->get_new_list(array('kyc_vendor_log.status' => 0));

        $this->load->view('admin/header', $data);

        $this->load->view('admin/kyc_vendor_approve_queue');
