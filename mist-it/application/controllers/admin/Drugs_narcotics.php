...
class Drugs_narcotics extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->perm_array = array('page_id' => 8);
        if ($this->input->is_ajax_request()) {
    ...
    public function drug_view_datatable()
    {
        ...
        try {
            ...
            $details = $this->drug_verificatiion_model->get_all_drug_record_datatable($params, $columns);

            $totalRecords = count($this->drug_verificatiion_model->get_all_drug_record_datatable_count($params, $columns));
    ...
    public function get_clients_for_drugs_list_view()
    {
        log_message('error', 'For Drugs list view get client');
        try {
            $params = $_REQUEST;

            $clients = $this->drug_verificatiion_model->select_client_list_view_drugs('clients', false, array('clients.id', 'clients.clientname'), $params);
            ...
            log_message('error', 'Drugs_narcotics::get_clients_for_drugs_list_view');
            log_message('error', $e->getMessage());
        }
    }

    public function get_assign_drugs_list_view()
    {
        log_message('error', 'Drugs List Assign in Assign View');
        try {

            $data['header_title'] = "Drugs Verification";

            $data['filter_view'] = $this->filter_view_assign_drugs_list();
            $data['users_list'] = $this->users_list();
            $data['vendor_list'] = $this->vendor_list('narcver');

        } catch (Exception $e) {
            log_message('error', 'Drugs_narcotics::get_assign_drugs_list_view');
            log_message('error', $e->getMessage());
        }

        echo $this->load->view('admin/assign_drugs', $data, true);
    }

    protected function filter_view_assign_drugs_list($true = false)
    {
        if ($true) {
        ...
        $data['clients'] = $this->get_clients_for_list_view_drugs();
    ...
    public function get_clients_for_list_view_drugs()
    {
        $this->load->model('assign_drug_verificatiion_model');
        $clients = $this->assign_drug_verificatiion_model->select_client_list_assign_add_view('clients', false, array('clients.id', 'clients.clientname'));
    ...
    public function add($candsid)
    {
        if ($this->input->is_ajax_request()) {
            ...
            try {
                ...
                $data['scope_of_work'] = $this->drug_verificatiion_model->get_scope_of_work(array('entity' => $data['get_cands_details']['entity_id'], 'package' => $data['get_cands_details']['package_id'], 'tbl_clients_id' => $data['get_cands_details']['clientid']));
            

                $data['states'] = $this->get_states();
            
                //$data['assigned_user_id'] = $this->drug_verificatiion_model->get_assign_users_id('user_profile', false, array("`user_profile`.`status`,`user_profile`.`id`,`user_profile`.`email`,`user_profile`.`department`,`user_profile`.`user_name`,concat(`user_profile`.`firstname`,' ',`user_profile`.`lastname`) as fullname,`user_profile`.`profile_pic`,`user_profile`.`firstname`,`user_profile`.`lastname`,`roles`.`role_name`,`roles`.`groups_id`,`roles_permissions`.*"), array('user_profile.status' => STATUS_ACTIVE));

                $data['assigned_user_id']  = $this->get_reporting_manager_for_execituve($data['get_cands_details']['clientid']);

                echo $this->load->view('admin/drug_add', $data, true);

            } catch (Exception $e) {
                log_message('error', 'Drugs_narcotics::add');
                log_message('error', $e->getMessage());
            }
        }else {
    ...
    {
        log_message('error', 'Drugs Ref No Creation');
        try {
            ...
            log_message('error', 'Drugs_narcotics::drug_com_ref');
            log_message('error', $e->getMessage());
        }
    }
    ...
    public function save_form()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->form_validation->set_rules('drug_test_code', 'drug_test_code', 'required');
            ...
            if ($this->form_validation->run() == false) {
                ...
                log_message('error', 'New Drugs Creation');
                try {
                    ...
                    $field_array = array('clientid' => $frm_details['clientid'],
                        ...
                        "appointment_time" => date("H:i", strtotime($frm_details['appointment_time'])),
                    ...
                    $drug_com_ref = $this->drug_com_ref($result);
                    ...
                    if (empty($error_msgs)) {
                        ...
                        if ($_FILES['attchments_cs']['name'][0] != '') {
                            ...
                            if (!empty($retunr_cd)) {
                                $this->common_model->common_insert_batch('drug_narcotis_files', $retunr_cd['success']);
                            }
                        }
                    }
                    ...
                    if ($result) {
                        ...
                        $user_activity_data = $this->common_model->user_actity_data(array('component' => "Drugs Verification", 'ref_no' => $drug_com_ref, 'candidate_name' => $frm_details['CandidateName'], 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id'], 'activity_type' => 'Manual', 'action' => 'Add'));

                        auto_update_overall_status($frm_details['candsid']);
                        auto_update_tat_status($frm_details['candsid']);

                                                       
                        $vendor_id_panel = $this->drug_verificatiion_model->get_vendor_id_form_panel($frm_details['drug_test_code']); 

                        if(!empty($vendor_id_panel))
                        {
                            $vendor_id = $vendor_id_panel[0]['id'];
                        }
                        else{
                        ...
                        { 
                            ...
                            $update = $this->drug_verificatiion_model->save_narcotics('drug_narcotis', array('vendor_id' => $vendor_id, 'vendor_list_mode' => 'physical visit', 'vendor_assgined_on' => date(DB_DATE_FORMAT)), array('id' => $result, 'vendor_id =' => 0));

                            $update1 = $this->drug_verificatiion_model->save_narcotics('drug_narcotis_result', array('verfstatus' => 1), array('drug_narcotis_id' => $result));
                                ...
                                if ($update) {
                                    ...
                                    $save_vendor_log = $this->drug_verificatiion_model->save_narcotics('drug_narcotis_vendor_log',$fiels);
                        ...
                        $json_array['redirect'] = ADMIN_SITE_URL . 'drugs_narcotics';
                    ...
                    log_message('error', 'Drugs_narcotics::save_form');
                    log_message('error', $e->getMessage());
                }    
            }
    ...
    public function view_from_inside($add_comp_ids = '')
    {
        if ($this->input->is_ajax_request()) {
            ...
            $cget_details = $this->drug_verificatiion_model->get_all_drug_record(array('drug_narcotis.drug_com_ref' => $add_comp_ids));

            if (!empty($cget_details)) {

                log_message('error', 'Drugs Add From Candidate');
                try {
                        $data['header_title'] = 'Edit Drugs';
                        ...
                        $check_insuff_raise = $this->drug_verificatiion_model->select_insuff(array('drug_narcotis_id' => $cget_details[0]['drug_narcotis_id'], 'status' => STATUS_ACTIVE, 'insuff_clear_date is null' => null));

                        $data['insuff_reason_list'] = $this->drug_verificatiion_model->insuff_reason_list(false, array('component_id' => 8));
                        ...
                        $data['attachments'] = $this->drug_verificatiion_model->select_file(array('id', 'file_name', 'real_filename', 'type'), array('drug_narcotis_id' => $cget_details[0]['drug_narcotis_id'], 'status' => 1));
                        ...
                        echo $this->load->view('admin/drug_edit_view_inside', $data, true);
    ...
    {
        if ($this->input->is_ajax_request()) {
            $this->form_validation->set_rules('drug_narcotis_id', 'ID', 'required');

            $this->form_validation->set_rules('has_case_id', 'Assigned To Executive', 'required');

            $this->form_validation->set_rules('clientid', 'Client', 'required');

            $this->form_validation->set_rules('drug_com_ref', 'Component Ref', 'required');
            ...
            if ($this->form_validation->run() == false) {
                ...
                $field_array = array('clientid' => $frm_details['clientid'],
                    'candsid' => $frm_details['candsid'],
                    'drug_com_ref' => $frm_details['drug_com_ref'],
                    'iniated_date' => convert_display_to_db_date($frm_details['iniated_date']),
                    "appointment_date" => convert_display_to_db_date($frm_details['appointment_date']),
                    "appointment_time" => date("H:i", strtotime($frm_details['appointment_time'])),
                    // "spoc_no" => $frm_details['spoc_no'],
                    "drug_test_code" => $frm_details['drug_test_code'],
                    'street_address' => $frm_details['street_address'],
                    'city' => $frm_details['city'],
                    'pincode' => $frm_details['pincode'],
                ...
                $result = $this->drug_verificatiion_model->save(array_map('strtolower', $field_array), array('id' => $frm_details['drug_narcotis_id']));
                ...
                $result_candidate_billed = $this->common_model->update_candidate_billed_date(array('build_date' => $this->components_key_val(array('0' => $component_name['addrver'], '1' => $component_name['courtver'], '2' => $component_name['globdbver'], '3' => $frm_details['build_date'], '4' => $component_name['refver'], '5' => $component_name['empver'], '6' => $component_name['eduver'], '7' => $component_name['identity'], '8' => $component_name['cbrver'], '9' => $component_name['crimver']))), array('id' => $frm_details['candsid']));
                ...
                $config_array = array('file_upload_path' => $file_upload_path, 'file_permission' => 'jpeg|jpg|png|pdf|docx|xls|xlsx', 'file_size' => BULK_UPLOAD_MAX_SIZE_MB * 1000, 'file_id' => $frm_details['drug_narcotis_id'], 'component_name' => 'drug_narcotis_id');
                if (empty($error_msgs)) {
                    if ($_FILES['attchments']['name'][0] != '') {
                        $config_array['files_count'] = count($_FILES['attchments']['name']);
                        $config_array['file_data'] = $_FILES['attchments'];
                ...
                if ($result) {
                    ...
                    $user_activity_data = $this->common_model->user_actity_data(array('component' => "Drugs Verification",
                        'ref_no' => $frm_details['drug_com_ref'], 'candidate_name' => $frm_details['CandidateName'], 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id'], 'activity_type' => 'Manual', 'action' => 'Edit'));
    ...
    {
        if (!empty($pcc_id)) {
            $details = $this->drug_verificatiion_model->get_all_drug_record(array('drug_narcotis.id' => decrypt($pcc_id)));

            if ($pcc_id && !empty($details)) {

                log_message('error', 'Drugs Edit View');
                try {
                    $data['header_title'] = 'Drugs Verification';
                    ...
                    $check_insuff_raise = $this->drug_verificatiion_model->select_insuff(array('drug_narcotis_id' => decrypt($pcc_id), 'status' => STATUS_ACTIVE, 'insuff_clear_date is null' => null));

                    $data['attachments'] = $this->drug_verificatiion_model->select_file(array('id', 'file_name', 'real_filename', 'type'), array('drug_narcotis_id' => decrypt($pcc_id), 'status' => 1));
                    ...
                    $this->load->view('admin/drug_edit_view');

                    $this->load->view('admin/footer');

                } catch (Exception $e) {
                    log_message('error', 'Drugs_narcotics::view_details');
                    log_message('error', $e->getMessage());
                }
            } else {
                show_404();
    ...
    {
        $details = $this->drug_verificatiion_model->get_all_drug_record(array('drug_narcotis.id' => $where_id));

        if ($where_id && !empty($details)) {

            log_message('error', 'Drugs Add Reult Model');
            try {
            $data['details'] = $details[0];

            $data['attachments'] = $this->drug_verificatiion_model->select_file(array('id', 'file_name', 'status'), array('drug_narcotis_id' => $where_id, 'type' => 1));

            $data['vendor_attachments'] = $this->drug_verificatiion_model->select_file_vendor(array('view_vendor_master_log_file.id', 'view_vendor_master_log_file.file_name', 'view_vendor_master_log_file.real_filename', 'view_vendor_master_log_file.status'), array('view_vendor_master_log_file.component_tbl_id' => 7), $where_id);

            $data['mode_of_verification'] = $this->drug_verificatiion_model->get_mode_of_verification(array('entity' => $details[0]['entity'], 'package' => $details[0]['package'], 'tbl_clients_id' => $details[0]['clientid']));

            $data['url'] = $url;

            echo $this->load->view('admin/drug_add_result_model_view', $data, true);

            } catch (Exception $e) {
                log_message('error', 'Drugs_narcotics::add_result_model');
                log_message('error', $e->getMessage());
            }
        } else {
    ...
    public function add_verificarion_result()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->form_validation->set_rules('drug_narcotis_result_id', 'Id', 'required');
            ...
            if ($this->form_validation->run() == false) {
                ...
                try {
                    ...
                    if (($frm_details['action_val'] != "Select") || ($frm_details['activity_last_id'] != "")) {
                        ...
                        $drug_narcotis_result_id = $frm_details['drug_narcotis_result_id'];
                        ...
                        $field_array = array(
                            ...
                            'drug_narcotis_id' => $frm_details['drug_narcotis_id'],
                            ...
                            'amphetamine_screen' => $frm_details['amphetamine_screen'],
                            'cannabinoids_screen' => $frm_details['cannabinoids_screen'],
                            'cocaine_screen' => $frm_details['cocaine_screen'],
                            'opiates_screen' => $frm_details['opiates_screen'],
                            'phencyclidine_screen' => $frm_details['phencyclidine_screen']
                        );
                       
                        if (isset($frm_details['remove_file'])) // delete uploaded file
                        {
                            $this->drug_verificatiion_model->delete_uploaded_file($frm_details['remove_file']);
                        }
                        if (isset($frm_details['add_file'])) // delete uploaded file
                        {
                            $this->drug_verificatiion_model->add_uploaded_file($frm_details['add_file']);
                        }

                        if (isset($frm_details['vendor_remove_file'])) // delete uploaded file
                        {
                            $this->drug_verificatiion_model->vendor_delete_uploaded_file($frm_details['vendor_remove_file']);
                        }
                        if (isset($frm_details['vendor_add_file'])) // delete uploaded file
                        {
                            $this->drug_verificatiion_model->vendor_add_uploaded_file($frm_details['vendor_add_file']);
                        }

                        $result = $this->drug_verificatiion_model->save_update_result(array_map('strtolower', $field_array), array('id' => $drug_narcotis_result_id));

                        $result_drugs = $this->drug_verificatiion_model->save_update_result_drugs(array_map('strtolower', $field_array));
                        ...
                        $config_array = array(
                            ...
                            'file_id'           => $frm_details['drug_narcotis_id'], 
                            'component_name'    => 'drug_narcotis_id'
                        );
                    ...
                    log_message('error', 'Drugs_narcotics::add_verificarion_result');
                    log_message('error', $e->getMessage());
                }
            }
            echo_json($json_array);
    ...
    public function add_verificarion_ver_result()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                log_message('error', 'Drugs verification reverification result');
                try {
                    ...
                    $result_addrverres_result = $this->drug_verificatiion_model->save_update_result_drugs(array_map('strtolower', $field_array), array('id' => $frm_details['result_update_id']));
                    ...
                    log_message('error', 'Drugs_narcotics::add_verificarion_ver_result');
                    log_message('error', $e->getMessage());
                }
            }
            echo_json($json_array);
    ...
    public function drugs_narcotics_result_list($emp_id = '')
    {

        if ($emp_id && $this->input->is_ajax_request()) {

            log_message('error', 'Drugs Result List View');
            try {
                $drugs_narcotics_result = $this->drug_verificatiion_model->select_result_log(array('drug_narcotis_id' => $emp_id, 'activity_log_id !=' => null));
                ...
                if (!empty($drugs_narcotics_result[0]['id'])) {
                    $l = 1;

                foreach ($drugs_narcotics_result as $key => $value) {


                    $vendor_attachments = $this->drug_verificatiion_model->select_file_vendor(array('view_vendor_master_log_file.id', 'view_vendor_master_log_file.file_name', 'view_vendor_master_log_file.real_filename', 'view_vendor_master_log_file.status'), array('view_vendor_master_log_file.component_tbl_id' => 7), $value['drug_narcotis_id']);
                    ...
                    if ($value['file_names']) {
                        ...
                        for ($i = 0; $i < count($files); $i++) {
                            $url = "'" . SITE_URL . DRUGS . $value['clientid'] . '/';
                            $actual_file = $files[$i] . "'";
                            $myWin = "'" . "myWin" . "'";
                    ...
                    if ($l == 1) {
                        ...
                        $html_view .= '<td><button data-id="showAddResultModel" data-url ="' . ADMIN_SITE_URL . 'drugs_narcotics/drugs_narcotics_result_list_idwise/' .$value['id'] . '/' . str_replace(" ", "", $value['activity_action']) . '" data-toggle="modal" data-target="#showAddResultModel1"  class="btn-info  showAddResultModel"> View </button></td>';
            ...
            $json_array['message'] = $html_view;
                ...
                log_message('error', 'Drugs_narcotics::drugs_narcotics_result_list');
                log_message('error', $e->getMessage());
            }
        } else {
            $json_array['status'] = SUCCESS_CODE;
    ...
    public function drugs_narcotics_result_list_idwise($where_id,$url)
    {

        $details = $this->drug_verificatiion_model->select_result_log1(array('drug_narcotis_ver_result.id' => $where_id));

        if ($where_id && !empty($details)) {

            log_message('error', 'Drugs Result Open Idwise');
            try {
                ...
                $details['attachments'] = $this->drug_verificatiion_model->select_file(array('id', 'file_name', 'status'), array('drug_narcotis_id' => $details[0]['drug_narcotis_id'], 'type' => 1));

                $details['vendor_attachments'] = $this->drug_verificatiion_model->select_file_vendor(array('view_vendor_master_log_file.id', 'view_vendor_master_log_file.file_name', 'view_vendor_master_log_file.real_filename', 'view_vendor_master_log_file.status'), array('view_vendor_master_log_file.component_tbl_id' => 7), $details[0]['drug_narcotis_id']);


                echo $this->load->view('admin/drug_add_result_model_view_log', $details, true);

            } catch (Exception $e) {
                log_message('error', 'Drugs_narcotics::drugs_result_list_idwise');
                log_message('error', $e->getMessage());
            }
        } else {
            echo "<h4>Record Not Found</h4>";
    ...
    public function approve_first_qc()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $result = $this->drug_verificatiion_model->save_first_qc_result(array("first_qc_approve" => "First QC Approve", "first_qc_updated_on" => $accepted_on, "first_qu_updated_by" => $this->user_info['id']), array("drug_narcotis_id" => $comp_id, "id" => $frist_qc_id, "candsid" => $cands_id));
    ...
    public function assign_to_executive()
    {
        ...
        if ($this->input->is_ajax_request() && ($this->permission['access_drugs_list_re_assign'] == '1')) {
            $frm_details = $this->input->post();

            if ($frm_details['users_list'] != 0 && $frm_details['cases_id'] != "") {
                $return = $this->common_model->update_in('drug_narcotis', array('has_case_id' => $frm_details['users_list'], 'has_assigned_on' => date(DB_DATE_FORMAT)), array('where_id' => $frm_details['cases_id']));
                if ($return) {
                    $json_array['status'] = SUCCESS_CODE;
    ...
    public function drugs_narcotics_reinitiated_date()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $drugs_narcotics_id = $this->input->post('update_id');
            $reinitiated_date = $this->input->post('reinitiated_date');
            $reinitiated_remark = $this->input->post('reinitiated_remark');
            $clientid = $this->input->post('clientid');

            $check = $this->drug_verificatiion_model->select_reinitiated_date(array('id' => $drugs_narcotics_id));

            if ($check[0]['drug_re_open_date'] == "0000-00-00" || $check[0]['drug_re_open_date'] == "") {
                $reinitiated_dates = $reinitiated_date;
            } else {
                $reinitiated_dates = $check[0]['drug_re_open_date'] . "||" . $reinitiated_date;
            }

            $result = $this->drug_verificatiion_model->save_update_initiated_date(array('drug_re_open_date' => $reinitiated_dates, 'drug_reinitiated_remark' => $reinitiated_remark), array('id' => $drugs_narcotics_id));

            $result_drugs = $this->drug_verificatiion_model->save_update_initiated_date_drugs(array('verfstatus' => 26, 'var_filter_status' => "WIP", 'var_report_status' => "WIP", 'first_qc_approve' => "", 'first_qc_updated_on' => "", 'first_qu_reject_reason' => "", 'first_qu_updated_by' => ""), array('drug_narcotis_id' => $drugs_narcotics_id));

            $file_upload_path = SITE_BASE_PATH . DRUGS . $clientid;
            if (!folder_exist($file_upload_path)) {
                mkdir($file_upload_path, 0777);
            } else if (!is_writable($file_upload_path)) {
                array_push($error_msgs, 'Problem while uploading');
            ...
            $config_array = array('file_upload_path' => $file_upload_path, 'file_permission' => 'jpeg|jpg|png|pdf|docx|xls|xlsx', 'file_size' => BULK_UPLOAD_MAX_SIZE_MB * 1000, 'file_id' => $drugs_narcotics_id, 'component_name' => 'drug_narcotis_id');
            ...
            $result_drugs_activity_data = $this->drug_verificatiion_model->initiated_date_drugs_activity_data(array('candsid' => $this->input->post('candidates_info_id'), 'comp_table_id' => $drugs_narcotics_id, 'action' => "Re-Initiated", '  activity_status' => "Re-Initiated", 'remarks' => 'Client requested to re-verify the case [' . $reinitiated_remark . ']', 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id']));

            if ($result && $result_drugs) {
    ...
    {
        $this->load->model(array('drug_vendor_log_model'));

        $assigned_option = array(0 => 'select');

        ($this->permission['access_drugs_aq_allow'] == 1) ? $assigned_option[1] = 'Assign' : '';
        ($this->permission['access_drugs_aq_allow'] == 1) ? $assigned_option[2] = 'Reject' : '';
        $data['assigned_option'] = $assigned_option;
        $data['header_title'] = "Vendor Approve List";

        // $data['lists']  = $this->drug_vendor_log_model->get_new_list(array('drug_narcotis_vendor_log.status' => 0));

        $this->load->view('admin/header', $data);

        $this->load->view('admin/drug_vendor_aq');
    ...
    public function view_approval_queue()
    {
        ...
        $lists = $this->drug_vendor_log_model->get_new_list(array('drug_narcotis_vendor_log.status' => 0));

        $totalRecords = count($this->drug_vendor_log_model->get_new_list(array('drug_narcotis_vendor_log.status' => 0)));

        if ($this->permission['access_drugs_aq_view'] == 1) {
            $x = 0;
            ...
            foreach ($lists as $list) {
                ...
                $data_arry[$x]['drug_test_code'] = $list['drug_test_code'];
                $data_arry[$x]['city'] = $list['city'];
                $data_arry[$x]['encry_id'] = ADMIN_SITE_URL . "drugs_narcotics/view_details/" . encrypt($list['case_id']);
    ...
    public function vendor_logs($id)
    {
        if ($this->input->is_ajax_request() && $id) {
            ...
            $vendor_result = $this->drug_verificatiion_model->vendor_logs(array('component_tbl_id' => "7", "component" => "narcver"), $id);
            $counter = 1;

            $html_view = '<thead><tr><th>Sr No</th><th>Trasaction Id</th><th>Vendor Name</th><th>Approved By</th><th>Approved Date</th><th>File Name</th><th>Closure Date</th><th>Status</th><th>Action</th><th>Remarks</th></tr></thead>';
            foreach ($vendor_result as $key => $value) {

                $vendor_attachments = $this->drug_verificatiion_model->select_file_vendor(array('view_vendor_master_log_file.id', 'view_vendor_master_log_file.file_name', 'view_vendor_master_log_file.real_filename', 'view_vendor_master_log_file.status'), array('view_vendor_master_log_file.component_tbl_id' => 7), $value['case_id']); 
                $file = "&nbsp;&nbsp;&nbsp;&nbsp;";
                ...
                if($value['status'] != "5")
                {
                    $access_cost = ($this->permission['access_drugs_aq_allow'] == 1) ? '#showvendorModel_cost'  : '';
                    $access_cancle = ($this->permission['access_drugs_aq_allow'] == 1) ? '#showvendorModel_cancel'  : '';

                    $html_view .= '<td><button data-id='.$value['id'].' data-url ="'.ADMIN_SITE_URL.'Drugs_narcotics/View_vendor_log/'.$value['id'].'" data-toggle="modal" data-target="#showvendorModel"  class="btn-info  showvendorModel"> View </button>';
               
                    if($value['final_status'] != "closed")
                    {
                    $html_view .= ' <button data-id="showvendorModel_cost" data-url ="'.ADMIN_SITE_URL.'address/View_vendor_log_cost/'.$value['id'].'" data-toggle="modal" data-target="'.$access_cost.'"  class="btn-info  showvendorModel_cost">Charge</button></tb> ';

                    }
                    if ($value['final_status'] == "wip") {

                   
                    $html_view .= ' <button data-id="showvendorModel_cancel" data-url ="'.ADMIN_SITE_URL.'Drugs_narcotics/View_vendor_log_cancel/'.$value['id'].'" data-toggle="modal" data-target="'.$access_cancle.'"   class="btn-info  showvendorModel_cancel">Cancel</button>';
                    }
                    $html_view .= '</td>'; 
    ...
    public function assign_to_vendor()
    {
        ...
        if ($this->input->is_ajax_request() && $this->permission['access_drugs_assign_drugs_assign'] == true) {
            $frm_details = $this->input->post();
            ...
            if ($frm_details['vendor_list'] != 0 && $frm_details['cases_id'] != "" && !empty($list)) {
                ...
                foreach ($list as $key => $value) {
                    ...
                    $update = $this->common_model->update_batch_vendor_assign('drug_narcotis', array('vendor_id' => $frm_details['vendor_list'], 'vendor_list_mode' => $frm_details['vendor_list_mode'], 'vendor_assgined_on' => date(DB_DATE_FORMAT)), array('id' => $value, 'vendor_id =' => 0));

                    $update1 = $this->common_model->update_status('drug_narcotis_result', array('verfstatus' => 1), array('   drug_narcotis_id' => $value));
                ...
                if (!empty($files)) {
                    $inserted = $this->common_model->common_insert_batch('drug_narcotis_vendor_log', $files);
                }

                if (!empty($inserted)) {

                    $result_get_count_plus = $this->common_model->get_count(array('controllers' => 'drugs_narcotics/approval_queue'));

                    $total_get_count_plus = $result_get_count_plus + count($list);

                    $result_update_count_plus = $this->common_model->update_count(array('count' => $total_get_count_plus), array('controllers' => 'drugs_narcotics/approval_queue'));

                    $result_get_count_minus = $this->common_model->get_count(array('controllers' => 'assign_drugs'));

                    $total_get_count_minus = $result_get_count_minus - count($list);

                    $result_update_count_minus = $this->common_model->update_count(array('count' => $total_get_count_minus), array('controllers' => 'assign_drugs'));
    ...
    public function drug_final_assigning()
    {
        $json_array = array();

        if ($this->input->is_ajax_request()) {

            if ($this->permission['access_drugs_aq_allow'] == 1) {
                ...
                if ($frm_details['action'] == 2 && $frm_details['cases_id'] != "") {
                    ...
                    foreach ($list as $key => $value) {
                        ...
                        $update = $this->drug_verificatiion_model->upload_vendor_assign('drug_narcotis_vendor_log', array('status' => 2, 'remarks' => $frm_details['reject_reason'], 'modified_by' => $this->user_info['id'], 'modified_on' => date(DB_DATE_FORMAT)), array('id' => $value));

                        if ($update) {
                            $update_counter++;
                            //update trigger is set
                        }
                    }
                    ...
                    foreach ($list as $key => $value) {
                        ...
                        $drugs_details[] = $this->drug_verificatiion_model->get_drugs_details_for_approval($value);
                    }
                                ...
                                $result = $this->email->address_approve_vendor_cases_mail_send($email_tmpl_data);
                                    ...
                                    if(!empty($result) &&  $result == "Success")
                                    {
                                        if(!empty($drug_narcotis_vendor_log_id))
                                        {
                                            ...
                                            {
                                                $update = $this->drug_verificatiion_model->upload_vendor_assign('drug_narcotis_vendor_log', array('status' => 1, 'approval_by' => $this->user_info['id'], "modified_on" => date(DB_DATE_FORMAT)), array('id' => $drug_narcotis_vendor_log_value));
    ...
    public function insuff_raised()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            log_message('error', 'Drugs Add Raised Insuff');
            try {
                $drug_narcotis_id = $this->input->post('update_id');
                ...
                $check = $this->drug_verificatiion_model->select_insuff(array('drug_narcotis_id' => $drug_narcotis_id, 'status !=' => 3, 'insuff_clear_date is null' => null));

                if (empty($check)) {
                    $result = $this->drug_verificatiion_model->save_update_insuff(array('insuff_raised_date' => convert_display_to_db_date($insff_date), 'insuff_raise_remark' => $this->input->post('insuff_raise_remark'), 'status' => STATUS_ACTIVE, 'insff_reason' => $insff_reason, 'created_by' => $this->user_info['id'], 'drug_narcotis_id' => $drug_narcotis_id, 'auto_stamp' => 1));

                    if ($result) {
                        $user_activity_data = $this->common_model->user_actity_data(array('component' => "Drugs Verification", 'ref_no' => $ref_no, 'candidate_name' => $CandidateName, 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id'], 'activity_type' => 'Manual', 'action' => 'Insuff Raised'));
                    }
                ...
                log_message('error', 'Drugs_narcotics::insuff_raised');
                log_message('error', $e->getMessage());
            }
        }
        echo_json($json_array);
    ...
    public function insuff_tab_view($emp_id = '')
    {
        if ($this->input->is_ajax_request() && $emp_id) {
            $data['insuff_details'] = $this->drug_verificatiion_model->select_insuff_join(array('drug_narcotis_id' => $emp_id));

            echo $this->load->view('admin/drug_insuff_view', $data, true);
        } else {
    ...
    public function update_insuff_details()
    {
        if ($this->input->is_ajax_request()) {
            ...
            log_message('error', 'Insuff Details');
            try {
                ...
                $result = $this->drug_verificatiion_model->save_update_insuff($fields, array('id' => $frm_details['id']));
                ...
                log_message('error', 'Drugs_narcotics::update_insuff_details');
                log_message('error', $e->getMessage());
            }
            echo_json($json_array);
        }
    ...
    public function insuff_clear()
    {
        ...
        if ($this->input->is_ajax_request() && $this->permission['access_drugs_list_insuff_clear'] == 1) {

            log_message('error', 'Drugs Insuff Clear');
            try {
                ...
                if (convert_display_to_db_date($frm_details['insuff_clear_date']) >= convert_display_to_db_date($frm_details['check_insuff_raise'])) {
                    ...
                    if ($result) {
                        $user_activity_data = $this->common_model->user_actity_data(array('component' => "Drugs Verification", 'ref_no' => $frm_details['component_ref_no'], 'candidate_name' => $frm_details['CandidateName'], 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id'], 'activity_type' => 'Manual', 'action' => 'Insuff Cleared'));
                    }
                ...
                log_message('error', 'Drugs_narcotics::insuff_clear');
                log_message('error', $e->getMessage());
            }    
        } else {
    ...
    public function insuff_delete()
    {
        ...
        $insuff_data = $this->input->post('insuff_data');
        if ($this->input->is_ajax_request() && $this->permission['access_drugs_list_insuff_delete'] == 1) {

            log_message('error', 'Drugs Insuff Delete');
            try {
                
                $fields = array('status' => 3, 'modified_on' => date(DB_DATE_FORMAT), 'modified_by' => $this->user_info['id']);

                $result = $this->drug_verificatiion_model->save_update_insuff($fields, array('id' => $insuff_data));
                ...
                log_message('error', 'Drugs_narcotics::insuff_delete');
                log_message('error', $e->getMessage());
            }
        } else {
            $json_array['message'] = "Don't have permission to access this function";
    ...
    public function componet_html_form($cmp_id)
    {
        if ($this->input->is_ajax_request() && $cmp_id) {
            $details = $this->drug_verificatiion_model->get_all_drug_record(array('drug_narcotis.id' => decrypt($cmp_id)));

            if (!empty($details)) {
                $data['details'] = $details[0];
                echo $this->load->view('admin/drug_add_result_model_view_first_qc', $data, true);
            } else {
                echo "<h4>Record Not Found</h4>";
            }
    ...
    public function bulk_upload_drug()
    {

        if ($this->input->is_ajax_request()) {
            $file_upload_path = SITE_BASE_PATH . DRUGS;
                ...
                if (!empty($excel_data)) {
                    ...
                    foreach ($excel_data as $value) {
                        ...
                        if (!empty($check_record_exits) && $value[0] != "") {
                            ...
                            $mode_of_verification = $this->drug_verificatiion_model->get_mode_of_verification(array('entity' => $check_record_exits['entity'], 'package' => $check_record_exits['package'], 'tbl_clients_id' => $check_record_exits['clientid']));

                            if (!empty($mode_of_verification)) {
                                $mode_of_verification_value = json_decode($mode_of_verification[0]['mode_of_verification']);

                                $mode_of_veri = $mode_of_verification_value->narcver;
                            } else {
                            ...
                            $field_array = array('clientid' => $check_record_exits['clientid'],
                                ...
                                'drug_com_ref' => '',
                                'appointment_date' => get_date_from_timestamp($value[2]),
                                'appointment_time' => date("H:i", strtotime($value[3])),
                                'drug_test_code' => $value[4],
                                'street_address' => $value[5],
                                'city' => $value[6],
                                'state' => $value[7],
                                'pincode' => $value[8],
                            ...
                            $insert_id = $this->drug_verificatiion_model->save($record);

                            $drug_com_ref = $this->drug_com_ref($insert_id);

                            $result_get_count = $this->common_model->get_count(array('controllers' => 'assign_drugs'));

                            $total_get_count = $result_get_count + 1;

                            $result_update_count = $this->common_model->update_count(array('count' => $total_get_count), array('controllers' => 'assign_drugs'));

                            auto_update_overall_status($check_record_exits['id']);
                            auto_update_tat_status($check_record_exits['id']);

                            $data['success'] = $drug_com_ref . " This Component Code Records Created Successfully";
        ...
        if ($this->input->is_ajax_request()) {
           ...
           // $assigned_user_id = $this->drug_verificatiion_model->get_assign_users_id('user_profile', false, array("`user_profile`.`status`,`user_profile`.`id`,`user_profile`.`email`,`user_profile`.`department`,`user_profile`.`user_name`,concat(`user_profile`.`firstname`,' ',`user_profile`.`lastname`) as fullname,`user_profile`.`profile_pic`,`user_profile`.`firstname`,`user_profile`.`lastname`,`roles`.`role_name`,`roles`.`groups_id`,`roles_permissions`.*"), array('user_profile.status' => STATUS_ACTIVE));
            ...
            $spreadsheet->getProperties()->setCreator(CRMNAME)
                ...
                ->setTitle(CRMNAME)
                ->setSubject('Drugs Import Template')
                ->setDescription('Drugs Import Template File for bulk upload');
            ...
            $spreadsheet->setActiveSheetIndex(0)
                ...
                ->setCellValue("C1", 'Appointment Date')
                ->setCellValue("D1", 'Appointment Time')
                ->setCellValue("E1", 'Drug Test Panel/Code')
            ...
            header("Content-Disposition: attachment;filename=Drugs Bulk Uplaod Template.xlsx");
            ...
            $json_array['file_name'] = "Drugs Bulk Uplaod Template";
    ...
    public function View_vendor_log($where_id)
    {
        ...
        $details = $this->drug_verificatiion_model->select_vendor_result_log(array('component_tbl_id' => "7", "component" => "narcver"), $where_id);

        if ($where_id && !empty($details)) {
            $details['attachments'] = $this->drug_verificatiion_model->select_file_from_vendor(array('id', 'file_name', 'real_filename'), array('view_venor_master_log_id' => $where_id, 'status' => 1, 'component_tbl_id' => 7));

            $details['attachments_file'] = $this->drug_verificatiion_model->select_file(array('id', 'file_name', 'real_filename'), array('drug_narcotis_id' => $details[0]['drugs_id'], 'status' => 1));
            ...
            echo $this->load->view('admin/drugs_ads_vendor_view', $details, true);
        } else {
            echo "<h4>Record Not Found</h4>";
        }
    ...
    public function View_vendor_log_cancel($where_id)
    {
        ...
        $details = $this->drug_verificatiion_model->select_vendor_result_log_cost(array('component_tbl_id' => "7", "component" => "narcver"), $where_id);

        if ($where_id && !empty($details)) {

            $details['details'] = $details[0];

            echo $this->load->view('admin/drugs_ads_vendor_view_cancel', $details, true);
        } else {
            echo "<h4>Record Not Found</h4>";
        }
    ...
    public function Save_vendor_details()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            log_message('error', 'Drugs Save Vendor  Details');
            try { 
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    $get_status = $this->drug_verificatiion_model->select_drugs_dt('view_vendor_master_log',array('id','final_status','case_id'),array('trasaction_id' => $transaction_id)); 
                     
                    if($get_status[0]['final_status'] != $frm_details['status'])
                    {
                        if($frm_details['status'] == 'wip')
                        {
                            $drugs_details_vendor = $this->drug_verificatiion_model->get_drugs_details_for_approval($get_status[0]['case_id']);
                     
                            $email_tmpl_data['subject'] = 'Drugs - Insufficiency cleared for_'.$drugs_details_vendor[0]['drug_com_ref'];
                            ...
                            $message .= "<table border = '2'  style='border-spacing: 10; border-collapse: collapse;'>
                                ...
                                <tr><td style='background-color: #EDEDED;text-align:center'>Client Name</td><td style='text-align:center'>". $drugs_details_vendor[0]['clientname']."</td></tr>
                                <tr><td style='background-color: #EDEDED;text-align:center'>Component Ref No</td><td style='text-align:center'>". $drugs_details_vendor[0]['drug_com_ref']."</td></tr>
                                <tr><td style='background-color: #EDEDED;text-align:center'>Candidate Name</td><td style='text-align:center'>". $drugs_details_vendor[0]['CandidateName']."</td></tr>
                                <tr><td style='background-color: #EDEDED;text-align:center'>Contact Nos</td><td style='text-align:center'>". $drugs_details_vendor[0]['CandidatesContactNumber']." | ".$drugs_details_vendor[0]['ContactNo1']." | ".$drugs_details_vendor[0]['ContactNo2']."</td></tr>
                                <tr><td style='background-color: #EDEDED;text-align:center'>Address</td><td style='text-align:center'>". $drugs_details_vendor[0]['street_address'].",".$drugs_details_vendor[0]['city'].",".$drugs_details_vendor[0]['pincode'].",".$drugs_details_vendor[0]['state']."</td></tr>
                            ...
                            $to_emails = $this->drug_verificatiion_model->vendor_email_id(array('vendors.id' => $drugs_details_vendor[0]['vendor_id']));
                    ...
                    $result = $this->drug_verificatiion_model->save_vendor_details("view_vendor_master_log", array_map('strtolower', $field_array), array('trasaction_id' => $transaction_id));
                    ...
                    $config_array = array('file_upload_path' => $file_upload_path, 'file_permission' => 'jpeg|jpg|png|pdf|tiff', 'file_size' => BULK_UPLOAD_MAX_SIZE_MB * 2000, 'view_venor_master_log_id' => $frm_details['view_vendor_master_log_id'], 'component_tbl_id' => 7, 'status' => 1);
                ...
                log_message('error', 'Drugs Verification::Save_vendor_details');
                log_message('error', $e->getMessage());
            }
        }
    }
    ...
    public function Save_vendor_details_cancel()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            log_message('error', 'Drugs Vendor Details Cancel');
            try {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $json_array['message'] = validation_errors('', '');
                   ...
                   $frm_details = $this->input->post();
                    ...
                    $drugs_details_vendor = $this->drug_verificatiion_model->get_drugs_details_for_approval($frm_details['case_id']);
                  
                    $email_tmpl_data['subject'] = 'Drugs - Stop check cases_'.date("d-M-Y H:i");
                    ...
                    $i = 1;
                    foreach ($drugs_details_vendor as $drugs_key => $drugs_value) {
            ...
            if(!empty($results) && $results == "Success")
            {
                ...
                $result = $this->drug_verificatiion_model->save_vendor_details_cancel($field_array, array('id' => $frm_details['update_id']));
                ...
                $drugs_vendor_result = $this->drug_verificatiion_model->update_drugs_vendor_log('credit_report_vendor_log', $field_array_drugs, array('id' => $frm_details['case_id']));
            ...
            echo_json($json_array);
                ...
                log_message('error', 'Credit_report::Save_vendor_details_cancel');
                log_message('error', $e->getMessage());
            }
        }
    }

     public function drugs_closure()
    {

        if ($this->input->is_ajax_request()) {

            if ($this->permission['access_drugs_list_activity'] == 1) {
                $frm_details = $this->input->post();
                $action = $frm_details['action'];
                ...
                if ($frm_details['action'] == 2 && $frm_details['closure_id'] != "") {
                    ...
                    foreach ($list as $key => $value) {
                        ...
                        $update_closure = $this->drug_verificatiion_model->update_closure_approval('view_vendor_master_log_closure_status', array('view_vendor_master_log_id' => $value, 'approve_reject_status' => 2, 'approve_reject_by' => $this->user_info['id'], 'reject_reasons' => $frm_details['reject_reason'], "approve_rejected_on" => date(DB_DATE_FORMAT)));

                        $update_closure1 = $this->drug_verificatiion_model->update_closure_approval_details('view_vendor_master_log', array('final_status' => 'wip'), array('id' => $value));
                    ...
                    foreach ($list as $key => $value) {
                        ...
                        $update_closure = $this->drug_verificatiion_model->update_closure_approval('view_vendor_master_log_closure_status', array('view_vendor_master_log_id' => $value, 'approve_reject_status' => 1, 'approve_reject_by' => $this->user_info['id'], "approve_rejected_on" => date(DB_DATE_FORMAT)));

                        $update_closure1 = $this->drug_verificatiion_model->update_closure_approval_details('view_vendor_master_log', array('final_status' => 'approve'), array('id' => $value));
    ...
    public function drugs_closure_entries($userid)
    {
        if ($this->input->is_ajax_request() && $userid) {

            log_message('error', 'Drugs Closure Entries');
            try {
                ...
                $details['vendor_executive_list'] = $this->drug_verificatiion_model->drugs_narcotics_case_list(array('view_vendor_master_log.status =' => 1,'component' => 'narcver'), $userid);
            } catch (Exception $e) {
                log_message('error', 'Drugs_narcotics::drugs_closure_entries');
                log_message('error', $e->getMessage());
            }
        ...
        echo $this->load->view('admin/drugs_vendor_closure_entries', $details, true);

    }

    public function drugs_closure_entries_vendor_insuff($userid)
    {
        if ($this->input->is_ajax_request() && $userid) {

            log_message('error', 'Drugs vendor insuff');
            try {
                ...
                $details['vendor_executive_list'] = $this->drug_verificatiion_model->drugs_case_list_insuff(array('view_vendor_master_log.status =' => 1, 'view_vendor_master_log.final_status =' => 'insufficiency', 'component' => 'narcver'), $userid);

            } catch (Exception $e) {
                log_message('error', 'Drugs_narcotics::drugs_narcotis_closure_entries_vendor_insuff');
                log_message('error', $e->getMessage());
            }
        ...
        echo $this->load->view('admin/drugs_vendor_insuff_entries', $details, true);
    }

    public function check_insuff_already_raised($where_id)
    {

        $drugs_details = $this->drug_verificatiion_model->get_all_drug_record(array('drug_narcotis.id' => $where_id));

        $check_insuff_raise = $this->drug_verificatiion_model->select_insuff(array('drug_narcotis_id' => $where_id, 'status' => STATUS_ACTIVE, 'insuff_clear_date is null' => null));
        ...
        if (empty($check_insuff_raise)) {
            $data['get_cands_details'] = $this->candidate_entity_pack_details($drugs_details[0]['cands_id']);
            $data['drugs_details'] = $drugs_details[0];

            echo $this->load->view('admin/drugs_insuff_view', $data, true);
    ...
    public function View_vendor_log1($where_id)
    {
        ...
        if ($where_id && !empty($details)) {
            ...
            echo $this->load->view('admin/drugs_ads_vendor_view_approve_tab', $details, true);
    ...
    public function export_to_excel()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            log_message('error', 'Drugs Export To Excel');
            try {
                ...
                header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                header("Content-Disposition: attachment;filename=Drugs Records of $client_name.xlsx");
                header('Cache-Control: max-age=0');
                // If you're serving to IE 9, then the following may be needed
                header('Cache-Control: max-age=1');
                ...
                $json_array['file_name'] = "Drugs Records of $client_name";
                ...
                log_message('error', 'Drugs_narcotics::export_to_excel');
                log_message('error', $e->getMessage());
            }
