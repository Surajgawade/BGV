...
class Identity extends MY_Controller
{
    ...
    public function identity_view_datatable()
    {
        ...
        $court_candidates = $this->identity_model->get_all_identity_record_datatable(false, $params, $columns);

        $totalRecords = count($this->identity_model->get_all_identity_record_datatable_count(false, $params, $columns));
        ...
        foreach ($court_candidates as $court_candidate) {
            ...
            $data_arry[$x]['id_number'] = (strlen($court_candidate['id_number']) == 12) ?  wordwrap($court_candidate['id_number'] , 4 , '-' , true ) : strtoupper($court_candidate['id_number']);
            ...
            $data_arry[$x]['encry_id'] = ADMIN_SITE_URL . "identity/view_details/" . encrypt($court_candidate['identity_id']);
            $x++;
        }
    ...
    public function add($candsid)
    {
        if ($this->input->is_ajax_request()) {
           ...
           // $data['assigned_user_id'] = $this->identity_model->get_assign_users_id('user_profile', false, array("`user_profile`.`status`,`user_profile`.`id`,`user_profile`.`email`,`user_profile`.`department`,`user_profile`.`user_name`,concat(`user_profile`.`firstname`,' ',`user_profile`.`lastname`) as fullname,`user_profile`.`profile_pic`,`user_profile`.`firstname`,`user_profile`.`lastname`,`roles`.`role_name`,`roles`.`groups_id`,`roles_permissions`.*"), array('user_profile.status' => STATUS_ACTIVE));
            ...
            echo $this->load->view('admin/identity_add', $data, true);
        } else {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                if (empty($error_msgs)) {
                    ...
                    if (isset($frm_details['upload_capture_image_identity'])) {

                        if ($frm_details['upload_capture_image_identity']) {

                            $upload_capture_image = explode("||", $frm_details['upload_capture_image_identity']);
                                    
                            foreach ($upload_capture_image as $key => $value) {
                                $key = $key + 1;

                                $file_name = $identity_com_ref . '-' .$key. '-' . date(UPLOAD_FILE_DATE_FORMAT) . '.png';
                                ...
                                $base64_to_jpeg = base64_to_jpeg($value, $uploadpath);
                                    if ($base64_to_jpeg) {
                                        ...
                                        $this->common_model->save('identity_files', ['file_name' => $file_name, 'real_filename' => $file_name,'status' => 1,'type' => 0,'identity_id' => $result]);
                                }
    ...
    public function get_assign_identity_list_view()
    {
        log_message('error', 'Identity List Assign in Assign View');
        try {

            $data['header_title'] = "Identity Verification";
            ...
            log_message('error', 'Identity::get_assign_identity_list_view');
            log_message('error', $e->getMessage());
        }
        echo $this->load->view('admin/assign_identity', $data, true);
    }
    ...
    {
        $this->load->model('assign_identity_model');
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if (!empty($cget_details)) {
               ...
               // $data['assigned_user_id'] = $this->identity_model->get_assign_users_id('user_profile', false, array("`user_profile`.`status`,`user_profile`.`id`,`user_profile`.`email`,`user_profile`.`department`,`user_profile`.`user_name`,concat(`user_profile`.`firstname`,' ',`user_profile`.`lastname`) as fullname,`user_profile`.`profile_pic`,`user_profile`.`firstname`,`user_profile`.`lastname`,`roles`.`role_name`,`roles`.`groups_id`,`roles_permissions`.*"), array('user_profile.status' => STATUS_ACTIVE));
                ...
                $data['attachments'] = $this->identity_model->select_file(array('id', 'file_name', 'real_filename', 'type'), array('identity_id' => $cget_details[0]['identity_id'], 'status' => 1));

                $check_insuff_raise = $this->identity_model->select_insuff(array('identity_id' => $cget_details[0]['identity_id'], 'status' => STATUS_ACTIVE, 'insuff_clear_date is null' => null));
                ...
                echo $this->load->view('admin/identity_edit_view_inside', $data, true);
            } else {
                echo "<h3>Something went wrong, please try again</h3>";
            }
        }
    ...
    public function update_form()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == false) {
                        ...
                        if ($frm_details['upload_capture_image_identity']) {
                            ...
                            $upload_capture_image = explode("||", $frm_details['upload_capture_image_identity']);
                                ...
                                foreach ($upload_capture_image as $key => $value) {
                                    ...
                                    $file_name = $frm_details['identity_com_ref'] . '-' .$key. '-' . date(UPLOAD_FILE_DATE_FORMAT) . '.png';
                                    $uploadpath = $file_upload_path . '/' . $file_name;
                                    $base64_to_jpeg = base64_to_jpeg($value, $uploadpath);
                                        if ($base64_to_jpeg) {
                                            log_message('error', 'Inside if condition success');
                                            $this->common_model->save('identity_files', ['file_name' => $file_name, 'real_filename' => $file_name,'status' => 1,'type' => 0,'identity_id' => $frm_details['identity_id']]);
                                        }
                ...
                if ($result) {
                    $user_activity_data = $this->common_model->user_actity_data(array('component' => "Identity",
                        'ref_no' => $frm_details['identity_com_ref'], 'candidate_name' => $frm_details['CandidateName'], 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id'], 'activity_type' => 'Manual', 'action' => 'Edit'));
    ...
    public function view_details($idenity_id = '')
    {
        if (!empty($idenity_id)) {
            $details = $this->identity_model->get_all_identity_record(array('identity.id' => decrypt($idenity_id)));

            if ($idenity_id && !empty($details)) {
                $data['header_title'] = 'Identity Verification';
                ...
                $check_insuff_raise = $this->identity_model->select_insuff(array('identity_id' => decrypt($idenity_id), 'status' => STATUS_ACTIVE, 'insuff_clear_date is null' => null));

                $data['reinitiated'] = ($details[0]['var_filter_status'] == 'Closed' || $details[0]['var_filter_status'] == 'closed') ? '1' : '2';

                $data['attachments'] = $this->identity_model->select_file(array('id', 'file_name', 'real_filename', 'type'), array('identity_id' => decrypt($idenity_id), 'status' => 1));
                ...
                $this->load->view('admin/identity_edit_view');
    ...
    public function add_result_model($where_id, $url)
    {
        ...
        if ($where_id && !empty($details)) {
            ...
            $candidate_details = $this->identity_model->select_identity_details('candidates_info',array('CandidatesContactNumber','ContactNo1','ContactNo2','cands_state','gender','DateofBirth'),array('candidates_info.id' => $details[0]['cands_id']));
            ...
            $data['attachments'] = $this->identity_model->select_file(array('id', 'file_name', 'status'), array('identity_id' => $where_id, 'type' => 1));
    ...
    public function add_verificarion_result()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    if (($frm_details['action_val'] != "Select") || ($frm_details['activity_last_id'] != "")) {
                        ...
                        $field_array = array('verfstatus' => $verfstatus['id'],
                            ...
                            'identity_id' => $frm_details['identity_id'],
                        ...
                        {
                            $this->identity_model->delete_uploaded_file($frm_details['remove_file']);
                        }
                        if (isset($frm_details['add_file'])) // delete uploaded file
                        {
                            $this->identity_model->add_uploaded_file($frm_details['add_file']);
                        }
                        ...
                        $result_identity = $this->identity_model->save_update_result_identity(array_map('strtolower', $field_array));

                        if ($verfstatus['id'] == 9 || $verfstatus['id'] == 27 || $verfstatus['id'] == 28) {

                            $get_vendor_log_deatail = $this->identity_model->check_vendor_status_closed_or_not(array('view_vendor_master_log.component' => "identity", 'view_vendor_master_log.component_tbl_id' => 9, 'identity.id' => $frm_details['identity_id']));

                            if (!empty($get_vendor_log_deatail)) {

                                $update_vendor_log_deatail = $this->identity_model->reject_cost_vendor(array('final_status' => 'cancelled', 'status' => 5, 'remarks' => 'Stop Check', 'rejected_by' => $this->user_info['id'], 'rejected_on' => date(DB_DATE_FORMAT), 'vendor_remark' => 'Stop Check', 'modified_on' => date(DB_DATE_FORMAT)), array('view_vendor_master_log.id' => $get_vendor_log_deatail[0]['id']));
                                ...
                                $identity_vendor_result = $this->identity_model->update_identity_vendor_log('identity_vendor_log', $field_array_identity, array('id' => $get_vendor_log_deatail[0]['identity_vendor_log_id']));
                        ...
                        $config_array = array(
                                ...
                                'file_id'           => $frm_details['identity_id'], 
                                'component_name'    => 'identity_id'
                        );
                        ...
                        if (empty($error_msgs)) {
                            ...
                            if (isset($frm_details['upload_capture_image_identity_result'])) {

                                if ($frm_details['upload_capture_image_identity_result']) {

                                    $upload_capture_image = explode("||", $frm_details['upload_capture_image_identity_result']);
                                    ...
                                    foreach ($upload_capture_image as $key => $value) {
                                        ...
                                        $base64_to_jpeg = base64_to_jpeg($value, $uploadpath);
                                            if ($base64_to_jpeg) {
                                                log_message('error', 'Inside if condition success');
                                                $this->common_model->save('identity_files', ['file_name' => $file_name, 'real_filename' => $file_name,'status' => 1,'type' => 1,'identity_id' => $frm_details['identity_id']]);
                                            }
                ...
                log_message('error', 'Identity::add_verificarion_result');
                log_message('error', $e->getMessage());
            }
        }
    }
        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    $result_identity = $this->identity_model->save_update_result_identity(array_map('strtolower', $field_array), array('id' => $frm_details['result_update_id']));
                    ...
                    if (isset($frm_details['upload_capture_image_identity_ver_result'])) {

                        if ($frm_details['upload_capture_image_identity_ver_result']) {


                            $upload_capture_image = explode("||", $frm_details['upload_capture_image_identity_ver_result']);
    ...
    public function identity_result_list($emp_id = '')
    {
        if ($emp_id && $this->input->is_ajax_request()) {
            $identity_result = $this->identity_model->select_result_log(array('identity_id' => $emp_id, 'activity_log_id !=' => null));
            ...
            if (!empty($identity_result[0]['id'])) {
                $l = 1;

                foreach ($identity_result as $key => $value) {
                    ...
                    if ($value['file_names']) {
                        ...
                        for ($i = 0; $i < count($files); $i++) {
                            $url = "'" . SITE_URL . IDENTITY . $value['clientid'] . '/';
                            $actual_file = $files[$i] . "'";
                            $myWin = "'" . "myWin" . "'";
                    ...
                    if ($l == 1) {
                        $html_view .= '<td><button data-id="showAddResultModel" data-url ="' . ADMIN_SITE_URL . 'identity/identity_result_list_idwise/' . $value['id'] . '/' . str_replace(" ", "", $value['activity_action']) . '" data-toggle="modal" data-target="#showAddResultModel1"  class="btn-info  showAddResultModel"> View </button></td>';
                    } else {
                        $html_view .= '<td> </td>';
                    }
                    $html_view .= '</tr>';
    ...
    public function identity_result_list_idwise($where_id, $url)
    {
        $details = $this->identity_model->select_result_log1(array('identity_ver_result.id' => $where_id));

        if ($where_id && !empty($details)) {

            $candidate_details = $this->identity_model->select_identity_details('candidates_info',array('CandidatesContactNumber','ContactNo1','ContactNo2','cands_state','gender','DateofBirth'),array('candidates_info.id' => $details[0]['candsid']));
            ...
            $data['attachments'] = $this->identity_model->select_file(array('id', 'file_name', 'status'), array('identity_id' => $details[0]['identity_id'], 'type' => 1));

            echo $this->load->view('admin/identity_add_result_model_view_log', $data, true);
        } else {
            echo "<h4>Record Not Found</h4>";
        }
    }
    ...
    public function insuff_raised()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if (empty($check)) {
                $result = $this->identity_model->save_update_insuff(array('insuff_raised_date' => convert_display_to_db_date($insff_date), 'insuff_raise_remark' => $this->input->post('insuff_raise_remark'), 'status' => STATUS_ACTIVE, 'insff_reason' => $insff_reason, 'created_by' => $this->user_info['id'], 'identity_id' => $identity_id, 'auto_stamp' => 1));

                if ($result) {
                    $user_activity_data = $this->common_model->user_actity_data(array('component' => "Identity", 'ref_no' => $ref_no, 'candidate_name' => $CandidateName, 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id'], 'activity_type' => 'Manual', 'action' => 'Insuff Raised'));
                }
    ...
    public function insuff_tab_view($emp_id = '')
    {
        if ($this->input->is_ajax_request() && $emp_id) {
            ...
            echo $this->load->view('admin/identities_insuff_view', $data, true);
        } else {
    ...
    public function insuff_clear()
    {
        ...
        if ($this->input->is_ajax_request() && $this->permission['access_identity_list_insuff_clear'] == 1) {
            if (convert_display_to_db_date($frm_details['insuff_clear_date']) >= convert_display_to_db_date($frm_details['check_insuff_raise'])) {
                ...
                $get_vendor_log_deatail = $this->identity_model->check_vendor_status_insufficiency(array('view_vendor_master_log.component' => "identity", 'view_vendor_master_log.component_tbl_id' => 9,
                    'view_vendor_master_log.final_status' => 'insufficiency', 'identity.id' => $frm_details['clear_update_id']));

                if (!empty($get_vendor_log_deatail)) {

                    $update_vendor_log_deatail = $this->identity_model->reject_cost_vendor(array('final_status' => 'wip', 'vendor_remark' => '', 'modified_on' => date(DB_DATE_FORMAT)), array('view_vendor_master_log.id' => $get_vendor_log_deatail[0]['id']));

                }

                if ($result) {
                    $user_activity_data = $this->common_model->user_actity_data(array('component' => "Identity", 'ref_no' => $frm_details['component_ref_no'], 'candidate_name' => $frm_details['CandidateName'], 'created_on' => date(DB_DATE_FORMAT), 'created_by' => $this->user_info['id'], 'activity_type' => 'Manual', 'action' => 'Insuff Cleared'));
                }
                    ...
                    for ($i = 0; $i < $files_count; $i++) {
                        ...
                        if ($this->upload->do_upload('attchment')) {
                            array_push($file_array, array(
                                ...
                                'identity_id' => $frm_details['clear_update_id'],
                                'type' => 2)
                            );
                        } else {
    ...
    public function insuff_delete()
    {
        ...
        $insuff_data = $this->input->post('insuff_data');
        if ($this->input->is_ajax_request() && $this->permission['access_identity_list_insuff_delete'] == 1) {
    ...
    public function identity_final_assigning()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->permission['access_identity_aq_allow'] == 1) {
                $frm_details = $this->input->post();
    ...
    public function View_vendor_log($where_id)
    {
        ...
        $details = $this->identity_model->select_vendor_result_log(array('component_tbl_id' => "9", "component" => "identity"), $where_id);

        if ($where_id && !empty($details)) {
            $details['attachments'] = $this->identity_model->select_file_from_vendor(array('id', 'file_name', 'real_filename'), array('view_venor_master_log_id' => $where_id, 'status' => 1, 'component_tbl_id' => 9));

            $details['attachments_file'] = $this->identity_model->select_file(array('id', 'file_name', 'real_filename'), array('identity_id' => $details[0]['identity_id'], 'status' => 1));
            ...
            echo $this->load->view('admin/identity_ads_vendor_view', $details, true);
        } else {
            echo "<h4>Record Not Found</h4>";
        }
    ...
    public function vendor_logs_cost()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $details = $this->identity_model->select_vendor_result_log_cost_details(array('components_tbl_id' => "9"), $frm_details['id']);
    ...
    public function View_vendor_log_cost($where_id)
    {
        ...
        $details = $this->identity_model->select_vendor_result_log_cost(array('component_tbl_id' => "9", "component" => "identity"), $where_id);

        if ($where_id && !empty($details)) {

            $details['details'] = $details[0];

            echo $this->load->view('admin/identity_ads_vendor_view_cost', $details, true);
        } else {
            echo "<h4>Record Not Found</h4>";
        }
    ...
    public function View_vendor_log_cancel($where_id)
    {
        ...
        if ($where_id && !empty($details)) {
            ...
            echo $this->load->view('admin/identity_ads_vendor_view_cancel', $details, true);
        } else {
            echo "<h4>Record Not Found</h4>";
        }
    ...
    public function Save_vendor_details_cost()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $result = $this->identity_model->save_vendor_details_costing("vendor_cost_details", $field_array);
    ...
    {
        if ($this->input->is_ajax_request()) {
            $details['detail'] = $this->identity_model->get_vendor_cost_aprroval_details();
        ...
        echo $this->load->view('admin/identity_vendor_cost_details', $details, true);
        //echo $this->load->view('admin/address_ajax_tab',$data,TRUE);
    ...
    public function remove_uploaded_file($id)
    {
        ...
        if ($this->input->is_ajax_request()) {
            $result = $this->identity_model->save_update_identity_files(array('status' => 2), array('id' => $id));
